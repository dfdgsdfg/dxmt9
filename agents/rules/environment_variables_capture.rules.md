...
| `DXMT_DUMP_TEXTURE_HANDLE` / `DXMT_DUMP_TEXTURE_DIR` | CPU-shadow texture dump at upload. Writes the existing BMP view plus a full raw `.bin` for every selected 2D/cube/volume subresource; the raw filename records dimensions, row pitch, and slice pitch so signed/non-color formats and mip chains remain inspectable | unset | core/texture path |
...
| `DXMT9_RENDER_TAPE_CAPTURE` | Enable the production Render Tape producer. With `DXMT9_RENDER_TAPE_OUTPUT_ROOT` set, the unix renderer selects a capture-only streaming profile with a 2,048-page / 8-MiB Arena. EventSerial/default capture retains its existing 512-page queue admission bound; only an authenticated token+event-ordinal capture event in queue-immutable `DXMT9_RENDER_IDENTITY_MODE=segment` uses the call-local 64-page-per-source SegmentSerial planner bound, allowing capture-authority source/sequence and pre-reorder FrameGraph identity to be published. Non-captured startup raws, capture-off, and EventSerial use 512 pages/source and one source unless an explicit proven mode requires otherwise. Canonical events larger than one SegmentSerial source are split into ordered source rows by the atomic Arena batch lease; payload/control/Ready admission and abort are all-or-nothing, and an indivisible layout over the planner bound remains a typed pre-effect rejection rather than a widened queue source. Captured first-use attachment seeds are loaded and live-out attachments are stored so capture and fresh-provider replay share deterministic action semantics. Capture without an output publisher and normal rendering when unset do not select this profile or change ordinary streaming bounds. **Side effect on the PE fast path:** setting this var (to any value the flag predicate accepts) also disables the per-(texture, level) surface-handle cache in `d3d9_pe_device_child_surface.cpp`, so `GetSurfaceLevel` / `GetCubeMapSurface` go back to resolving a fresh unix `D9CSurface` handle, wire identity and descriptor per call. The cache gives every level one stable wire identity, which would make the tape's per-identity ObjectDestroy and surface-alias registration ambiguous; the capture lane keeps the uncached path rather than reason about it. A capture run is therefore not a valid sample of that getter's cost | `0` | `d3d9_pe_device.cpp`, `d3d9_pe_device_child_surface.cpp`, `dxmt9_device.cpp`, `dxmt9_command_queue.cpp` |
...
| `DXMT9_DUMP_COLOR_ATTACHMENT_AFTER_DRAW` / `_PATH` / `_DIR` / `_HANDLE` / `_INDEX` / `_SEQ` / `_ENC` / `_DRAW(S)` / `_COMMAND_INDEX(_MIN/_MAX)` / `_COMMAND_DRAW_INDEX` / `_TEXTURE0(S)` | Dump a raw color attachment at a selected pass end or, with `AFTER_DRAW=1`, immediately after a selected replay draw. `_COMMAND_DRAW_INDEX` addresses an exact subdraw within an expanded DrawRun command, avoiding ambiguity when all subdraws share one encoder draw ordinal. `_PATH` captures the first match; `_DIR` preserves distinct matching identities | unset |
