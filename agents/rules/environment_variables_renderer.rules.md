...
| `DXMT9_RENDER_PARTITION_MODE` | Queue-immutable partition execution selector. Unset or `identity` uses the allocation-free identity cursor. `serial` runs the bounded production planner after final Traditional/FrameGraph replay order and DCE selection, subdividing only large DrawRuns at indexed-merge-preserving edges before complete validation and serial consumption. `parallel` resolves to `ExplicitParallel`, extracts sealed source-local passes from the final replay order, and partitions them into two to 16 children. A single DrawRun is divided evenly with every child at least 64 draws; a multi-command pass preserves whole commands and order, closes each non-final child at the earliest qualifying 64-draw prefix while retaining a complete 64-draw suffix, and absorbs a thin final suffix. It then revalidates every source-qualified locator, draw, ABI, PSO, and uniform value before enforcing the pure economics gate, which rejects child-size imbalance greater than 64; rejection returns to exact serial replay before render-pass preparation or parent Metal effects. Accepted passes create a real WMT `MTLParallelRenderCommandEncoder` and dispatch ordered child encoders on the bounded executor. Clear, Present, actions, sidecars, completion, and command-buffer ownership stay on the coordinator. One immutable pass proof selects either Stage 1 direct binding or Stage 2b direct constant buffers at slots 0/3; each child has an independent binding shadow, while slot-30 tables, resource arrays, mixed ABIs, PSO-rebuilding overrides, UP, tile FFP, carried/incomplete passes, active sidecars, and unresolved late Store actions fall back before effects. Empty and unknown values fail closed to identity. Partition edges do not create render-pass, command-buffer, action, or completion boundaries. Matched GT2 evidence reduced encode stage wall but regressed throughput 6.44%, so the default remains `identity`. | identity |
