...
| `DXMT9_RENDER_PARTITION_MODE` | Queue-immutable partition execution selector. Unset or `identity` uses the allocation-free identity cursor. `serial` runs the bounded production planner after final Traditional/FrameGraph replay order and DCE selection, subdividing only large DrawRuns at indexed-merge-preserving edges before complete validation and serial consumption. `parallel` resolves to the distinct `ExplicitParallel` provider and runs that same validated planner. With perf observation enabled, it extracts a fixed batch of source-local logical-pass candidates from the final replay order; Clear and Present remain coordinator-owned and every child is a validated DrawRun range. Static eligibility additionally requires explicit proof-owner callbacks for canonical resource identity and one fixed render route, plus explicit query/update/capture/initializer/ordered-control/sidecar absence and a nonzero action epoch. The current production seam supplies pool-backed identity and route proofs but cannot yet prove those coordinator facts before effects, so it records candidates and precise grouped rejections, publishes no eligible pass, and uses the unchanged serial consumer. Real WMT `MTLParallelRenderCommandEncoder` calls and a worker pool remain unimplemented. Empty and unknown values fail closed to identity. Partition edges do not create render-pass, command-buffer, action, or completion boundaries. | identity |
...
| `parallel_pass_shadow_sealed`, `parallel_pass_shadow_eligible`, `parallel_pass_shadow_eligible_max` | Boundary-complete candidates, candidates satisfying every explicit static proof, and maximum eligible passes in one batch. Current production is expected to report zero eligible until coordinator-proof plumbing lands. |
