...
| `DXMT9_RENDER_IDENTITY_MODE` | Queue-immutable CPU-ready source identity granularity. `event` (default) admits one EventSerial source per raw event; `segment` opts into the bounded SegmentSerial multi-source transaction only after complete planner validation. Empty and unknown values fail closed to `event`. | event |
