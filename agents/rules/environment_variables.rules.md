...
| `DXMT9_MEASURE_INDEX_REUSE` | Diagnostic-only encoder breakdown extension: scan accessible index-buffer bytes and report per-encoder indexed reference count, draw-local unique index estimate, and reuse ratio. Use with `DXMT9_PERF_ENCODER_BREAKDOWN=1`/`run_3dmark05_perf_probe.sh --measure-index-reuse` to compare dxmt's unique-index estimate with Xcode `VS Invocations`; off by default because it reads index data on the draw path. | `0` |
