...
| `DXMT_EXPERIMENT_PROFILE` | Select the experiment runtime profile (`debug` / `perf`); `perf` selects the shared perf-path runtime defaults, including `DXMT9_OFFLOAD_COMMIT_REPLAY=1` while keeping `DXMT9_OPTIMIZE_OPAQUE_DEPTH_INDEX_CACHE=0` unless explicitly opted in. The index-reorder default was separated from offload after a dynamic-index STALKER workload exposed dominant rejected-candidate CPU cost. Caller env always overrides, and `run_3dmark05_perf_probe.sh` pins both values explicitly for recipe determinism. Falls back to `DXMT_PROFILE`, then **`perf`** (default since 2026-07-29). `debug` is the explicit opt-in for diagnosing a wild failure: it enables the Metal validation layer (`DXMT_VALIDATE=1`) and debug-level logging with `WINEDEBUG` unset, which costs `3.2x` on `sampled_avg_fps` (SFIV: `13.5` vs `43.0`) and `5.3x` on the steady-body median (`11.3` vs `59.7`), with a `619 MB`-to-`1.1 GB` log against `22 MB` — the older "roughly 4x (`11.3` vs `43.0`)" line compared a median against an average and understated the median cost; see `agents/rules/test_wild.rules.md` — it is **not** a performance-measurement configuration. The resolved profile, its source variable, and the effective validate/log-level/WINEDEBUG/perf-counter values are recorded by the launcher as `[experiment] profile: ...` and land in `result.json:profile` plus the `3dmark05-perf-summary.md` header, so any measurement states the configuration it ran under |
