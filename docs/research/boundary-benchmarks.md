...
All four launchers (`experiments/launchers/dxmt9_perf_probe_*.sh`) call
...
### (b) Bridge-ABI throughput — `dxmt9_perf_probe_bridge_empty.sh` 🟢 HIGH
...
### (c) Encode-only throughput — `dxmt9_perf_probe_encode_replay.sh` 🟢 HIGH
...
### (e) Sub-CB chain sensitivity — `dxmt9_perf_probe_chain_parametric.sh` 🟢 HIGH
...
### (f) Drawable-acquire isolation — `dxmt9_perf_probe_present_loop.sh` 🟡 MEDIUM
