...
| H8 | The `v0.0.1` tag was a useful early coherent screenshot-diff artifact, but the last known GT1 visual-safe code point is `v0.0.3`; screenshot diffs remain broad corruption finders, not raw pixel gates | superseded by `v0.0.3` | baselines-visual-capture.02, [snapshot-cache-visual.01](../snapshot-cache/snapshot-cache-visual.01.md) |
...
[hidden-backend-storage](../hidden-backend-storage/index.md). The post-visualfix baselines-frame60.02
