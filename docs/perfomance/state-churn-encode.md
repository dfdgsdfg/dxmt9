...
| H121 | Argbuf reopen/probe children must rank in the standalone summary | accepted summary tooling | [[state-churn-encode-encode-phase.109]] adds reopen/post child rows and cbuf cached-repoint/content-probe rows to the `Argbuf CPU Derived` ranking. This is not a runtime win; it keeps the next 120s argbuf scout from burying reopen/post attribution under aggregate `argbuf_open`, and lets the next candidate choose between table open/reopen, cached repoint, content probe, or dirty VS cbuf update before spending an A/B run |
...
| H178 | Disabling the resource-shape memo does not fix the then-suspected black-foreground visual class | rejected visual fix; rejected FPS lever | [[state-churn-encode-encode-phase.168]] runs the targeted opt-out `DXMT9_DISABLE_ENCODE_SLOT_PSO_RESOURCE_SHAPE_MEMO=1`. The h190 counters prove the memo is fully disabled (`resource_shape_memo_* = 0`) and repeated work moves to probe-key memo hits (`169,745`), but `actual.png` still contains the sampled dark foreground/silhouette class. The run remains in the same cadence class as h189: command buffers per present `3.999`, ready depth `1.000`, completion wait `26.723ms/present`, and `draw_key_resolve=1.108ms/present`. H182 later shows this window class also exists in `v0.0.3`; keep the stale resource-shape memo branch closed unless a future same-frame A/B contradicts it |
...
([[state-churn-encode-encode-phase.123]]) keeps the same conclusion after the
...
otherwise.
[[state-churn-encode-encode-phase.127]] removes the queue-observation legacy
...
[[state-churn-encode-encode-phase.134]] adds the missing attribution surface for
...
[[state-churn-encode-encode-phase.135]] then measures the width of that float
...
cbuf storage.
[[state-churn-encode-encode-phase.132]] and
[[state-churn-encode-encode-phase.133]] close the unsafe table-sharing branch.
...
[[state-churn-encode-encode-phase.143]] closes that ABI precondition without
...
[[state-churn-encode-encode-phase.144]] then adds that bounded runtime scout
...
[[state-churn-encode-encode-phase.177]] closes the next pending-flush
...
design. [[state-churn-encode-encode-phase.184]] records the resulting owner
review and keeps `v0.0.3` as the current visual-safe anchor.

[[state-churn-encode-encode-phase.185]] implements the first direct-compact
