...
bottleneck — that is owned by [hidden-backend-storage](hidden-backend-storage.md).

## Hypotheses & verdicts

| # | Hypothesis | Verdict | Evidence |
|---|-----------|---------|----------|
| H1 | The per-draw encode path stays hot because ~99.9% of draws fail to batch | accepted | [state-churn-encode-drawrun.01](state-churn-encode/state-churn-encode-drawrun.01.md) (580 submits vs 913k draws) |
| H2 | A draw-run cannot blindly cross a constant-upload record (one shared uniform) | accepted | [state-churn-encode-drawrun.02](state-churn-encode/state-churn-encode-drawrun.02.md) (ConstantUpload stop; per-draw snapshot fallback) |
| H3 | Draw-run breaks are offset/stride churn | rejected | [state-churn-encode-stream.01](state-churn-encode/state-churn-encode-stream.01.md), [state-churn-encode-stream.02](state-churn-encode/state-churn-encode-stream.02.md) (handle churn 81.5-81.9%) |
| H4 | Handle churn is per-draw object creation (lock/rename) | rejected | [state-churn-encode-stream.03](state-churn-encode/state-churn-encode-stream.03.md) (bounded ~184/93 handles, managed-pool alternation) |
| H5 | State-delta breaks are dominated by exact stream+IB pairs | accepted | [state-churn-encode-statedelta.01](state-churn-encode/state-churn-encode-statedelta.01.md)→[state-churn-encode-statedelta.03](state-churn-encode/state-churn-encode-statedelta.03.md) (82.17% stream+IB-only) |
| H6 | A per-draw stream/IB binding override cuts encode CPU without moving GPU or churn | accepted (CPU win) | [state-churn-encode-binding.01](state-churn-encode/state-churn-encode-binding.01.md) (-30.13% stream-bind CPU, GPU +0.03%) |
| H7 | More draw/submission batching moves the GPU limiter | rejected | [state-churn-encode-batch.01](state-churn-encode/state-churn-encode-batch.01.md) (VS write flat at ~1627 MiB) |
| H8 | Disabling auto-expand-indexed reduces top-pass GPU buffer writes | rejected (GPU); inconclusive (correctness) | [state-churn-encode-expand.01](state-churn-encode/state-churn-encode-expand.01.md), [state-churn-encode-expand.02](state-churn-encode/state-churn-encode-expand.02.md) |
| H9 | Extending `_skipped` bind-cache pattern to vertex_buffer / index_buffer / pipeline / rasterizer / viewport / scissor / depth_state cuts per-CB encode below the 16.67 ms vsync slot | rejected | [present-pacing-bind-cache-work-a.01](present-pacing/present-pacing-bind-cache-work-a.01.md) (0% wallclock movement; `encode_chunk_cpu_ms` rose) |
...
| H11 | The old unattributed `encode_draw_cpu_ms` remainder is mostly argbuf setup and binding-packet construction/bookkeeping | accepted attribution | [state-churn-encode-encode-phase.01](state-churn-encode/state-churn-encode-encode-phase.01.md), [state-churn-encode-encode-phase.02](state-churn-encode/state-churn-encode-encode-phase.02.md) (`argbuf_cbuf_update=3.92s`, `binding_packet_cache=1.87s`, `argbuf_open=1.21s`) |
| H12 | Skipping clean/no-op argbuf cbuf updates is enough to move the encode budget | rejected as major lever; accepted micro | [state-churn-encode-encode-phase.03](state-churn-encode/state-churn-encode-encode-phase.03.md) (272,956 clean skips, but cbuf-update only -39.7ms; 778,587 dirty writes remain) |
| H13 | Dirty-bit-only partial cbuf repoint can reuse clean categories on argbuf reopen | rejected; new attribution accepted | [state-churn-encode-encode-phase.04](state-churn-encode/state-churn-encode-encode-phase.04.md) (0 cached repoints; 760,157 no-dirty hash mismatches force conservative full dirty upload) |
| H14 | Per-category cbuf identity can repoint cached VS/PS/FFPPS entries on no-dirty payload mismatch | accepted CPU win | [state-churn-encode-encode-phase.05](state-churn-encode/state-churn-encode-encode-phase.05.md) (`encode_draw_cpu_ms` -3.42s, transient bytes -62.05%, dirty cbuf calls -41.88%) |
| H15 | The current category-identity checkout still renders a normal GT1 frame and stable counters | accepted smoke | [state-churn-encode-encode-phase.06](state-churn-encode/state-churn-encode-encode-phase.06.md) (`actual.png` normal robot/flare/HUD frame; cbuf repoint calls +0.02% vs identity-r1) |
| H16 | The remaining binding-packet cache cost is mostly full key probe/equality rather than pure miss/store cost | accepted attribution | [state-churn-encode-encode-phase.07](state-churn-encode/state-churn-encode-encode-phase.07.md) (`probe=939ms`, `key=495ms`, hit rate 85.51%, collision share of misses 99.92%; attribution timers only) |
| H17 | Hashing/probing the already-built binding packet plan directly removes the hot key/probe cost | accepted CPU win | [state-churn-encode-encode-phase.08](state-churn-encode/state-churn-encode-encode-phase.08.md) (`binding_packet_cache` -57.34%, `binding_packet` -30.96%, `encode_draw` -7.40% vs identity smoke; normal GT1 frame) |
| H18 | `argbuf_open` is mostly Metal argument-encoder retarget cost | rejected; attribution accepted | [state-churn-encode-encode-phase.09](state-churn-encode/state-churn-encode-encode-phase.09.md) (`setArgumentBuffer=115ms`; reserve `746ms`, table bind `193ms`, skip `0`) |
| H19 | Skipping transient arena overlap scans on the non-wrapped append path reduces argbuf-open CPU | accepted CPU win | [state-churn-encode-encode-phase.10](state-churn-encode/state-churn-encode-encode-phase.10.md) (`reserve` -51.95%, `argbuf_setup` -16.33%, `encode_draw` -3.87%; same `1680` presents) |
| H20 | Dirty cbuf categories often still match encoder-local cached identities and can be repointed | rejected | [state-churn-encode-encode-phase.11](state-churn-encode/state-churn-encode-encode-phase.11.md) (VS/PS/FFPPS dirty identity hits all `0`; probe removed) |
| H21 | The remaining `stream_bind` parent has one dominant bind class | rejected as single-class; attribution accepted | [state-churn-encode-encode-phase.12](state-churn-encode/state-churn-encode-encode-phase.12.md) (texture/sampler `1065ms`, index `670ms`, shader stream `497ms`, raster `389ms`; FFP stream negligible) |
| H22 | Texture/sampler bind cost is spread across fragment/vertex/resource-array/LOD-bias lanes | rejected as broad split; attribution accepted | [state-churn-encode-encode-phase.13](state-churn-encode/state-churn-encode-encode-phase.13.md) (fragment resolve `575ms`, fragment direct `317ms`; resource-array, vertex textures, and LOD bias all `0`) |
| H23 | Sampler binds can skip `samplerStateFor()` before materializing a Metal handle | accepted CPU win | [state-churn-encode-encode-phase.14](state-churn-encode/state-churn-encode-encode-phase.14.md) (`sampler_lookup_skipped_prehandle=2,108,453`, texture/sampler parent -18.84%, encode_draw -117ms) |
| H24 | Direct sampler shadowing should reuse the packet's sampler-state hash instead of rehashing each entry | accepted CPU win | [state-churn-encode-encode-phase.15](state-churn-encode/state-churn-encode-encode-phase.15.md) (`fragment_direct` -68.425ms, texture/sampler parent -69.616ms, `encode_draw` -126.622ms; heavy split counters are opt-in) |
| H25 | Fragment texture resolve can skip `findTexture()` / `textureForShaderRead()` by matching D3D texture handle + sRGB before materializing the Metal handle | rejected as default CPU win; removed from hot path | [state-churn-encode-encode-phase.16](state-churn-encode/state-churn-encode-encode-phase.16.md) (`1,206,015` pre-resolve skips and local resolve -28.902ms, but texture/sampler parent +48.224ms; temporary default-off smoke proved skip counter `0`; after branch removal, texture/sampler parent returned to baseline `822.864 -> 821.007`) |
| H26 | Remaining cbuf update time is mainly Metal `setBuffer` or transient upload | rejected; attribution accepted | [state-churn-encode-encode-phase.17](state-churn-encode/state-churn-encode-encode-phase.17.md) (`setBuffer=114.568ms`, upload `276.019ms`, build `477.921ms`, inferred residual `954.163ms`; VS residual `618.150ms`) |
| H27 | The phase.17 cbuf residual is mostly upload-plan or observer cost | rejected; binding-hash attribution accepted | [state-churn-encode-encode-phase.18](state-churn-encode/state-churn-encode-encode-phase.18.md) (`binding_hash=570.070ms`, VS `489.627ms`; `upload_plan=43.287ms` nested in build; observer `0`) |
| H28 | `hashConstantBufferBytes()` is still required in the default cbuf cache path | rejected; CPU win accepted | [state-churn-encode-encode-phase.19](state-churn-encode/state-churn-encode-encode-phase.19.md) (`binding_hash=570.070 -> 0ms`; cbuf update `1.216 -> 0.875ms/present`; encode_draw `10.359 -> 10.006ms/present`) |
| H29 | Dirty cbuf upload must first materialize full `VsConsts` / `PsConsts` structs | rejected; prefix-preserving CPU win accepted | [state-churn-encode-encode-phase.20](state-churn-encode/state-churn-encode-encode-phase.20.md) (build `0.333815 -> 0.175342ms/present`; cbuf update `0.875284 -> 0.679652ms/present`; live-range-only prefix zeroing failed visual smoke) |
| H30 | Binding-packet sampler plans must rehash `FlatStateSet` payloads after snapshot key build | rejected; CPU win accepted | [state-churn-encode-encode-phase.21](state-churn-encode/state-churn-encode-encode-phase.21.md) (`binding_packet_plan` `0.666122 -> 0.599724ms/present`; packet parent `-4.42%`; full sampler equality retained) |
| H31 | Forcing full VS/PS cbuf uploads clearly fixes the suspected black/translucent GT1 geometry | inconclusive visual check; full-upload fallback rejected | [state-churn-encode-encode-phase.22](state-churn-encode/state-churn-encode-encode-phase.22.md) (`argbuf_hybrid_bytes_per_encoder` +519.59%; no obvious visual normalization; `actual.png` frame drift prevents exact verdict) |
| H32 | Draw submission batches can reuse stale argbuf cbuf slices when per-draw uniform payloads change but base hot constant hashes do not | accepted correctness fix; visual smoke restored | [state-churn-encode-encode-phase.23](state-churn-encode/state-churn-encode-encode-phase.23.md) (disable-batch A/B localizes artifact; payload component hash identity keeps batching and cuts dirty-fix traffic `-34.24%`) |
| H33 | Current stream/IB churn is a direct backend-storage Xcode candidate | rejected-current; handle-stable A/B required | [state-churn-encode-stream.04](state-churn-encode/state-churn-encode-stream.04.md) (`60/2` binding tuple changes `160/187`, unique tuples `58`, stream1 extra changes `111`, explicit writers `0.089 B/vertex`) |
| H34 | Row-scoped staging can isolate stream/IB handle churn without changing draw/PSO/argbuf shape | accepted diagnostic; GPU win unproven | [state-churn-encode-stream.08](state-churn-encode/state-churn-encode-stream.08.md) (`60/2` stream handle changes `271 -> 0`, IB `160 -> 0`, PSO `48 -> 48`, argbuf table `5056 -> 5056`, but staged copy `7.38 MiB` and offset churn remains) |
| H35 | Stream/IB handle identity owns the Xcode hidden backend write bucket | rejected as first-order GPU owner | [state-churn-encode-stream.09](state-churn-encode/state-churn-encode-stream.09.md) (`60/2` stream/IB handle changes `271/160 -> 0/0`, but GPU `19.184 -> 19.278 ms`, VS write `981.159 -> 981.166 MiB`, VS invocations unchanged) |
| H36 | `bridge_commit_latency_ns` is raw Wine bridge/ABI overhead | rejected as bridge owner; commit_chunk replay accepted | [state-churn-encode-encode-phase.24](state-churn-encode/state-churn-encode-encode-phase.24.md) (`bridge_commit_latency=22.473s`, replay `21.839s`, import `88ms`, handle `542ms`, draw-batch-submit `3.234s`) |
| H37 | `commit_chunk_replay_cpu_ms` is mostly draw-run scan/state/const dispatch | rejected; queued submission/snapshot accepted | [state-churn-encode-encode-phase.25](state-churn-encode/state-churn-encode-encode-phase.25.md) (replay `22.224s`, queue submission `9.927s`, nested snapshot `7.697s`, draw-batch submit `3.229s`, draw-run submit `2.094s`) |
| H38 | `CommandQueue` submit residual can be localized before changing batching behavior | accepted attribution | [state-churn-encode-encode-phase.26](state-churn-encode/state-churn-encode-encode-phase.26.md) (`commit_chunk_draw_batch_submit_cpu_ms=3629.383`; batch append `2379.837ms`, compat scan `559.625ms`; resource mark/slot/chunk commit are small) |
| H39 | Snapshot cache misses may be inflated by declared draw-packet deltas that do not actually change non-binding state | rejected-current | [state-churn-encode-encode-phase.28](state-churn-encode/state-churn-encode-encode-phase.28.md) (`draw_packet_declared_nonbinding=419,990`, `actual_nonbinding=419,990`, `redundant_nonbinding=0`) |
| H40 | Snapshot/uniform cost is mostly real payload construction and hashing, not redundant invalidation | accepted local CPU win; FPS flat | [state-churn-encode-encode-phase.28](state-churn-encode/state-churn-encode-encode-phase.28.md), [snapshot-cache-snapshot.10](snapshot-cache/snapshot-cache-snapshot.10.md) (`uniform_refresh 2014.263ms→814.507ms`, snapshot submission `7622.807ms→6495.069ms`, sampled FPS `15.717→15.752`) |
| H41 | The batch append owner is raw payload byte copy | rejected; state/uniform attribution accepted | [state-churn-encode-encode-phase.29](state-churn-encode/state-churn-encode-encode-phase.29.md) (`state=958ms`, `uniform=902ms`, `payload=65ms` despite `232.5MB` copied) |
| H42 | Removing extra `CanonicalDrawState` value hops cuts the state append child | accepted CPU win | [state-churn-encode-encode-phase.30](state-churn-encode/state-churn-encode-encode-phase.30.md) (`append` `2708→2116ms`, `state` `958→720ms`, GPU flat) |
| H43 | Remaining state append cost is PSO/invariant construction | rejected; SoA append attribution accepted | [state-churn-encode-encode-phase.31](state-churn-encode/state-churn-encode-encode-phase.31.md) (`state=879ms`, `SoA=707ms`, `PSO=50ms`, `invariant=22ms`; split timers add overhead) |
| H44 | Slot-local full-state interning amortizes the state SoA push | rejected; frontend generation fast path accepted as next proof | [state-churn-encode-encode-phase.32](state-churn-encode/state-churn-encode-encode-phase.32.md) (`0.150866%` state reuse hit rate; SoA `707ms→962ms`; first add generation/lane opportunity counters) |
| H45 | Adjacent same stable-generation/lane submissions can skip the deep compat compare | accepted proof | [state-churn-encode-encode-phase.33](state-churn-encode/state-churn-encode-encode-phase.33.md) (`52.524409%` same-generation/lane pairs; `385,120/385,120` compatible; `0` incompatible) |
| H46 | Using the generation/lane stamp as a compat fast path removes the compat scan bucket | accepted CPU win | [state-churn-encode-encode-phase.34](state-churn-encode/state-churn-encode-encode-phase.34.md) (`compat_scan` `557.621ms→44.923ms`; `draw_batch_submit` `3491.771ms→3070.200ms`) |
| H47 | Filling queued `DrawRunSubmission` entries in place removes a first-order queue copy | accepted small CPU win; rejected as first-order | [state-churn-encode-encode-phase.35](state-churn-encode/state-churn-encode-encode-phase.35.md) (`queue-snapshot residual` `2286.890ms→2193.875ms`; total queue `-0.60%`) |
| H48 | Reusing a device-owned pending submission vector removes per-chunk allocation churn | rejected-current | [state-churn-encode-encode-phase.36](state-churn-encode/state-churn-encode-encode-phase.36.md) (`queue-snapshot residual` `2193.875ms→2200.657ms`; scratch branch removed) |
| H49 | Reusing draw binding snapshot scratch removes per-group vector allocation churn | accepted small CPU win | [state-churn-encode-encode-phase.37](state-churn-encode/state-churn-encode-encode-phase.37.md) (`batch_binding_snapshot` `0.119755→0.089566ms/present`; total replay flat) |
| H50 | Shader bytecode value ownership makes layout/state copies allocate and copy bytecode per draw | accepted CPU win | [state-churn-encode-encode-phase.38](state-churn-encode/state-churn-encode-encode-phase.38.md) (`state_copy` `0.421921→0.271225ms/present`; queue submit `5.176033→4.792983ms/present`) |
| H51 | Sampler `FlatStateSet` capacity can be reduced from ID-space 64 to public D3DSAMP 16 | accepted CPU win | [state-churn-encode-encode-phase.39](state-churn-encode/state-churn-encode-encode-phase.39.md) (`FlatDrawStateRecord` `18,736→11,056B`; `state_copy` `0.271225→0.181239ms/present`) |
| H52 | Texture-stage `FlatStateSet` active-entry capacity can be split from the 64-key DeviceState id space | accepted targeted CPU win | [state-churn-encode-encode-phase.40](state-churn-encode/state-churn-encode-encode-phase.40.md) (`FlatDrawStateRecord` `11,056→9,008B`; `append_state_soa` `0.263628→0.220626ms/present`; `encode_draw` mixed) |
| H53 | Render-state `FlatStateSet` needs an active-entry count proof before compaction | accepted proof; shrink held | [state-churn-encode-encode-phase.41](state-churn-encode/state-churn-encode-encode-phase.41.md) (`render_state_entries_max=62`, `gt64=0`, but default table already has 62 entries) |
| H54 | Render-state flat payload can use a 128-slot priority active-entry set while keeping the full 256-state digest | accepted bounded CPU width win | [state-churn-encode-encode-phase.42](state-churn-encode/state-churn-encode-encode-phase.42.md) (`FlatDrawStateRecord` `9,008→7,984B`; GT1 render max `62`, overflow `0`; GPU flat) |
...
- **Encode-slot PSO prefetch counter** —
  ...
  depth-state handle resolution after [state-churn-encode-encode-phase.70](state-churn-encode/state-churn-encode-encode-phase.70.md).
...
- **Encode-slot PSO semantic memo counters** —
  ...
  [state-churn-encode-encode-phase.74](state-churn-encode/state-churn-encode-encode-phase.74.md). Use
  ...
  attribution probes added in [state-churn-encode-encode-phase.75](state-churn-encode/state-churn-encode-encode-phase.75.md); they add
  ...
  [state-churn-encode-encode-phase.81](state-churn-encode/state-churn-encode-encode-phase.81.md); set
  ...
  behavior smokes. [state-churn-encode-encode-phase.80](state-churn-encode/state-churn-encode-encode-phase.80.md) repeats the paired A/B
...
- **Discarded-state counters** —
  ...
  [state-churn-encode-encode-phase.48](state-churn-encode/state-churn-encode-encode-phase.48.md), the large N-1 state materialization
...
~99.94% of draws fail to batch into draw-runs ([state-churn-encode-drawrun.01](state-churn-encode/state-churn-encode-drawrun.01.md)):
constant-upload boundaries are the largest break class, and state-delta breaks
are second. The per-encoder breakdown ([state-churn-encode-encoder.01](state-churn-encode/state-churn-encode-encoder.01.md),
[state-churn-encode-stream.01](state-churn-encode/state-churn-encode-stream.01.md)) proved the state-delta churn is **handle
...
([state-churn-encode-stream.03](state-churn-encode/state-churn-encode-stream.03.md)). The state-delta taxonomy
([state-churn-encode-statedelta.01](state-churn-encode/state-churn-encode-statedelta.01.md)→[state-churn-encode-statedelta.03](state-churn-encode/state-churn-encode-statedelta.03.md))
...
([state-churn-encode-churn.01](state-churn-encode/state-churn-encode-churn.01.md), [state-churn-encode-binding.01](state-churn-encode/state-churn-encode-binding.01.md)) then carried
...
post-submission-batch HEAD recheck ([state-churn-encode-batch.01](state-churn-encode/state-churn-encode-batch.01.md)) left the
top-three VS buffer write at ~`1627.3 MiB` (unchanged); disabling auto-expand
([state-churn-encode-expand.01](state-churn-encode/state-churn-encode-expand.01.md), [state-churn-encode-expand.02](state-churn-encode/state-churn-encode-expand.02.md)) removed the
CPU transient amplifier but left the Xcode top-pass buffer/device writes
unchanged at ~`1.63 GiB`. The label-join validation
([state-churn-encode-encoder.03](state-churn-encode/state-churn-encode-encoder.03.md)) is the clean proof: the top three encoders
...
([state-churn-encode-stream.04](state-churn-encode/state-churn-encode-stream.04.md)) keeps the GPU-side question alive but narrows
...
The follow-up tuple-structure pass ([state-churn-encode-stream.05](state-churn-encode/state-churn-encode-stream.05.md)) makes the
...
([state-churn-encode-stream.06](state-churn-encode/state-churn-encode-stream.06.md)) closes two invalid shortcuts:
...
staging-cost preflight ([state-churn-encode-stream.07](state-churn-encode/state-churn-encode-stream.07.md)) says row-stable
...
claim. The implemented row-scoped A/B ([state-churn-encode-stream.08](state-churn-encode/state-churn-encode-stream.08.md)) proves
...
([state-churn-encode-stream.09](state-churn-encode/state-churn-encode-stream.09.md)) rejects the GPU-side owner hypothesis:
...
[present-pacing-bind-cache-work-a.01](present-pacing/present-pacing-bind-cache-work-a.01.md). Follow-up encode-phase timers first
named argbuf setup and binding-packet construction/bookkeeping
([state-churn-encode-encode-phase.01](state-churn-encode/state-churn-encode-encode-phase.01.md)), then split them into child buckets:
dirty cbuf mirroring (`3.92s`), binding-packet cache lookup/store (`1.87s`),
and argbuf table open/repoint (`1.21s`) ([state-churn-encode-encode-phase.02](state-churn-encode/state-churn-encode-encode-phase.02.md)).
The first mutating A/B then skipped `272,956` clean/no-op argbuf cbuf updates
but saved only `39.7ms` from that bucket ([state-churn-encode-encode-phase.03](state-churn-encode/state-churn-encode-encode-phase.03.md)).
The dirty-path split then rejected dirty-bit-only partial repoint:
[state-churn-encode-encode-phase.04](state-churn-encode/state-churn-encode-encode-phase.04.md) saw `0` cached repoints and `760,157`
...
[state-churn-encode-encode-phase.05](state-churn-encode/state-churn-encode-encode-phase.05.md) repoints cached VS/PS/FFPPS entries on
...
[state-churn-encode-encode-phase.06](state-churn-encode/state-churn-encode-encode-phase.06.md) keeps the same counter shape and produces
...
[state-churn-encode-encode-phase.07](state-churn-encode/state-churn-encode-encode-phase.07.md), `cacheDrawBindingPacket()` spends most
...
[state-churn-encode-encode-phase.08](state-churn-encode/state-churn-encode-encode-phase.08.md) hashes/probes the existing
...
normal GT1 frame. After [snapshot-cache-snapshot.10](snapshot-cache/snapshot-cache-snapshot.10.md), the priority has shifted
...
[state-churn-encode-encode-phase.60](state-churn-encode/state-churn-encode-encode-phase.60.md) closes issue cost as indexed Metal draw
call attribution rather than wrapper overhead.

The first follow-up split of that broader encode path is
[state-churn-encode-encode-phase.09](state-churn-encode/state-churn-encode-encode-phase.09.md). It rejects the simple "argbuf open is
...
[state-churn-encode-encode-phase.10](state-churn-encode/state-churn-encode-encode-phase.10.md). `ResourceArena` now skips the O(n)
...
[state-churn-encode-encode-phase.11](state-churn-encode/state-churn-encode-encode-phase.11.md). A temporary dirty-category identity
...
The next retained attribution split is [state-churn-encode-encode-phase.12](state-churn-encode/state-churn-encode-encode-phase.12.md).
...
[state-churn-encode-encode-phase.13](state-churn-encode/state-churn-encode-encode-phase.13.md). It is a same-present attribution run
...
The follow-up implementation [state-churn-encode-encode-phase.14](state-churn-encode/state-churn-encode-encode-phase.14.md) accepts that
...
[state-churn-encode-encode-phase.15](state-churn-encode/state-churn-encode-encode-phase.15.md) closes the remaining sampler-side hash
...
[state-churn-encode-encode-phase.16](state-churn-encode/state-churn-encode-encode-phase.16.md). Matching D3D texture handle + sRGB before
...
With the texture path back at baseline, [state-churn-encode-encode-phase.17](state-churn-encode/state-churn-encode-encode-phase.17.md)
...
by VS residual (`618.150ms`). [state-churn-encode-encode-phase.18](state-churn-encode/state-churn-encode-encode-phase.18.md) performs
...
`actual.png` is not a correctness oracle ([baselines-visual-capture.01](baselines/baselines-visual-capture.01.md)).
[state-churn-encode-encode-phase.19](state-churn-encode/state-churn-encode-encode-phase.19.md) closes that bet as a default CPU win:
...
[state-churn-encode-encode-phase.20](state-churn-encode/state-churn-encode-encode-phase.20.md) closes the build sub-bet, with an
...
[state-churn-encode-encode-phase.21](state-churn-encode/state-churn-encode-encode-phase.21.md) then returns to the broader encode
...
[state-churn-encode-encode-phase.22](state-churn-encode/state-churn-encode-encode-phase.22.md) adds a diagnostic fallback for the
...
[state-churn-encode-encode-phase.23](state-churn-encode/state-churn-encode-encode-phase.23.md) closes that visual bisection with the
...
[state-churn-encode-encode-phase.24](state-churn-encode/state-churn-encode-encode-phase.24.md) corrects the next CPU attribution
...
[state-churn-encode-encode-phase.25](state-churn-encode/state-churn-encode-encode-phase.25.md) names the first child inside that replay
...
[state-churn-encode-encode-phase.26](state-churn-encode/state-churn-encode-encode-phase.26.md) and
[state-churn-encode-encode-phase.29](state-churn-encode/state-churn-encode-encode-phase.29.md) split the queued submit path far enough
...
[state-churn-encode-encode-phase.30](state-churn-encode/state-churn-encode-encode-phase.30.md) closes the first state-append
...
[state-churn-encode-encode-phase.31](state-churn-encode/state-churn-encode-encode-phase.31.md) closes that split as attribution. The
...
[state-churn-encode-encode-phase.32](state-churn-encode/state-churn-encode-encode-phase.32.md) rejects the first stored-state
...
[state-churn-encode-encode-phase.33](state-churn-encode/state-churn-encode-encode-phase.33.md) closes that non-mutating proof. In a
...
[state-churn-encode-encode-phase.34](state-churn-encode/state-churn-encode-encode-phase.34.md) applies the guarded compat fast path.
...
[state-churn-encode-encode-phase.35](state-churn-encode/state-churn-encode-encode-phase.35.md) closes the first F2 microfix. The replay
...
[state-churn-encode-encode-phase.36](state-churn-encode/state-churn-encode-encode-phase.36.md) rejects the remaining F2 persistent
...
[state-churn-encode-encode-phase.37](state-churn-encode/state-churn-encode-encode-phase.37.md) accepts the narrower F6 hygiene fix but
...
[state-churn-encode-encode-phase.38](state-churn-encode/state-churn-encode-encode-phase.38.md) accepts the first F3 ownership fix.
...
[state-churn-encode-encode-phase.39](state-churn-encode/state-churn-encode-encode-phase.39.md) accepts the first F4 width fix. Render
...
[state-churn-encode-encode-phase.40](state-churn-encode/state-churn-encode-encode-phase.40.md) refines that last sentence: the
...
[state-churn-encode-encode-phase.41](state-churn-encode/state-churn-encode-encode-phase.41.md) adds that proof layer without changing
...
[state-churn-encode-encode-phase.42](state-churn-encode/state-churn-encode-encode-phase.42.md) implements the conservative variant of
...
[state-churn-encode-encode-phase.43](state-churn-encode/state-churn-encode-encode-phase.43.md) accepts a smaller resource-retention
...
[state-churn-encode-encode-phase.44](state-churn-encode/state-churn-encode-encode-phase.44.md) implements the F1 N-1 state-copy elision
...
[state-churn-encode-encode-phase.45](state-churn-encode/state-churn-encode-encode-phase.45.md) adds the next split before that larger
...
[state-churn-encode-encode-phase.46](state-churn-encode/state-churn-encode-encode-phase.46.md) implements that cleanup without changing
...
[state-churn-encode-encode-phase.47](state-churn-encode/state-churn-encode-encode-phase.47.md) then adds a non-mutating counter for the
...
[state-churn-encode-encode-phase.48](state-churn-encode/state-churn-encode-encode-phase.48.md) promotes the safe subset of that target:
...
[state-churn-encode-encode-phase.49](state-churn-encode/state-churn-encode-encode-phase.49.md) then rejects the next obvious
...
[state-churn-encode-encode-phase.50](state-churn-encode/state-churn-encode-encode-phase.50.md) accepts the indexed draw default fast
...
[state-churn-encode-encode-phase.51](state-churn-encode/state-churn-encode-encode-phase.51.md) rejects the simple
...
[state-churn-encode-encode-phase.52](state-churn-encode/state-churn-encode-encode-phase.52.md) performs that split as attribution-only
...
[state-churn-encode-encode-phase.53](state-churn-encode/state-churn-encode-encode-phase.53.md) closes the easy copy-construction part of
...
[state-churn-encode-encode-phase.54](state-churn-encode/state-churn-encode-encode-phase.54.md) rejects a simple prefetched-PSO resolve
...
[state-churn-encode-encode-phase.60](state-churn-encode/state-churn-encode-encode-phase.60.md) and reads as per-draw Metal indexed
draw-call cost.

[state-churn-encode-encode-phase.55](state-churn-encode/state-churn-encode-encode-phase.55.md) then fixes the argbuf-open attribution.
...
[state-churn-encode-encode-phase.56](state-churn-encode/state-churn-encode-encode-phase.56.md) rejects that pre-open whole-table skip
...
[state-churn-encode-encode-phase.57](state-churn-encode/state-churn-encode-encode-phase.57.md) performs that residual split. It adds
...
[state-churn-encode-encode-phase.58](state-churn-encode/state-churn-encode-encode-phase.58.md) performs the same attribution-only split
...
texture pre-resolve regression from [state-churn-encode-encode-phase.16](state-churn-encode/state-churn-encode-encode-phase.16.md).

[state-churn-encode-encode-phase.59](state-churn-encode/state-churn-encode-encode-phase.59.md) closes the adjacent-uniform follow-up on
the backend append side. Since [snapshot-cache-snapshot.20](snapshot-cache/snapshot-cache-snapshot.20.md) reports `0`
...
[state-churn-encode-encode-phase.60](state-churn-encode/state-churn-encode-encode-phase.60.md) closes the `encode_draw_issue_cpu_ms`
...
[state-churn-encode-encode-phase.61](state-churn-encode/state-churn-encode-encode-phase.61.md) closes the next argbuf cbuf attribution
...
[state-churn-encode-encode-phase.62](state-churn-encode/state-churn-encode-encode-phase.62.md) refreshes the dirty VS cbuf question
...
[state-churn-encode-encode-phase.63](state-churn-encode/state-churn-encode-encode-phase.63.md) closes the broad-payload-hash variant of
...
[state-churn-encode-encode-phase.64](state-churn-encode/state-churn-encode-encode-phase.64.md) then rejects the narrower "dirty range
...
[state-churn-encode-encode-phase.65](state-churn-encode/state-churn-encode-encode-phase.65.md) lowers that last packed-constant branch:
...
[state-churn-encode-encode-phase.66](state-churn-encode/state-churn-encode-encode-phase.66.md) then probes that indexed-window branch
...
access semantics.
[state-churn-encode-encode-phase.67](state-churn-encode/state-churn-encode-encode-phase.67.md) converts that storage-model question into
...
strategy.
[state-churn-encode-encode-phase.68](state-churn-encode/state-churn-encode-encode-phase.68.md) runs the required low-overhead FPS gate
...
([state-churn-encode-encode-phase.123](state-churn-encode/state-churn-encode-encode-phase.123.md)) keeps the same conclusion after the
...
otherwise.
[state-churn-encode-encode-phase.127](state-churn-encode/state-churn-encode-encode-phase.127.md) removes the queue-observation legacy
...
[state-churn-encode-encode-phase.128](state-churn-encode/state-churn-encode-encode-phase.128.md) then rejects the adjacent lazy-lookup
...
[state-churn-encode-encode-phase.130](state-churn-encode/state-churn-encode-encode-phase.130.md) refreshes the current low-overhead owner
...
[state-churn-encode-encode-phase.131](state-churn-encode/state-churn-encode-encode-phase.131.md) then rechecks the most obvious argbuf
...
[state-churn-encode-encode-phase.134](state-churn-encode/state-churn-encode-encode-phase.134.md) adds the missing attribution surface for
...
[state-churn-encode-encode-phase.135](state-churn-encode/state-churn-encode-encode-phase.135.md) then measures the width of that float
...
cbuf storage.
[state-churn-encode-encode-phase.132](state-churn-encode/state-churn-encode-encode-phase.132.md) and
[state-churn-encode-encode-phase.133](state-churn-encode/state-churn-encode-encode-phase.133.md) close the unsafe table-sharing branch.
...
[state-churn-encode-encode-phase.143](state-churn-encode/state-churn-encode-encode-phase.143.md) closes that ABI precondition without
...
[state-churn-encode-encode-phase.144](state-churn-encode/state-churn-encode-encode-phase.144.md) then adds that bounded runtime scout
...
[state-churn-encode-encode-phase.145](state-churn-encode/state-churn-encode-encode-phase.145.md) keeps this classification: removing
...
still dominates.
[state-churn-encode-encode-phase.156](state-churn-encode/state-churn-encode-encode-phase.156.md) then wires compact uniform submissions
...
[state-churn-encode-encode-phase.157](state-churn-encode/state-churn-encode-encode-phase.157.md) runs that first follow-up. Last
...
[state-churn-encode-encode-phase.158](state-churn-encode/state-churn-encode-encode-phase.158.md) adds diagnostic timers for that compact
...
submission carrier.
[state-churn-encode-encode-phase.162](state-churn-encode/state-churn-encode-encode-phase.162.md) adds explicit carrier-footprint counters
...
[state-churn-encode-encode-phase.164](state-churn-encode/state-churn-encode-encode-phase.164.md) adds the missing distinction between
...
[state-churn-encode-encode-phase.165](state-churn-encode/state-churn-encode-encode-phase.165.md) then implements that carrier-shape
...
[state-churn-encode-encode-phase.166](state-churn-encode/state-churn-encode-encode-phase.166.md) removes the temporary full-submission
...
[state-churn-encode-encode-phase.167](state-churn-encode/state-churn-encode-encode-phase.167.md) checks whether the default-on
...
[state-churn-encode-encode-phase.168](state-churn-encode/state-churn-encode-encode-phase.168.md) runs that opt-out A/B and closes the
...
[state-churn-encode-encode-phase.169](state-churn-encode/state-churn-encode-encode-phase.169.md) then checks whether the sampled black
...
isolation.
[state-churn-encode-encode-phase.170](state-churn-encode/state-churn-encode-encode-phase.170.md) closes that proof gate with h200. The
...
[state-churn-encode-encode-phase.177](state-churn-encode/state-churn-encode-encode-phase.177.md) closes the next pending-flush
...
[state-churn-encode-encode-phase.183](state-churn-encode/state-churn-encode-encode-phase.183.md) implements the corrected H192 mixed
...
design. [state-churn-encode-encode-phase.184](state-churn-encode/state-churn-encode-encode-phase.184.md) records the resulting owner
review and keeps `v0.0.3` as the current visual-safe anchor.

[state-churn-encode-encode-phase.185](state-churn-encode/state-churn-encode-encode-phase.185.md) implements the first direct-compact
...
[state-churn-encode-encode-phase.186](state-churn-encode/state-churn-encode-encode-phase.186.md) implements the next compact-source
...
[state-churn-encode-encode-phase.187](state-churn-encode/state-churn-encode-encode-phase.187.md) runs that gate and rejects runtime
...
[state-churn-encode-encode-phase.188](state-churn-encode/state-churn-encode-encode-phase.188.md) sizes the separate chunk `End` drain.
...
[state-churn-encode-encode-phase.189](state-churn-encode/state-churn-encode-encode-phase.189.md) adds the missing state+uniform
...
[state-churn-encode-encode-phase.190](state-churn-encode/state-churn-encode-encode-phase.190.md) audits the implementation shape before
...
[state-churn-encode-encode-phase.191](state-churn-encode/state-churn-encode-encode-phase.191.md) adds that forced resource-marking submit
...
[state-churn-encode-encode-phase.192](state-churn-encode/state-churn-encode-encode-phase.192.md) implements that owned end-carry object
...
[state-churn-encode-encode-phase.193](state-churn-encode/state-churn-encode-encode-phase.193.md) performs that runtime gate and rejects
...
[state-churn-encode-encode-phase.194](state-churn-encode/state-churn-encode-encode-phase.194.md) adds and runs the missing attribution
...
[state-churn-encode-encode-phase.195](state-churn-encode/state-churn-encode-encode-phase.195.md) summarizes the current "wall" review.
...
[state-churn-encode-encode-phase.196](state-churn-encode/state-churn-encode-encode-phase.196.md) runs that follow-up counter and rejects
...
[state-churn-encode-encode-phase.197](state-churn-encode/state-churn-encode-encode-phase.197.md) then reuses the same H225 run with a
...
[state-churn-encode-encode-phase.198](state-churn-encode/state-churn-encode-encode-phase.198.md) further narrows the uniform half of that
...
[state-churn-encode-encode-phase.199](state-churn-encode/state-churn-encode-encode-phase.199.md) adds the missing component split for the
...
[state-churn-encode-encode-phase.200](state-churn-encode/state-churn-encode-encode-phase.200.md) runs that probe and implements the
...
- [const-upload](const-upload.md) — constant-upload boundaries are the larger, separate
...
- [snapshot-cache](snapshot-cache.md) — the D3D9 draw-state snapshot cache and binding-agnostic
  snapshot reuse address the same per-draw state-binding cost from the front end.
- [hidden-backend-storage](hidden-backend-storage.md) — the GPU-side ~1.63GiB VS-write bucket these CPU
  wins do not touch; the label-join here is shared evidence for that domain.
- [index-cache-locality](index-cache-locality.md) — the one accepted *GPU-side* win, which reduces VS
