...
flowchart TD
  ...
  CPU --> PP["[[present-pacing]]<br/>completion_wait dominated by present completion<br/>current direct path already immediate<br/>no next-CB enqueue during wait<br/>entry→publish p50 6.2ms<br/>encode→commit p50 11.4ms"]
  SCE --> SNAP
  PP --> SCE
...
[[state-churn-encode-encode-phase.57]]).
