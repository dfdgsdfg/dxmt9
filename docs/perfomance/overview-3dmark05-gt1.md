...
[snapshot-cache-visual.02](snapshot-cache/snapshot-cache-visual.02.md) before promoting any run that shows black vertices,
...
[snapshot-cache-visual.03](snapshot-cache/snapshot-cache-visual.03.md). A later current wide-window internal capture
`100..1000:100` also does not reproduce it across red corridor, wide firefight,
`f900`, and `f1000` close-up frames; see [snapshot-cache-visual.04](snapshot-cache/snapshot-cache-visual.04.md).
...
[hidden-backend-storage](hidden-backend-storage.md) for the model and [tvb-mechanism-proof](tvb-mechanism-proof.md) for the
...
locality** ([index-cache-locality](index-cache-locality.md)): reordering indices for opaque
...
[present-pacing-producer-sampling-attribution.196](present-pacing/present-pacing-producer-sampling-attribution.196.md) and
[present-pacing-readonly-managed-buffer-cache.197](present-pacing/present-pacing-readonly-managed-buffer-cache.197.md).
...
[backend-shape-classifiers-alpha.04](backend-shape-classifiers/backend-shape-classifiers-alpha.04.md). The current visual-coupling frame60
...
backend routes. [hidden-backend-storage-shape.27](hidden-backend-storage/hidden-backend-storage-shape.27.md)
...
[hidden-backend-storage-shape.32](hidden-backend-storage/hidden-backend-storage-shape.32.md).
...
alternation in frame60 hot rows ([state-churn-encode-stream.05](state-churn-encode/state-churn-encode-stream.05.md)), the
row-scoped staging A/B proves `60/2` can be made handle-stable without changing
draw/PSO/argbuf shape ([state-churn-encode-stream.08](state-churn-encode/state-churn-encode-stream.08.md)), and the Xcode
follow-up rejects handle identity as the first-order backend owner
([state-churn-encode-stream.09](state-churn-encode/state-churn-encode-stream.09.md)). Stream/IB handle churn remains relevant for
...
([hidden-backend-storage-shape.13](hidden-backend-storage/hidden-backend-storage-shape.13.md)). The current gate also keeps the semantic
...
([hidden-backend-storage-shape.16](hidden-backend-storage/hidden-backend-storage-shape.16.md)), and the joined visibility-positive gate
now emits `visibility-positive-oracle=reject-positive-oracle`
([hidden-backend-storage-shape.17](hidden-backend-storage/hidden-backend-storage-shape.17.md)). Historical opaque-depth and screen-blend
...
([hidden-backend-storage-shape.18](hidden-backend-storage/hidden-backend-storage-shape.18.md)). The automated full gate now records that
as `pso-backend-isolation=reject-current` and adds a `pso-backend-spill =
blocked-current-telemetry` implementation track
([hidden-backend-storage-shape.19](hidden-backend-storage/hidden-backend-storage-shape.19.md)). The same full gate now carries the
...
LRU32 is `0` ([hidden-backend-storage-shape.20](hidden-backend-storage/hidden-backend-storage-shape.20.md)). The backend escape audit is
...
block direct GT1 Xcode spend ([hidden-backend-storage-shape.22](hidden-backend-storage/hidden-backend-storage-shape.22.md)). The reduced
...
([hidden-backend-storage-shape.23](hidden-backend-storage/hidden-backend-storage-shape.23.md)). The Tile-FFP expansion split then shows
...
([hidden-backend-storage-shape.24](hidden-backend-storage/hidden-backend-storage-shape.24.md)). The programmable route split then
...
the full programmable textured route ([hidden-backend-storage-shape.25](hidden-backend-storage/hidden-backend-storage-shape.25.md)).
...
to position-only `0x0` ([hidden-backend-storage-shape.26](hidden-backend-storage/hidden-backend-storage-shape.26.md)). The follow-up
...
([hidden-backend-storage-shape.34](hidden-backend-storage/hidden-backend-storage-shape.34.md)).
...
([hidden-backend-storage-shape.35](hidden-backend-storage/hidden-backend-storage-shape.35.md)).
...
Latest snapshot-cache update: [snapshot-cache-snapshot.23](snapshot-cache/snapshot-cache-snapshot.23.md) rechecks the
...
argbuf table/open path, [snapshot-cache-snapshot.24](snapshot-cache/snapshot-cache-snapshot.24.md) adds the missing
batch-only exclusive reason buckets, and [snapshot-cache-snapshot.25](snapshot-cache/snapshot-cache-snapshot.25.md) rejects
...
redundant shader-constant no-op guard from [snapshot-cache-snapshot.22](snapshot-cache/snapshot-cache-snapshot.22.md) as a
...
([state-churn-encode-encode-phase.32](state-churn-encode/state-churn-encode-encode-phase.32.md) through
[state-churn-encode-encode-phase.37](state-churn-encode/state-churn-encode-encode-phase.37.md)). Shared shader bytecode plus sampler
and TSS/render-state flat-capacity compaction reduce copied state width
([state-churn-encode-encode-phase.38](state-churn-encode/state-churn-encode-encode-phase.38.md),
[state-churn-encode-encode-phase.39](state-churn-encode/state-churn-encode-encode-phase.39.md),
[state-churn-encode-encode-phase.40](state-churn-encode/state-churn-encode-encode-phase.40.md),
[state-churn-encode-encode-phase.42](state-churn-encode/state-churn-encode-encode-phase.42.md)). The resource-retention follow-up then
marks batch-front draw resources once while preserving per-draw binding
override/snapshot resource marking
([state-churn-encode-encode-phase.43](state-churn-encode/state-churn-encode-encode-phase.43.md)). The F1 N-1 state-copy elision was
first proven behind `DXMT9_DRAWRUN_GROUP_BY_GEN_LANE=1`
([state-churn-encode-encode-phase.44](state-churn-encode/state-churn-encode-encode-phase.44.md)), then the default-path attribution
showed `411,362` non-front materialized states / `4.209GB` discarded by batch
append ([state-churn-encode-encode-phase.47](state-churn-encode/state-churn-encode-encode-phase.47.md)). The safe subset is now
...
([state-churn-encode-encode-phase.48](state-churn-encode/state-churn-encode-encode-phase.48.md)). The current follow-up accepts
...
([state-churn-encode-encode-phase.45](state-churn-encode/state-churn-encode-encode-phase.45.md)). Optional queued-submission carrier
...
with a normal output frame ([state-churn-encode-encode-phase.46](state-churn-encode/state-churn-encode-encode-phase.46.md)). The
...
([state-churn-encode-encode-phase.41](state-churn-encode/state-churn-encode-encode-phase.41.md),
[state-churn-encode-encode-phase.42](state-churn-encode/state-churn-encode-encode-phase.42.md)). The latest structural sizes are
...
cleanup, not the next FPS owner ([snapshot-cache-snapshot.19](snapshot-cache/snapshot-cache-snapshot.19.md)). The
...
([state-churn-encode-encode-phase.51](state-churn-encode/state-churn-encode-encode-phase.51.md)). That split now exists as attribution:
...
([state-churn-encode-encode-phase.52](state-churn-encode/state-churn-encode-encode-phase.52.md)). The first narrow copy cleanup is now
...
([state-churn-encode-encode-phase.53](state-churn-encode/state-churn-encode-encode-phase.53.md)). A follow-up prefetched-PSO resolve
...
([state-churn-encode-encode-phase.54](state-churn-encode/state-churn-encode-encode-phase.54.md)). Returning to that larger encode
...
([state-churn-encode-encode-phase.55](state-churn-encode/state-churn-encode-encode-phase.55.md)). The pre-open whole-table reuse check
...
as attribution only ([state-churn-encode-encode-phase.56](state-churn-encode/state-churn-encode-encode-phase.56.md),
[state-churn-encode-encode-phase.57](state-churn-encode/state-churn-encode-encode-phase.57.md)). The binding-packet plan split then
...
because they perturb the parent bucket ([state-churn-encode-encode-phase.58](state-churn-encode/state-churn-encode-encode-phase.58.md)).
...
current FPS owner by itself ([state-churn-encode-encode-phase.59](state-churn-encode/state-churn-encode-encode-phase.59.md)).
...
([state-churn-encode-encode-phase.60](state-churn-encode/state-churn-encode-encode-phase.60.md)).
...
FFPPS repoint ([state-churn-encode-encode-phase.61](state-churn-encode/state-churn-encode-encode-phase.61.md)). The later default
...
([state-churn-encode-encode-phase.86](state-churn-encode/state-churn-encode-encode-phase.86.md)). The dirty VS identity
...
([state-churn-encode-encode-phase.62](state-churn-encode/state-churn-encode-encode-phase.62.md),
[state-churn-encode-encode-phase.123](state-churn-encode/state-churn-encode-encode-phase.123.md)). The completed-seq snapshot cleanup
...
([state-churn-encode-encode-phase.124](state-churn-encode/state-churn-encode-encode-phase.124.md)). The follow-up
...
consumer cleanup rather than an FPS owner ([state-churn-encode-encode-phase.125](state-churn-encode/state-churn-encode-encode-phase.125.md)).
...
actual Metal draw encoding ([state-churn-encode-encode-phase.126](state-churn-encode/state-churn-encode-encode-phase.126.md)).
...
([state-churn-encode-encode-phase.127](state-churn-encode/state-churn-encode-encode-phase.127.md)). The adjacent lazy command-uniform
...
waste ([state-churn-encode-encode-phase.128](state-churn-encode/state-churn-encode-encode-phase.128.md)). The follow-up
...
([state-churn-encode-encode-phase.63](state-churn-encode/state-churn-encode-encode-phase.63.md)). The scoped VS-cbuf plan-shape run
...
([state-churn-encode-encode-phase.64](state-churn-encode/state-churn-encode-encode-phase.64.md),
[state-churn-encode-encode-phase.65](state-churn-encode/state-churn-encode-encode-phase.65.md),
[state-churn-encode-encode-phase.66](state-churn-encode/state-churn-encode-encode-phase.66.md)).
...
cbuf ABI, not small-delta slicing ([state-churn-encode-encode-phase.138](state-churn-encode/state-churn-encode-encode-phase.138.md),
[state-churn-encode-encode-phase.139](state-churn-encode/state-churn-encode-encode-phase.139.md)). The setter-range follow-up then
...
([state-churn-encode-encode-phase.140](state-churn-encode/state-churn-encode-encode-phase.140.md),
[state-churn-encode-encode-phase.141](state-churn-encode/state-churn-encode-encode-phase.141.md)). The follow-up Stage 2b opportunity
...
([state-churn-encode-encode-phase.142](state-churn-encode/state-churn-encode-encode-phase.142.md)). The Stage 2b ABI/runtime follow-up
...
more argbuf table microfixes ([state-churn-encode-encode-phase.143](state-churn-encode/state-churn-encode-encode-phase.143.md),
[state-churn-encode-encode-phase.144](state-churn-encode/state-churn-encode-encode-phase.144.md)). A follow-up legacy uniform scratch
...
completion wait dominant ([state-churn-encode-encode-phase.145](state-churn-encode/state-churn-encode-encode-phase.145.md)).
...
Frame120 (historical bottleneck-shape capture, [baselines-frame120.01](baselines/baselines-frame120.01.md)):
...
([mini-replay-bisection-semantic.02](mini-replay-bisection/mini-replay-bisection-semantic.02.md)) proved one `60/2 depth-read +
...
depth. The rank-1 real-texture replay ([mini-replay-bisection-texture.02](mini-replay-bisection/mini-replay-bisection-texture.02.md))
...
([mini-replay-bisection-texture.04](mini-replay-bisection/mini-replay-bisection-texture.04.md)) keeps final color exact while cutting
LRU32 `19,131 -> 13,194` (`-31.0%`), but canonical primitive ownership still
changes at `809` pixels. Rank 3 ([mini-replay-bisection-texture.05](mini-replay-bisection/mini-replay-bisection-texture.05.md)) repeats
the color-exact owner-masked shape with LRU32 `11,398 -> 8,946` (`-21.5%`) and
`52` owner pixels changed. Rank 4 ([mini-replay-bisection-texture.06](mini-replay-bisection/mini-replay-bisection-texture.06.md)) also
...
scout ([mini-replay-bisection-texture.07](mini-replay-bisection/mini-replay-bisection-texture.07.md)) then checks the cheap threshold
...
([mini-replay-bisection-texture.08](mini-replay-bisection/mini-replay-bisection-texture.08.md)) rejects the current implementation as
that oracle: D3D9 occlusion query resolution is
primitive-count based. The diagnostic Metal visibility scout
([mini-replay-bisection-texture.09](mini-replay-bisection/mini-replay-bisection-texture.09.md)) is now wired into dxmt9 draw encoding and
...
sample-visible. The follow-up cache join ([mini-replay-bisection-texture.10](mini-replay-bisection/mini-replay-bisection-texture.10.md))
...
visibility join ([mini-replay-bisection-texture.11](mini-replay-bisection/mini-replay-bisection-texture.11.md)) closes the positive side:
...
efficiency question, not an average-FPS ownership claim: [hidden-backend-storage-shape.30](hidden-backend-storage/hidden-backend-storage-shape.30.md)
...
- The GPU limiter is hidden vertex/tiler/parameter (TVB) backend storage,
  ...
  denominator remains open. [hidden-backend-storage](hidden-backend-storage.md), [tvb-mechanism-proof](tvb-mechanism-proof.md)
- Opaque-depth index-cache locality is a real, semantic-safe GPU win, but
  ...
  amortized by a broader runtime gate. [index-cache-locality](index-cache-locality.md)
- Several CPU reductions are real (dirty-range reset + FFP-VS slice reuse
  ...
  but every one left GPU frame time flat. [const-upload](const-upload.md), [state-churn-encode](state-churn-encode.md), [snapshot-cache](snapshot-cache.md)

**Rejected as first-order GPU owner**
- Visible `VSOut`/varying width, point-size, half-precision varyings. [vsout-layout](vsout-layout.md)
- Translated-shader temp/scratch sizing; owner is below AIR. [shader-codegen](shader-codegen.md)
- Current primitive-order-preserving backend-shape probes: half-VSOut moves
  ...
  candidate outright. [hidden-backend-storage](hidden-backend-storage.md)
- Render/raster state toggles (depth-write, depth-func, cull, scissor) and
  ...
  [backend-shape-classifiers](backend-shape-classifiers.md), [hidden-backend-storage-shape.10](hidden-backend-storage/hidden-backend-storage-shape.10.md)
- Current PSO/state churn as an Xcode candidate. The hot rows are
  ...
  counters. [hidden-backend-storage-shape.11](hidden-backend-storage/hidden-backend-storage-shape.11.md),
  [hidden-backend-storage-shape.18](hidden-backend-storage/hidden-backend-storage-shape.18.md), [hidden-backend-storage-shape.19](hidden-backend-storage/hidden-backend-storage-shape.19.md)
- Current stream/IB churn as a production claim. The hot rows are true handle
  ...
  buffer writes stay flat. [state-churn-encode-stream.04](state-churn-encode/state-churn-encode-stream.04.md),
...
- Primitive/triangle reorder as a *stable* lever — apparent wins were
  frame-shape/tile-coverage artifacts that did not reproduce on HEAD. [primitive-reorder-diagnostics](primitive-reorder-diagnostics.md)
- Const-upload payload size, R32F/X8 PixelFormatView suppression. [const-upload](const-upload.md), [attachment-pixelformat](attachment-pixelformat.md)

**Open**
- Which sub-component of the hidden backend dominates (stage-out vs binning
  parameter storage vs compiler spill). [hidden-backend-storage](hidden-backend-storage.md)
- Whether a correctness-preserving alpha/backend-state A/B exists after the
  ...
  [hidden-backend-storage-shape.10](hidden-backend-storage/hidden-backend-storage-shape.10.md),
...
- Whether an actual Apple position-only/binning path can avoid hidden
  ...
  path was enabled or impossible. [vsout-layout](vsout-layout.md), [shader-codegen](shader-codegen.md)
...
- Tile-FFP as a current GT1 FPS lever. The two-stage base-colour draw + tile
  ...
  performance path. [hidden-backend-storage-shape.15](hidden-backend-storage/hidden-backend-storage-shape.15.md)
- Whether a deliberately isolated PSO/state-churn A/B changes hidden backend
  ...
  [hidden-backend-storage-shape.11](hidden-backend-storage/hidden-backend-storage-shape.11.md), [hidden-backend-storage-shape.18](hidden-backend-storage/hidden-backend-storage-shape.18.md),
  [hidden-backend-storage-shape.19](hidden-backend-storage/hidden-backend-storage-shape.19.md)
...
- Residual row `50/2` / refreshed `60/2` locality: useful under explicit
  ...
  [index-cache-locality](index-cache-locality.md), [hidden-backend-storage-shape.04](hidden-backend-storage/hidden-backend-storage-shape.04.md),
  [mini-replay-bisection-semantic.02](mini-replay-bisection/mini-replay-bisection-semantic.02.md), [mini-replay-bisection-texture.02](mini-replay-bisection/mini-replay-bisection-texture.02.md),
  [mini-replay-bisection-texture.04](mini-replay-bisection/mini-replay-bisection-texture.04.md), [mini-replay-bisection-texture.05](mini-replay-bisection/mini-replay-bisection-texture.05.md),
  [mini-replay-bisection-texture.08](mini-replay-bisection/mini-replay-bisection-texture.08.md), [mini-replay-bisection-texture.09](mini-replay-bisection/mini-replay-bisection-texture.09.md),
  [mini-replay-bisection-texture.10](mini-replay-bisection/mini-replay-bisection-texture.10.md)
...
- Remaining CPU tracks: pacing/completion wait, backend encode, commit_chunk
  ...
  [present-pacing-boundary-latency-ab.06](present-pacing/present-pacing-boundary-latency-ab.06.md) rejects dxmt9's explicit boundary
  ...
  [present-pacing-subcb-cap.25](present-pacing/present-pacing-subcb-cap.25.md)
  That follow-up [present-pacing-prepublish-stage.07](present-pacing/present-pacing-prepublish-stage.07.md) shows the app/Wine/PE
  ...
  Static follow-up [state-churn-encode-encode-phase.27](state-churn-encode/state-churn-encode-encode-phase.27.md) points at the nested
  ...
  [state-churn-encode-encode-phase.28](state-churn-encode/state-churn-encode-encode-phase.28.md) rejects that branch for current GT1:
  ...
  [snapshot-cache-snapshot.10](snapshot-cache/snapshot-cache-snapshot.10.md) accepts the narrower shader-constant refresh
  ...
  owner.
  [baselines-frame50.05](baselines/baselines-frame50.05.md), [state-churn-encode-encode-phase.02](state-churn-encode/state-churn-encode-encode-phase.02.md),
  [state-churn-encode-encode-phase.03](state-churn-encode/state-churn-encode-encode-phase.03.md), [state-churn-encode-encode-phase.04](state-churn-encode/state-churn-encode-encode-phase.04.md),
  [state-churn-encode-encode-phase.05](state-churn-encode/state-churn-encode-encode-phase.05.md),
  [state-churn-encode-encode-phase.06](state-churn-encode/state-churn-encode-encode-phase.06.md),
  [state-churn-encode-encode-phase.07](state-churn-encode/state-churn-encode-encode-phase.07.md),
  [state-churn-encode-encode-phase.08](state-churn-encode/state-churn-encode-encode-phase.08.md),
  [state-churn-encode-encode-phase.09](state-churn-encode/state-churn-encode-encode-phase.09.md),
  [state-churn-encode-encode-phase.10](state-churn-encode/state-churn-encode-encode-phase.10.md),
  [state-churn-encode-encode-phase.11](state-churn-encode/state-churn-encode-encode-phase.11.md),
  [state-churn-encode-encode-phase.12](state-churn-encode/state-churn-encode-encode-phase.12.md),
  [state-churn-encode-encode-phase.13](state-churn-encode/state-churn-encode-encode-phase.13.md),
  [state-churn-encode-encode-phase.14](state-churn-encode/state-churn-encode-encode-phase.14.md),
  [state-churn-encode-encode-phase.15](state-churn-encode/state-churn-encode-encode-phase.15.md),
  [state-churn-encode-encode-phase.16](state-churn-encode/state-churn-encode-encode-phase.16.md),
  [state-churn-encode-encode-phase.17](state-churn-encode/state-churn-encode-encode-phase.17.md),
  [state-churn-encode-encode-phase.18](state-churn-encode/state-churn-encode-encode-phase.18.md),
  [state-churn-encode-encode-phase.19](state-churn-encode/state-churn-encode-encode-phase.19.md),
  [state-churn-encode-encode-phase.20](state-churn-encode/state-churn-encode-encode-phase.20.md),
  [state-churn-encode-encode-phase.21](state-churn-encode/state-churn-encode-encode-phase.21.md),
  [state-churn-encode-encode-phase.22](state-churn-encode/state-churn-encode-encode-phase.22.md),
  [state-churn-encode-encode-phase.23](state-churn-encode/state-churn-encode-encode-phase.23.md),
  [state-churn-encode-encode-phase.24](state-churn-encode/state-churn-encode-encode-phase.24.md),
  [state-churn-encode-encode-phase.25](state-churn-encode/state-churn-encode-encode-phase.25.md),
  [state-churn-encode-encode-phase.61](state-churn-encode/state-churn-encode-encode-phase.61.md),
  [state-churn-encode-encode-phase.62](state-churn-encode/state-churn-encode-encode-phase.62.md),
  [state-churn-encode-encode-phase.63](state-churn-encode/state-churn-encode-encode-phase.63.md),
  [state-churn-encode-encode-phase.64](state-churn-encode/state-churn-encode-encode-phase.64.md),
  [state-churn-encode-encode-phase.65](state-churn-encode/state-churn-encode-encode-phase.65.md),
  [state-churn-encode-encode-phase.66](state-churn-encode/state-churn-encode-encode-phase.66.md),
  [snapshot-cache-snapshot.04](snapshot-cache/snapshot-cache-snapshot.04.md),
  [snapshot-cache-snapshot.05](snapshot-cache/snapshot-cache-snapshot.05.md),
  [snapshot-cache-snapshot.06](snapshot-cache/snapshot-cache-snapshot.06.md),
  [snapshot-cache-snapshot.07](snapshot-cache/snapshot-cache-snapshot.07.md),
  [snapshot-cache-snapshot.08](snapshot-cache/snapshot-cache-snapshot.08.md),
  [snapshot-cache-snapshot.09](snapshot-cache/snapshot-cache-snapshot.09.md),
...
|--------|------|------------------|
| [baselines](baselines.md) | frame120 / frame50 / frame60 reference captures | shape stable across regimes |
| [hidden-backend-storage](hidden-backend-storage.md) | TVB/parameter storage model, VS-write density, scaling | model ACCEPTED; dominant sub-component OPEN; visible `VSOut` gate rejected in [hidden-backend-storage-shape.08](hidden-backend-storage/hidden-backend-storage-shape.08.md), stale live-vsout smoke closed in [hidden-backend-storage-shape.13](hidden-backend-storage/hidden-backend-storage-shape.13.md), backend escape feasibility triaged in [hidden-backend-storage-shape.14](hidden-backend-storage/hidden-backend-storage-shape.14.md), Tile-FFP hot-row coverage rejected in [hidden-backend-storage-shape.15](hidden-backend-storage/hidden-backend-storage-shape.15.md), stream/IB handle identity rejected in [state-churn-encode-stream.09](state-churn-encode/state-churn-encode-stream.09.md), seq-range System Trace route attribution accepted in [hidden-backend-storage-shape.28](hidden-backend-storage/hidden-backend-storage-shape.28.md), encoder-summary route sidecars enabled in [hidden-backend-storage-shape.29](hidden-backend-storage/hidden-backend-storage-shape.29.md), GPU floor vs wall-clock owner split accepted in [hidden-backend-storage-shape.30](hidden-backend-storage/hidden-backend-storage-shape.30.md), current System Trace refresh accepted in [hidden-backend-storage-shape.31](hidden-backend-storage/hidden-backend-storage-shape.31.md), current shader-dump liveness refresh keeps generic varying trim closed in [hidden-backend-storage-shape.35](hidden-backend-storage/hidden-backend-storage-shape.35.md) |
| [tvb-mechanism-proof](tvb-mechanism-proof.md) | VS-inv ↓ → TVB write ↓, row-local + full-frame | ACCEPTED (load-bearing) |
| [index-cache-locality](index-cache-locality.md) | opaque-depth cache, screen-blend, min-gain, CPU cost | opaque-depth WIN with refreshed frame60 proof; gate-shape scout says hot-row CPU waste is valid candidate build/lookup, not failed-gate churn; screen-blend target movement passes but aggregate top-GPU proof fails by non-target timing drift |
| [index-reuse-measurement](index-reuse-measurement.md) | index reuse, geometry signature/size, state-class | VS-inv tracks cache-miss estimate |
| [primitive-reorder-diagnostics](primitive-reorder-diagnostics.md) | reverse/min-index/split reorder probes | order = frame-shape artifact, not stable owner |
| [mini-replay-bisection](mini-replay-bisection.md) | row-local replay + encoder bisection | reproduced amplification; enabled the proof |
| [vsout-layout](vsout-layout.md) | visible varying width attempts | all REJECTED as owner |
| [shader-codegen](shader-codegen.md) | temp/scratch trim, offline Metal IR | REJECTED; owner below AIR |
| [backend-shape-classifiers](backend-shape-classifiers.md) | alpha/depth/cull/scissor/fog/texture/expand | REJECTED/secondary; indexed path mandatory |
| [attachment-pixelformat](attachment-pixelformat.md) | R32F / X8 PixelFormatView suppression | secondary (texture-write), not VS owner |
| [const-upload](const-upload.md) | cbuf/argbuf class/volatility/dirty-range/sparse | CPU amplifier, GPU unmoved |
| [state-churn-encode](state-churn-encode.md) | stream/IB churn, draw-run, binding override | CPU wins, GPU flat; stream/IB handle-stable A/B accepted as diagnostic in [state-churn-encode-stream.08](state-churn-encode/state-churn-encode-stream.08.md), Xcode rejected handle identity in [state-churn-encode-stream.09](state-churn-encode/state-churn-encode-stream.09.md), argbuf broad-payload-hash reopen path rejected in [state-churn-encode-encode-phase.63](state-churn-encode/state-churn-encode-encode-phase.63.md), dirty VS cbuf width attributed to usage-prefix/indexed fallback in [state-churn-encode-encode-phase.64](state-churn-encode/state-churn-encode-encode-phase.64.md), generic non-indexed packed cbuf layout rejected in [state-churn-encode-encode-phase.65](state-churn-encode/state-churn-encode-encode-phase.65.md), indexed BLENDINDICES window rejected for the hottest VS sample in [state-churn-encode-encode-phase.66](state-churn-encode/state-churn-encode-encode-phase.66.md), Stage2 argbuf hybrid is CPU-negative in [state-churn-encode-encode-phase.67](state-churn-encode/state-churn-encode-encode-phase.67.md), the low-overhead FPS gate rejects disabling Stage2 as the current average-FPS lever in [state-churn-encode-encode-phase.68](state-churn-encode/state-churn-encode-encode-phase.68.md), adjacent full-uniform payload elision is rejected in [state-churn-encode-encode-phase.93](state-churn-encode/state-churn-encode-encode-phase.93.md), component generations clean up only the smaller PS half in [state-churn-encode-encode-phase.95](state-churn-encode/state-churn-encode-encode-phase.95.md), fixed-payload split storage narrows append bytes but not FPS/P2/P3 in [state-churn-encode-encode-phase.116](state-churn-encode/state-churn-encode-encode-phase.116.md), command-front uniform payload copies are removed but do not move FPS/P4 in [state-churn-encode-encode-phase.117](state-churn-encode/state-churn-encode-encode-phase.117.md), VS/PS stage split shrinks the payload record to `96B` but leaves aggregate append bytes dominated by VS constants in [state-churn-encode-encode-phase.118](state-churn-encode/state-churn-encode-encode-phase.118.md), stage constants are then compacted to the usage-live floor in [state-churn-encode-encode-phase.120](state-churn-encode/state-churn-encode-encode-phase.120.md) (`uniform_append_bytes_per_present=490,549.644`, stage amplification `0.971x`) without FPS/P4 promotion, [state-churn-encode-encode-phase.121](state-churn-encode/state-churn-encode-encode-phase.121.md) shows remaining backend legacy uniform scratch materialization is `17.542MB/present` but only `0.616ms/present`, [state-churn-encode-encode-phase.122](state-churn-encode/state-churn-encode-encode-phase.122.md) cuts that scratch path to `12.345MB/present` / `0.449ms/present` by reusing command-front uniform scratch while P4 stays flat, [state-churn-encode-encode-phase.123](state-churn-encode/state-churn-encode-encode-phase.123.md) rejects dirty VS argbuf identity repoint (`862,747` probes, `0` hits), [state-churn-encode-encode-phase.124](state-churn-encode/state-churn-encode-encode-phase.124.md) trims only argbuf table reserve (`0.196 -> 0.158ms/present`) without moving the parent, [state-churn-encode-encode-phase.125](state-churn-encode/state-churn-encode-encode-phase.125.md) removes PSO-prefetch legacy uniform materialization (`12.345 -> 9.011MB/present`, PSO state-copy `0.150 -> 0.016ms/present`) while encode/replay/P4 stay flat, [state-churn-encode-encode-phase.126](state-churn-encode/state-churn-encode-encode-phase.126.md) attributes the remaining materialization to draw-encoder command `36.81%`, queue observation `36.79%`, and draw-encoder param `26.40%`, [state-churn-encode-encode-phase.127](state-churn-encode/state-churn-encode-encode-phase.127.md) eliminates the queue-observation site (`0` bytes/cpu) via compact projected-texture state while P4 remains no-enqueue dominated, [state-churn-encode-encode-phase.128](state-churn-encode/state-churn-encode-encode-phase.128.md) rejects lazy command materialization because command materialized draws stay flat at about `323/present`, [state-churn-encode-encode-phase.129](state-churn-encode/state-churn-encode-encode-phase.129.md) moves VS/FFPVS cbuf content-history scans behind `DXMT9_PERF_ENCODER_BREAKDOWN_CBUF_CONTENT=1` so default encoder breakdown does not self-report diagnostic observer CPU as a renderer owner, [state-churn-encode-encode-phase.130](state-churn-encode/state-churn-encode-encode-phase.130.md) confirms current legacy uniform scratch is only `0.228ms/present`, [state-churn-encode-encode-phase.131](state-churn-encode/state-churn-encode-encode-phase.131.md) refreshes the argbuf payload-delta probe with `changed_nonconst_only=0`, [state-churn-encode-encode-phase.132](state-churn-encode/state-churn-encode-encode-phase.132.md) rejects sharing a mutable argbuf table across changed cbuf pointers as last-write-wins, [state-churn-encode-encode-phase.133](state-churn-encode/state-churn-encode-encode-phase.133.md) classifies direct cbuf Stage 2 as a shader/PSO ABI project rather than a host-only microfix, [state-churn-encode-encode-phase.134](state-churn-encode/state-churn-encode-encode-phase.134.md) proves the remaining argbuf payload-delta churn is float-constant turnover (`changed_vs=843,136 == changed_vs_float`, int/bool `0`) rather than int/bool invalidation, [state-churn-encode-encode-phase.135](state-churn-encode/state-churn-encode-encode-phase.135.md) is superseded because the first width probe retained unsafe previous-payload scratch, [state-churn-encode-encode-phase.136](state-churn-encode/state-churn-encode-encode-phase.136.md) fixes ownership and shows a mixed width distribution (`76.86%` of VS-float-changed rows `<=16`, `21.48%` `>64`, VS upload/changed-byte ratio `1.13x`), [state-churn-encode-encode-phase.137](state-churn-encode/state-churn-encode-encode-phase.137.md) shows the `>64` VS tail owns `91.07%` of changed registers, [state-churn-encode-encode-phase.138](state-churn-encode/state-churn-encode-encode-phase.138.md) shows those wide changes are mostly contiguous full-prefix churn (`span / changed = 1.057x`, full-prefix `78.46%` of regs), [state-churn-encode-encode-phase.139](state-churn-encode/state-churn-encode-encode-phase.139.md) attributes the full-prefix churn to a few shader-pair buckets, and [state-churn-encode-encode-phase.140](state-churn-encode/state-churn-encode-encode-phase.140.md) shows PE dirty-span flush merging, not full-range app setters, creates many wide VS const records. Next local cbuf experiment is sparse VS constant dirty-run flushing; [present-pacing-lowoverhead-serial.24](present-pacing/present-pacing-lowoverhead-serial.24.md) still requires future CPU wins to move P4 wait/overlap before claiming FPS ownership |
| [snapshot-cache](snapshot-cache.md) | D3D9 draw-state snapshot rebuild | historical CPU owner; current P2/P3 residual after direct-cbuf. Stream/IB miss-reason counts are not a new binding-only owner because pure binding invalidation does not bump `drawStableStateGeneration_`; redundant shader constant no-op invalidation is fixed but GT1 still has zero adjacent uniform-generation reuse; the latest direct-cbuf scout leaves lookup `2.859ms/present` and batch miss `2.162ms/present`; batch-only miss reasons show texture in `75.006%` of batch misses, but tuple counters show mixed rows are dominated by `shader+FVF/VDecl`, so texture-only is a scoped candidate, not the whole fix; whole-payload reuse is only a small cleanup (`-4,752` batch uniform builds, lookup `2.850 -> 2.843ms/present`), so the next owner remains batch-miss count/co-churn, hot-state storage, compact/interner work, or P4 overlap; see [snapshot-cache-snapshot.21](snapshot-cache/snapshot-cache-snapshot.21.md), [snapshot-cache-snapshot.22](snapshot-cache/snapshot-cache-snapshot.22.md), [snapshot-cache-snapshot.23](snapshot-cache/snapshot-cache-snapshot.23.md), [snapshot-cache-snapshot.24](snapshot-cache/snapshot-cache-snapshot.24.md), and [snapshot-cache-snapshot.25](snapshot-cache/snapshot-cache-snapshot.25.md) |
| [render-pass-store](render-pass-store.md) | RT/depth re-entry, store DontCare, pass-chain | re-entry real; dominant top rows are immediate role-pair A/B/A target reuse; coalescing OPEN |

Latest state-churn visual update: [state-churn-encode-encode-phase.172](state-churn-encode/state-churn-encode-encode-phase.172.md)
...
Latest state-churn CPU update: [state-churn-encode-encode-phase.193](state-churn-encode/state-churn-encode-encode-phase.193.md)
...
visual gate. [state-churn-encode-encode-phase.194](state-churn-encode/state-churn-encode-encode-phase.194.md) adds the next attribution
counter family for that branch:
`commit_chunk_replay_pending_flush_forced_resource_marking_*`.

Latest present-pacing update:
[present-pacing-current-visual-p4.136](present-pacing/present-pacing-current-visual-p4.136.md) restores a current default
...
[present-pacing-current-pe-cadence-fixed-carry.137](present-pacing/present-pacing-current-pe-cadence-fixed-carry.137.md) refreshes the PE
...
[present-pacing-pe-producer-cadence-source-audit.138](present-pacing/present-pacing-pe-producer-cadence-source-audit.138.md) ties that current run
...
[present-pacing-open-cb-bounded-tail-wait.146](present-pacing/present-pacing-open-cb-bounded-tail-wait.146.md) tests the direct open-CB
...
mechanism.
[present-pacing-encode-session-pass-streaming-runtime.147](present-pacing/present-pacing-encode-session-pass-streaming-runtime.147.md) then repairs the
...
[present-pacing-encode-session-multisource-storeproof.148](present-pacing/present-pacing-encode-session-multisource-storeproof.148.md) closes the
...
shape.
[present-pacing-encode-session-semantic-release-gate.149](present-pacing/present-pacing-encode-session-semantic-release-gate.149.md) then adds the
...
[present-pacing-encode-session-wait-stage-counters.150](present-pacing/present-pacing-encode-session-wait-stage-counters.150.md) shows that the gate
...
[present-pacing-encode-session-wait-stage-durations.151](present-pacing/present-pacing-encode-session-wait-stage-durations.151.md) then rules out slow
...
[present-pacing-encode-session-current-smoke.152](present-pacing/present-pacing-encode-session-current-smoke.152.md) refreshes the current
...
[present-pacing-encode-session-completion-wait-wakeup.153](present-pacing/present-pacing-encode-session-completion-wait-wakeup.153.md) then confirms that
...
[present-pacing-encode-session-ready-preempt-release.155](present-pacing/present-pacing-encode-session-ready-preempt-release.155.md) then closes the
...
[present-pacing-encode-session-prepresent-initboundary.156](present-pacing/present-pacing-encode-session-prepresent-initboundary.156.md) then retries the
...
[present-pacing-encode-session-strict-semantic-start.157](present-pacing/present-pacing-encode-session-strict-semantic-start.157.md) then reruns the
...
[present-pacing-encode-session-ready-source-miss-counter.158](present-pacing/present-pacing-encode-session-ready-source-miss-counter.158.md) then splits out
...
[present-pacing-encode-session-no-wait-writer-split.159](present-pacing/present-pacing-encode-session-no-wait-writer-split.159.md) classifies that
...
[present-pacing-encode-session-writer-active-slot-state.160](present-pacing/present-pacing-encode-session-writer-active-slot-state.160.md) then proves the
...
[present-pacing-encode-session-wait-start-cpuready-publish.168](present-pacing/present-pacing-encode-session-wait-start-cpuready-publish.168.md) then tests
...
[present-pacing-encode-session-ordinary-source-append.169](present-pacing/present-pacing-encode-session-ordinary-source-append.169.md) then validates a
...
[present-pacing-encode-session-semantic-prefix.170](present-pacing/present-pacing-encode-session-semantic-prefix.170.md) then lets the selector
...
[present-pacing-encode-session-selector-counters.171](present-pacing/present-pacing-encode-session-selector-counters.171.md) (present-pacing H171)
...
enqueue-during-wait.
[present-pacing-encode-session-selector-wait-phase.172](present-pacing/present-pacing-encode-session-selector-wait-phase.172.md) (present-pacing H172)
...
window.
[present-pacing-encode-session-producer-active-wait-publish.173](present-pacing/present-pacing-encode-session-producer-active-wait-publish.173.md)
...
that wait.
[present-pacing-encode-session-ordinary-head-start.174](present-pacing/present-pacing-encode-session-ordinary-head-start.174.md) (present-pacing H174)
...
[present-pacing-encode-session-ordinary-prefix-counters.175](present-pacing/present-pacing-encode-session-ordinary-prefix-counters.175.md)
...
[present-pacing-encode-session-source-class-counters.176](present-pacing/present-pacing-encode-session-source-class-counters.176.md) (present-pacing
...
Metal work. [present-pacing-encode-session-active-entry-loss.177](present-pacing/present-pacing-encode-session-active-entry-loss.177.md) then shows
...
[present-pacing-encode-session-draw-continuation-source.178](present-pacing/present-pacing-encode-session-draw-continuation-source.178.md) creates explicit
...
active-entry loss `0`). [present-pacing-encode-session-batch-run-source-boundary.179](present-pacing/present-pacing-encode-session-batch-run-source-boundary.179.md)
...
[present-pacing-encode-session-boundary-disabled.180](present-pacing/present-pacing-encode-session-boundary-disabled.180.md) then shows that
...
rows stay zero. [present-pacing-encode-session-pe-clear-flush-boundary-disabled.181](present-pacing/present-pacing-encode-session-pe-clear-flush-boundary-disabled.181.md)
adds the existing PE clear-flush diagnostic and again leaves P4 closed, so the
blocker is not merely PE bridge entry. [present-pacing-encode-session-semantic-continuation-overlap.182](present-pacing/present-pacing-encode-session-semantic-continuation-overlap.182.md)
...
[present-pacing-encode-session-semantic-attachment-only.183](present-pacing/present-pacing-encode-session-semantic-attachment-only.183.md) is the first
...
[present-pacing-encode-session-semantic-attachment-only-rerun.184](present-pacing/present-pacing-encode-session-semantic-attachment-only-rerun.184.md) then
...
[present-pacing-encode-session-selected-prefix-retain.185](present-pacing/present-pacing-encode-session-selected-prefix-retain.185.md) then tightens the
...
[present-pacing-encode-session-draw-continuation-command-floor.186](present-pacing/present-pacing-encode-session-draw-continuation-command-floor.186.md) then
...
output-frame visual evidence. [present-pacing-encode-session-stable-rerun.187](present-pacing/present-pacing-encode-session-stable-rerun.187.md)
...
under `32966.656ms` of present-boundary wait. [present-pacing-encode-session-deferred-boundary.188](present-pacing/present-pacing-encode-session-deferred-boundary.188.md)
...
[present-pacing-deferred-boundary-isolated.189](present-pacing/present-pacing-deferred-boundary-isolated.189.md) then closes the pending
...
[present-pacing-commit-replay-offload.190](present-pacing/present-pacing-commit-replay-offload.190.md) then lands the first such
...
[present-pacing-offload-backpressure-attribution.191](present-pacing/present-pacing-offload-backpressure-attribution.191.md) then kills that
...
[present-pacing-pe-cost-verification.192](present-pacing/present-pacing-pe-cost-verification.192.md) then verifies the PE cost with an
...
promotion gate.
[present-pacing-pe-const-overhead-cut.193](present-pacing/present-pacing-pe-const-overhead-cut.193.md) then tests H192's fixed-cost
...
[state-churn-encode-encode-phase.195](state-churn-encode/state-churn-encode-encode-phase.195.md) reframes the current "wall" question as
closed local copy/carrier branches versus the still-open P4/no-enqueue and
replay/encode owners. [state-churn-encode-encode-phase.196](state-churn-encode/state-churn-encode-encode-phase.196.md) then runs the
...
reanalysis in [state-churn-encode-encode-phase.197](state-churn-encode/state-churn-encode-encode-phase.197.md) rejects the broad
...
[state-churn-encode-encode-phase.198](state-churn-encode/state-churn-encode-encode-phase.198.md) then narrows the uniform half: payload
...
P4/run-ahead design. [state-churn-encode-encode-phase.199](state-churn-encode/state-churn-encode-encode-phase.199.md) adds the missing
...
[state-churn-encode-encode-phase.200](state-churn-encode/state-churn-encode-encode-phase.200.md) then implements the resulting
...
[state-churn-encode-encode-phase.201](state-churn-encode/state-churn-encode-encode-phase.201.md) then puts a ceiling on the remaining
...
- Links use the wiki-link form, e.g. `[index-cache-locality](index-cache-locality.md)` (basename
