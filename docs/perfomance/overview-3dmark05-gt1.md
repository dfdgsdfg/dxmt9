...
Latest state-churn CPU update: [[state-churn-encode-encode-phase.186]]
implements the direct compact uniform cache source for the existing opt-in
compact submission carrier. The compact lane can now source fixed payload,
component hashes, and current VS/PS constants without first materializing a
full `DrawUniformPayload`; full submission paths still rebuild a valid full
payload before copying. This is only an implementation gate: focused native
tests pass, but the next runtime decision needs a 120s no-gputrace GT1 smoke
that moves snapshot/cache or P4 counters and passes the `v0.0.3` visual-safe
gate before FPS or Xcode/gputrace promotion.

Latest present-pacing update: [[present-pacing-pe-callsite-disassembly.126]]
correlates H125's stable app RVAs against the installed `3DMark05.exe`.
`3DMark05.exe+0x2afeb` is the return site after the wrapper's
`GetViewport` vtable call, not a dxmt9 getter-body owner; the related VS/PS
constant and cube-surface RVAs are the same wrapper-return class. This closes
the H123-H125 local getter branch. The next FPS-facing candidate must either
reduce residual producer/record cadence
(`--require-pe-focused-between-call-gap-residual-decrease` plus P4 gates), move
enough serial replay/encode work that no-enqueue rows fall, or introduce a true
render-pass/encoder carry overlap path that does not increase command buffers,
render passes, tile preservation, final same-key reopens, or load/store rows.
