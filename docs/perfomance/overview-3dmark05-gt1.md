...
([[state-churn-encode-encode-phase.43]]). The render-state entry-count probe
...
`DrawRunSubmission=20,992B`; the 120s scout stayed visually normal with
render-state overflow `0`. These are CPU state-width wins/proofs only: do not
spend Xcode budget on them without a new GPU-facing mechanism.
