...
color both compare with `0` changed bytes, so the reduced `60/0` fragmentless
keep-VSOut route is now an Xcode counter candidate, not a production fix
...
| Recovered capture-layer file route | accepted Xcode counter refresh | `capture-layer-redebug-r1` writes `frame60.gputrace`, `frame60-performance.gputrace`, and `frame60-counters-xcode.csv` after the fragment-function lifetime fix and same-directory temp-file `mv` Wine-binary replacement. The latest `capture-layer-wrapper-live-r1` refresh proves the integrated `--with-wine-capture-layer` wrapper path with GPU `37.492ms`, top-three encoder share `98.40%`, and top-three VS buffer device write `1779.246 MiB`. | Use the recovered file route for Xcode replay counters and keep System Trace as the lower-overhead route-timing path. This result changes measurement availability, not the owner: the next GPU gate still needs invocation/locality reduction or a below-visible backend-route A/B; average FPS remains a separate pacing/CPU proof. [[baselines-gputrace-capture.02]], [[hidden-backend-storage-shape.32]] |
| Current Xcode/dxmt joined attribution | accepted next gate | The existing Xcode counter summarizer joins `frame60-counters-xcode.csv` to dxmt encoder and stream sidecars with top-three coverage. Top rows `60/2`, `60/1`, and `60/0` represent different state classes (depth-read textured alpha, opaque depth-write color, opaque depth-write textured) but share the same hidden-density band; the latest joined report estimates hidden backend write at `1750.007 MiB`, `VS buffer / expected VSOut = 8.6x`, `VS buffer / named tiled-buffer counters = 61.5x`, and dxmt CPU writer bytes are negligible. | Do not spend the next Xcode capture on a one-off alpha/texture/depth toggle, visible `VSOut`, stream/IB handle identity, or partial-render/PB overflow. The next GPU capture must be backed by invocation/locality movement with an oracle, a final-color/final-writer oracle for sample-visible `60/2`, or a real backend-route denominator A/B. Average-FPS work remains P2/P3/P4. [[hidden-backend-storage-shape.33]], [[hidden-backend-storage-shape.32]], [[present-pacing]] |
...
Latest state-churn update: [[state-churn-encode-encode-phase.146]] repeats the
Stage 2b direct-cbuf scout against the current state-elision baseline. The
mechanism is still real: `DXMT9_ARGBUF_DIRECT_CBUF=1` drops
`argbuf_table_bind_calls`, argbuf open/setup, and argbuf cbuf update to `0`,
and cuts encode `11.110 -> 8.500ms/present`. It still does not move average
FPS ownership: `wait -> next enqueue` stays flat (`30.482 -> 30.703ms/present`)
because `commit entry -> publish` grows `13.672 -> 16.260ms/present`. The
current repeat is also visual-open because the screenshot is much darker than
baseline and shows a large white band. Keep direct-cbuf default-off; the next
FPS-facing work is replay/snapshot/publish cadence or true P4 overlap, not
more local argbuf table cleanup.

Latest present-pacing update: [[present-pacing-run-ahead-design.68]] converts
the current P4 evidence into an implementation gate. The accepted low-overhead
baseline still has `completion_wait_without_enqueue=29.336ms/present` and
`wait -> next enqueue=30.482ms/present`, while direct-cbuf cuts encode but only
shifts the exposed wait into `commit entry -> publish`. Draw-count publishing
proves overlap is reachable, yet fails locality by increasing Metal command
buffers, render passes, tile preservation, and GPU CB time. The next FPS-facing
design therefore must decouple CPU readiness from Metal command-buffer
boundaries through run-ahead staging, encode-side multi-slot coalescing, or a
strictly locality-preserving pass-boundary experiment.
