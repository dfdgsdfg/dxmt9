...
|---|---|---|---|
| H55 | The same DEF-overlay copy owns GT2's `~6.95 GiB` VS device-write bucket, and GT2 is GPU-bound on it | accepted; scene fps doubles | [shader-codegen-defselect.02](../shader-codegen/shader-codegen-defselect.02.md) (scene fps `8.84 -> 18.58`, `+110%`, median over `520`/`1,148` per-frame samples of the same fixed ~68 s timeline; the baseline reproduces the historical `~8 fps`. At frame279 `VS Buffer Device Memory Bytes Written` goes `7,371,414,784 -> 0` and GPU time `135.552 -> 9.714 ms` while the candidate carries `+19.24%` VS invocations and `+13.13%` primitives — the frame279 pair is scene-shifted because GT2 is time-based, so the `-92.83%` is not a clean same-workload delta, but the zero write bucket under *more* geometry is.) |
| H54 | GT1's unexplained `VS Buffer Device Memory Bytes Written` bucket is per-invocation stack spill from the DEF-overlay constant register-file copy | accepted; the bucket goes to zero | [shader-codegen-defselect.01](../shader-codegen/shader-codegen-defselect.01.md) (same-frame A/B differing only in translator emission: `VS Buffer Device Memory Bytes Written` `1,593,314,944 -> 0`, frame `GPU Time` `31.414 -> 4.044 ms` (`-87.13%`), with `VS Invocations` and `Primitives` bit-identical at `1,102,915` / `715,432`. `Tiled Vertex Buffer Bytes` moves `-0.20%` and `Partial Render Count` is `0` both sides, ruling out tiling and parameter-buffer overflow. Scene fps `22.09 -> 24.03` (`+8.8%`): the GPU headroom is real but GT1 is not GPU-bound at this point.) |
| H53 | A pure vertex permutation recovers `replay.03`'s `3.86x` hidden VS-write density delta without needing a final-color oracle | rejected on row `60/1`; positive control failed, so the `replay.03` question stays open | [mini-replay-bisection-vertexremap.01](../mini-replay-bisection/mini-replay-bisection-vertexremap.01.md) (four lanes on row `60/1`. The first-reference permutation is an exact null: VS invocations `794,896` and TVB bytes `14,057,472` identical to baseline, VS device writes `-0.005%`, GPU `+0.251%`, LRU32 miss delta `0.0000%`, color hash equal. Hidden write volume tracks VS invocations at `~1591.9 B/inv`, which a permutation cannot change. But the `sort-min-index` control measured `1623.4 B/inv`, not the `442.6` the design's sanity gate requires — `+2.0%` above baseline instead of `3.86x` below — so `sort-min-index` is row-dependent and this row cannot answer the original question. Needs the encoder2 row `60/2` dump. Do not retry the remap on `60/1`.) |
| H52 | A bounded cross-chunk DCE window can remove GT2's dead final R32F pass without creating a new encode-lane bubble | accepted removal mechanism; rejected wait policy and default promotion | [hidden-backend-storage-shape.42](hidden-backend-storage-shape.42.md) (waiting proof removes `389` passes and cuts GPU-CB p50 `2.886 -> 1.064ms`, but instantaneous FPS falls `9.157 -> 6.902` (`-24.6%`). The accepted no-wait bootstrap exposes `498` prefixes but sees only `1/536` successors already ready and omits `30` commands. DCE remains opt-in.) |
