...
**Related.** [[state-churn-encode]] · prev: [[state-churn-encode-encoder.02]] ·
[[hidden-backend-storage]] (the unexplained ~1.63GiB VS-write bucket) ·
[[state-churn-encode-expand.02]] (same-frame no-auto-expand validation) ·
[[state-churn-encode-batch.01]] (later recheck on the same frame).
