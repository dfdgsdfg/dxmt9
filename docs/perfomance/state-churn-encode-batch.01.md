...
**Related.** [[state-churn-encode]] · [[state-churn-encode-binding.01]] (the
override CPU win) · [[state-churn-encode-encoder.03]] (same-frame join) ·
[[hidden-backend-storage]] (the unmoved GPU owner) ·
[[index-cache-locality]] (the accepted GPU-side win path).
