...
**Related.** [[const-upload]] · next: [[const-upload-sparse.02]] (the Xcode
validation that rejects it as a GPU fix) · [[state-churn-encode]] (the draw-run
batching it does not fix) · [[hidden-backend-storage]].
