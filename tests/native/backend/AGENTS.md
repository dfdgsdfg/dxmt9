...
| `pipeline_lifecycle_observer_spec.cpp` | Fixed-capacity source-to-reclaim lifecycle validator and CV fake backend (`R-BACK-2.88`, `R-VERIF-2.23`) |
| `mutation_composition_observer_spec.cpp` | Source-qualified mutation observation, one typed ordering domain across mixed sync/deferred paths, generation/use distance, conservative range candidates, barriers, completion/disposition settlement, dead chains, overflow/reset, and native production-classifier cases (`R-BACK-44.9`–`44.11`) |
