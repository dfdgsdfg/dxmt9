...
BoundaryPolicy resolve(const char* disableEnv,
  ...
  return dxmt9::resolveBoundaryPolicy(disableEnv, presentCompletionEnv,
                                      completionEnv, afterAcquireEnv);
}
...
void testEmptyStringPresentCompletionStaysOn() {
  ...
  check(resolve(nullptr, "", nullptr, nullptr) == BoundaryPolicy::PresentCompletion,
...
void testExplicitZeroPresentCompletionDemotes() {
  ...
  check(resolve(nullptr, kPresentCompletionOff, nullptr, nullptr) == BoundaryPolicy::Default,
        "PRESENT_BOUNDARY_PRESENT_COMPLETION=0 with no other env falls through to Default");
}

void testDisableEnvWinsOverEverything() {
  check(resolve("1", nullptr, nullptr, nullptr) == BoundaryPolicy::Disabled,
        "DXMT9_DISABLE_PRESENT_BOUNDARY=1 alone resolves to Disabled");
  check(resolve("1", "1", "1", "1") == BoundaryPolicy::Disabled,
        "DXMT9_DISABLE_PRESENT_BOUNDARY=1 outranks all other boundary env-vars");
  check(resolve("1", kPresentCompletionOff, "1", "1") == BoundaryPolicy::Disabled,
...
void testPresentCompletionWinsOverCompletion() {
  ...
  check(resolve(nullptr, "1", "1", nullptr) == BoundaryPolicy::PresentCompletion,
        "PresentCompletion=1 outranks Completion=1");
}

void testPresentCompletionWinsOverAfterAcquire() {
  check(resolve(nullptr, "1", nullptr, "1") == BoundaryPolicy::PresentCompletion,
...
void testCompletionSelected() {
  ...
  check(resolve(nullptr, kPresentCompletionOff, "1", nullptr) == BoundaryPolicy::Completion,
        "PRESENT_COMPLETION=0 + COMPLETION=1 selects Completion");
}

void testCompletionWinsOverAfterAcquire() {
  check(resolve(nullptr, kPresentCompletionOff, "1", "1") == BoundaryPolicy::Completion,
...
void testAfterAcquireSelected() {
  ...
  check(resolve(nullptr, kPresentCompletionOff, nullptr, "1") == BoundaryPolicy::AfterAcquire,
        "PRESENT_COMPLETION=0 + AFTER_ACQUIRE=1 selects AfterAcquire");
  check(resolve(nullptr, kPresentCompletionOff, "0", "1") == BoundaryPolicy::AfterAcquire,
        "COMPLETION=0 (explicit) does not block AfterAcquire");
}

void testDefaultFallsThroughWhenAllExplicitlyOff() {
  check(resolve("0", kPresentCompletionOff, "0", "0") == BoundaryPolicy::Default,
...
void testDisableLiteralZeroIsUnset() {
  ...
  check(resolve("0", nullptr, nullptr, nullptr) == BoundaryPolicy::PresentCompletion,
...
void testNonZeroStringsCountAsSet() {
  ...
  check(resolve("yes", kPresentCompletionOff, nullptr, nullptr) == BoundaryPolicy::Disabled,
        "non-zero string activates Disabled");
  check(resolve(nullptr, "true", nullptr, nullptr) == BoundaryPolicy::PresentCompletion,
        "non-zero string keeps PresentCompletion on");
  check(resolve(nullptr, kPresentCompletionOff, "yes", nullptr) == BoundaryPolicy::Completion,
        "non-zero string activates Completion");
  check(resolve(nullptr, kPresentCompletionOff, nullptr, "on") == BoundaryPolicy::AfterAcquire,
