...
// winemetal_dxmt9.so and verify the returned hash matches the codegen-time
// constant baked into THIS TU. Any mismatch indicates a winemetal_dxmt9.dll /
// winemetal_dxmt9.so version skew — the slot ordering or argument layout of the
...
extern "C" int dxmt9_winemetal_check_abi_handshake(void) {
  ...
  if (params.hash != dxmt9::bridge::kBridgeAbiHash) {
    dxmt9::util::logf(
        ...
        "winemetal_dxmt9.dll/winemetal_dxmt9.so ABI hash mismatch: "
