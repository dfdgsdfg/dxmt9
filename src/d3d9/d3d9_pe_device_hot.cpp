/* src/d3d9/d3d9_pe_device_hot.cpp — PE-side hot state, draw, and present owner. */
...
[[nodiscard]] inline HRESULT D3D9DeviceImpl::validateConstRangeFast(
...
HRESULT STDMETHODCALLTYPE D3D9DeviceImpl::SetStreamSource(UINT stream,
...
HRESULT STDMETHODCALLTYPE D3D9DeviceImpl::Present(const RECT* src, const RECT* dst,
                                   HWND wnd, const RGNDATA* dirty) noexcept {
...
HRESULT STDMETHODCALLTYPE D3D9DeviceImpl::DrawIndexedPrimitive(D3DPRIMITIVETYPE type,
                                                INT baseVertex,
...
UINT D3D9DeviceImpl::maxPendingCommandRecords() {
...
size_t D3D9DeviceImpl::maxPendingCommandBytes() {
...
bool D3D9DeviceImpl::shadowedStreamSourceEquals(UINT stream,
...
void D3D9DeviceImpl::notePeDeviceCallAfterPresent(const char* callName,
                                  const void* callerPc) {
...
D3D9PePresentCallSlot D3D9DeviceImpl::pushPeCallScope(const char* callName,
...
void D3D9DeviceImpl::notePeCallScopeReturn(D3D9PePresentCallSlot slot,
...
void D3D9DeviceImpl::popPeCallScope(D3D9PePresentCallSlot slot) noexcept {
...
void D3D9DeviceImpl::setRecordedRef(Table table, Key slot,
...
template<typename Table, typename Key, typename Validated>
void D3D9DeviceImpl::setRecordedStreamRef(
...
HRESULT D3D9DeviceImpl::prepareSoftwareDrawCandidate(Prepare&& prepare) noexcept {
...
template<typename Body>
__attribute__((always_inline))
HRESULT D3D9DeviceImpl::withPeCallAndHotStateSetter(
...
__attribute__((always_inline))
HRESULT D3D9DeviceImpl::drawPrimitiveCore(D3DPRIMITIVETYPE type, UINT startVertex,
...
HRESULT D3D9DeviceImpl::applyConstStateWrite(ConstShadow& live,
...
D3D9DeviceImpl::PeCallScope::PeCallScope(PeDiagnosticsState& diagnostics,
            const char* callName,
            const void* callerPc,
...
D3D9DeviceImpl::PeCallScope::~PeCallScope() noexcept {
    finishEnabled();
}

HRESULT D3D9DeviceImpl::PeCallScope::finish(const char* callName, HRESULT hr) noexcept {
    if (observer_) {
...
PeDecimatedScopeStats* D3D9DeviceImpl::PeCallScope::decimatedStats() const noexcept {
    return decimatedStats_;
}

DxmtPeDecimatedPhaseTimer D3D9DeviceImpl::PeCallScope::phase(
...
void D3D9DeviceImpl::PeCallScope::startEnabled(
    const char* callName, const void* callerPc,
...
__attribute__((noinline))
void D3D9DeviceImpl::PeCallScope::finishEnabled() noexcept {
    if (observer_) {
...
D3D9DeviceImpl::PeHotStateSetterTimer::PeHotStateSetterTimer(D3D9DeviceImpl& device,
...
D3D9DeviceImpl::PeHotStateSetterTimer::~PeHotStateSetterTimer() noexcept {
    finishEnabled();
}

void D3D9DeviceImpl::PeHotStateSetterTimer::markDirty() noexcept {
    dirty_ = true;
}

__attribute__((noinline))
void D3D9DeviceImpl::PeHotStateSetterTimer::startEnabled(
    const char* callName,
    PeDecimatedScopeStats PeDiagnosticsState::* entryStats,
    const void* callerPc) noexcept {
    const std::uint32_t decimationN =
...
__attribute__((noinline))
void D3D9DeviceImpl::PeHotStateSetterTimer::finishEnabled() noexcept {
    if (entryNs_ > 0) {
...
std::chrono::steady_clock::time_point D3D9DeviceImpl::AppendPhaseTimer::begin() const noexcept {
...
void D3D9DeviceImpl::AppendPhaseTimer::recordEncode(
...
void D3D9DeviceImpl::AppendPhaseTimer::recordFlush(
    std::chrono::steady_clock::time_point t0) const noexcept {
    if (!diagnostics) {
        return;
    }
