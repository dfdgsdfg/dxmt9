...
constexpr int kExitFrame = 240;
constexpr char kWindowClass[] = "dxmt9_basic_hlsl_window";
constexpr char kWindowTitle[] = "BasicHLSL";
...
bool create_checker_texture(IDirect3DDevice9* device, IDirect3DTexture9** out_texture) {
    ...
    for (UINT y = 0; y < kTextureSize; ++y) {
        ...
        for (UINT x = 0; x < kTextureSize; ++x) {
            const bool even = (((x / 32) + (y / 32)) & 1) == 0;
            const unsigned char r = even ? 235 : 55;
            const unsigned char g = even ? 190 : 90;
            const unsigned char b = even ? 80 : 200;
...
bool create_device(AppState& app) {
    ...
    app.device->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW);
...
bool create_scene_resources(AppState& app) {
    ...
    if (!asset_path("sample-d3d9-basic-hlsl.hlsl", shader_path, sizeof(shader_path))) {
        std::fprintf(stderr, "FAIL: unable to resolve sample-d3d9-basic-hlsl.hlsl path\n");
        return false;
    }
    ...
    if (!compile_shader("BasicHLSLVS", "vs_2_0", &vs_bytecode, &app.vertexConstants)) {
        return false;
    }
    ...
    if (!compile_shader("BasicHLSLPS", "ps_2_0", &ps_bytecode, nullptr)) {
        return false;
    }
    ...
    if (!create_checker_texture(app.device, &app.texture)) {
        return false;
    }
...
void render_frame(AppState& app) {
    ...
    D3DXVECTOR4 material_ambient(0.28f, 0.24f, 0.22f, 1.0f);
    D3DXVECTOR4 material_diffuse(0.95f, 0.95f, 0.95f, 1.0f);
    D3DXVECTOR4 light_ambient(0.30f, 0.30f, 0.34f, 1.0f);
    D3DXVECTOR4 light_dir0(0.25f, 0.65f, -0.71f, 0.0f);
    D3DXVECTOR4 light_diffuse0(1.0f, 0.95f, 0.88f, 1.0f);
    D3DXMATRIX identity;
    D3DXMatrixIdentity(&identity);

    app.device->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER,
                      D3DCOLOR_XRGB(18, 28, 46), 1.0f, 0);
    ...
    if (app.vertexConstants) {
        app.vertexConstants->SetMatrix(app.device, "g_mWorld", &identity);
        ...
        app.vertexConstants->SetVector(app.device, "g_MaterialAmbientColor", &material_ambient);
        app.vertexConstants->SetVector(app.device, "g_MaterialDiffuseColor", &material_diffuse);
        app.vertexConstants->SetVector(app.device, "g_LightAmbient", &light_ambient);
        app.vertexConstants->SetVector(app.device, "g_LightDir0", &light_dir0);
    ...
    if (app.frame == 0 && !app.quit) {
        trace_log("OK: frame0 drew fullscreen pass");
    }

    if (app.frame == 0 && !app.quit) {
        trace_log("OK: frame0 ending scene");
    }
    app.device->EndScene();
    if (app.frame == 0 && !app.quit) {
...
int main() {
    ...
    trace_log("OK: BasicHLSL startup");
    ...
    trace_log("OK: BasicHLSL device created");
    ...
    trace_log("OK: BasicHLSL finished at frame %d", app.frame);
    cleanup(app);
    if (g_trace) {
        std::fclose(g_trace);
        g_trace = nullptr;
    }
    return app.frame >= 180 ? 0 : 1;
}
