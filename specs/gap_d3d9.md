...
| Remaining deferred/unsupported API surface | N-patch/adaptive tessellation, broad `ProcessVertices`/real SWVP shader execution beyond the current fixed-function WVP path and limited straight-line programmable VS subset, `D3DSAMP_ELEMENTINDEX`, `D3DSAMP_DMAPOFFSET`, per-draw multisample mask, AA line raster toggle, `ComposeRects`, and convolution kernel. |
| Remaining validation work | Broader conformance Wine-run sweeps remain ongoing; the `ProcessVertices` M4x4 subset passed targeted PE singleton validation, while the expanded programmable scaffold and the P8/A8P8 fixed-function + ps_2_0 sampler readback scaffold are build-covered and current app-local replay is blocked before D3D entry by the known Sikarugir `winemetal.dll` ABI attach failure. |
...
| `ProcessVertices` / real software vertex processing | Partial. Device-lost gating remains first; fixed-function `D3DFVF_XYZ` or simple POSITION/COLOR/TEXCOORD source declaration input through WORLD/VIEW/PROJECTION to `D3DFVF_XYZRHW` or simple POSITIONT/COLOR/TEXCOORD destination declaration CPU transform is implemented, including matching diffuse/specular/texture-coordinate passthrough. Fixed-function lighting now covers NORMAL input plus material/global-ambient and enabled directional/point/spot-light diffuse into destination diffuse color plus specular secondary color when `D3DRS_SPECULARENABLE` is set. Source declarations may read supported attributes from multiple bound streams and feed FLOAT3/normalized NORMAL/TANGENT/BINORMAL, FLOAT1-4 BLENDWEIGHT, and UBYTE4 BLENDINDICES to simple programmable shaders; FVF `D3DFVF_NORMAL` is covered as a programmable input source, and programmable TEXCOORD input now decodes normalized/half declaration data (`SHORT2N`, `SHORT4N`, `USHORT2N`, `USHORT4N`, `UBYTE4N`, `FLOAT16_2`, `FLOAT16_4`) into float registers. `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` acceptance are covered. A limited programmable VS path now executes simple DCL/DEFI/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4 bytecode over PE vertex shader float/integer constants for `ProcessVertices`, including `_PARTIALPRECISION` destination-modifier acceptance, D3DSPSM source modifiers, simple IF/IFC/ELSE/ENDIF, REP/LOOP counts, LOOP `aL` initial/step source reads, MOVA address-register source reads, source relative addressing through `a0`/`aL`, temp destination relative addressing, BREAK/BREAKC, and CALL/CALLNZ/LABEL/RET and SETP/BREAKP predicate flow-control, and programmable source `FLOAT4 POSITION` / `D3DFVF_XYZW` reads that preserve clip-space w before viewport projection. The PE conformance scaffold covers vs_3_0 DCL + M4x4/MOV, MAD/MOV, NRM/DP3/MUL/ADD vector math, SLT/SGE/MIN/MAX/LRP compare-select math, RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP scalar-cross/transcendent math and M3x2 matrix math, `_PARTIALPRECISION`, D3DSPSM source modifiers, DEFI/CONSTINT REP/LOOP counts, LOOP `aL` initial/step source reads, MOVA address-register source reads, source relative addressing through `a0`/`aL`, temp destination relative addressing, directional/point/spot fixed-function lighting and specular secondary-color readback, `SHORT2N`/`USHORT2N`/`FLOAT16_2` TEXCOORD declaration decode readbacks, `UBYTE4N`/`DEC3N` NORMAL declaration decode readbacks, BREAK/BREAKC, CALL/CALLNZ/LABEL/RET, SETP/BREAKP predicate, and simple IF/IFC/ELSE/ENDIF flow-control, NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES-input arithmetic, FVF NORMAL input arithmetic, and FLOAT4 POSITION MOV paths for both declaration and FVF XYZW source layouts; targeted Wine singleton validation passed for the earlier M4x4/MOV subset (`visual_process_vertices_xyzhw_policy`, 146 checks), while the current app-local runtime replay is blocked by the known Sikarugir `winemetal.dll` ABI attach failure. Broader destination relative forms, texture/sample opcodes, broader lighting edge cases, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred. `SetSoftwareVertexProcessing` now tracks the mutable PE shadow. |
...
| `ProcessVertices` / SWVP execution | D.4 | Partial. Fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW CPU transform/readback is implemented for FVF and simple POSITION/COLOR/TEXCOORD source declarations, with matching diffuse/specular/texcoord passthrough into FVF or POSITIONT/COLOR/TEXCOORD declaration destinations; fixed-function directional/point/spot-light diffuse output and specular secondary-color output are covered for NORMAL input and material-driven lighting; source declarations may split supported attributes across bound streams and feed FLOAT3/normalized NORMAL/TANGENT/BINORMAL, BLENDWEIGHT, and BLENDINDICES into simple programmable shaders, FVF `D3DFVF_NORMAL` is covered as a programmable input source, and normalized/half TEXCOORD declaration data decodes into programmable float inputs. `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` acceptance are covered. Limited programmable VS execution covers simple DCL/DEFI/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4 bytecode and `_PARTIALPRECISION` destination-modifier acceptance over PE vertex shader float/integer constants, plus programmable `FLOAT4 POSITION` / `D3DFVF_XYZW` source w preservation, LOOP `aL` initial/step source reads, MOVA address-register source reads, source relative addressing through `a0`/`aL`, and temp destination relative addressing. The targeted PE Wine singleton passed before the MAD/FLOAT4-POSITION/NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES-input expansion (`visual_process_vertices_xyzhw_policy`, 146 checks); current replay is blocked before D3D entry by the known Sikarugir app-local `winemetal.dll` ABI attach failure. Broader destination relative forms, texture/sample opcodes, broader lighting edge cases, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred. |
...
| Palette/P8 sampling | D.4/C.2 | Partial. 2D P8/A8P8 texture caps are exposed; 2D/cube/volume locks keep index data (plus texel alpha for A8P8) and expand through the current palette into an A8R8G8B8 backing texture; palette switches during a lock defer backend upload until unlock and then use the latest palette; 2D P8 and A8P8 fixed-function and ps_2_0 `texld` sampling have PE render-target readback scaffolds, including A8P8 texel alpha, level-1 `SetLOD` sampling, and palette updates while the texture is bound; `Format::A8P8` / `Format::P8` are explicit metadata formats but direct core storage is unsupported. |
...
|---|---|---|---|---|---|
| FLOAT1 | 0 | ✅ | dxmt9_ffp_shaders.hpp:43; dxmt9_ffp_shaders.cpp:20, 115; d3d9_pe_device.cpp:276 | ❌ none | size + components decoded; no decl-level dedicated test |
...
| UBYTE4 | 5 | ✅ | dxmt9_ffp_shaders.hpp:48; dxmt9_ffp_shaders.cpp:29; d3d9_pe_device.cpp:281 | shader_transform_spec.cpp:1559 | |
| SHORT2 | 6 | ✅ | dxmt9_ffp_shaders.hpp:49; dxmt9_ffp_shaders.cpp:31; d3d9_pe_device.cpp:282 | ❌ none | |
| SHORT4 | 7 | ✅ | dxmt9_ffp_shaders.hpp:50; dxmt9_ffp_shaders.cpp:38; d3d9_pe_device.cpp:283 | ❌ none | |
| UBYTE4N | 8 | ✅ | dxmt9_ffp_shaders.hpp:51; dxmt9_ffp_shaders.cpp:30; d3d9_pe_device.cpp:284 | ❌ none | |
| SHORT2N | 9 | ✅ | dxmt9_ffp_shaders.hpp:52; dxmt9_ffp_shaders.cpp:32; d3d9_pe_device.cpp:285 | d3d9_conformance_visual_misc.c:779 | ProcessVertices TEXCOORD readback covers PE decode |
| SHORT4N | 10 | ✅ | dxmt9_ffp_shaders.hpp:53; dxmt9_ffp_shaders.cpp:39; d3d9_pe_device.cpp:286 | shader_transform_spec.cpp:1560, 1572 | |
| USHORT2N | 11 | ✅ | dxmt9_ffp_shaders.hpp:54; dxmt9_ffp_shaders.cpp:33; d3d9_pe_device.cpp:287 | d3d9_conformance_visual_misc.c:786 | ProcessVertices TEXCOORD readback covers PE decode |
...
| DEC3N | 14 | ✅ | dxmt9_ffp_shaders.hpp:57; dxmt9_ffp_shaders.cpp:35; d3d9_pe_device.cpp:290 | d3d9_conformance_visual_misc.c:826 | ProcessVertices NORMAL readback covers PE decode |
| FLOAT16_2 | 15 | ✅ | dxmt9_ffp_shaders.hpp:58; dxmt9_ffp_shaders.cpp:36; d3d9_pe_device.cpp:291 | shader_transform_spec.cpp:1561; d3d9_conformance_visual_misc.c:793 | ProcessVertices TEXCOORD readback covers PE decode |
...
- **D3DDECLTYPE**: 18 codes (0..17 incl. UNUSED). 18 fully tested or trivially covered (✅), 0 missing test coverage from the inventory tests. UDEC3 now has a programmable VS declaration load test and DEC3N is covered by ProcessVertices NORMAL readback. 0 missing constants.
...
| MULTISAMPLEMASK | 162 | ⚠️ inline literal `162u` (core_state.cpp:135) | ✅ | ⚠️ | ❌ not consumed | ❌ | |
...
2. **Broad `ProcessVertices` / real SWVP execution** — fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW CPU transform plus matching color/texcoord passthrough is implemented for FVF and simple source/destination declarations, including directional/point/spot fixed-function lighting into destination diffuse and spot specular into destination specular, simple multistream source declarations, and FLOAT3/normalized NORMAL/TANGENT/BINORMAL plus BLENDWEIGHT/BLENDINDICES inputs for simple programmable shaders; normalized/half TEXCOORD declaration inputs decode into programmable float registers, with readback coverage for `SHORT2N`, `USHORT2N`, and `FLOAT16_2`, and `UBYTE4N`/`DEC3N` NORMAL; `SrcStart`/`DestIndex` and `D3DPV_DONOTCOPYDATA` are covered; limited programmable VS execution covers DCL/DEFI/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4, accepts `_PARTIALPRECISION` destination modifiers, and covers D3DSPSM source modifiers and simple IF/IFC/ELSE/ENDIF, REP/LOOP counts with LOOP `aL` initial/step source reads, MOVA address-register source reads, source relative addressing through `a0`/`aL`, temp destination relative addressing, BREAK/BREAKC, and CALL/CALLNZ/LABEL/RET and SETP/BREAKP predicate flow-control while preserving source FLOAT4 POSITION / XYZW w. Broader destination relative forms, texture/sample opcodes, broader lighting edge cases, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred.
3. **`D3DRS_MULTISAMPLEMASK`** — default/shadow only; no per-draw Metal sample-mask programming.
4. **Palette/P8 sampling breadth** — 2D P8/A8P8 caps are exposed and 2D/cube/volume locks expand through the active palette into an A8R8G8B8 sampling backing, including deferred locked-subresource palette switches on unlock; 2D P8/A8P8 fixed-function and ps_2_0 `texld` sampling now have PE render-target readback scaffolds, including A8P8 texel alpha, level-1 `SetLOD` sampling, and palette updates while the texture is bound; direct core P8/A8P8 storage remains explicitly unsupported.
...
- `src/d3d9/d3d9_pe_device.cpp:2149-2155` — `IDirect3DDevice9::GetRasterStatus` impl.
...
| Name | Code | defined | mapped | runtime-use | test | Notes |
|---|---|---|---|---|---|---|
| A8P8 | 40 | PE | PE | partial | native + PE visual scaffold | 2D PE `CheckDeviceFormat(TEXTURE)` reports support; texture/cube/volume creation uses an A8R8G8B8 core backing, exposes 2-byte index+alpha locks, and expands palette RGB plus texel alpha before sampling (`dxmt9-core-device-com-spec`). `test_visual_p8_texture_sampler_policy` covers 2D A8P8 fixed-function and ps_2_0 `texld` sampling into a render target with pixel readback, including texel alpha, level-1 `SetLOD` sampling, and palette updates while the texture is bound. `Format::A8P8` / `fmtFromD3D` exposure is explicit for metadata and pitch; direct core storage and non-texture caps remain unsupported. |
| P8 | 41 | PE | PE | partial | native + PE visual scaffold | 2D PE `CheckDeviceFormat(TEXTURE)` reports support; texture/cube/volume creation uses an A8R8G8B8 core backing, exposes 1-byte index locks, and expands through the current palette before sampling, including palette changes deferred while the subresource is locked (`dxmt9-core-device-com-spec`). `test_visual_p8_texture_sampler_policy` covers 2D P8 fixed-function and ps_2_0 `texld` sampling into a render target with pixel readback, including level-1 `SetLOD` sampling and palette updates while the texture is bound. `Format::P8` / `fmtFromD3D` exposure is explicit for metadata and pitch; direct core storage and non-texture caps remain unsupported. |
...
| Field | defined | mapped | runtime-use | test | Notes |
|---|---|---|---|---|---|
| InVBlank | NO | NO | NO | OK | `GetRasterStatus` `memset(p, 0, sizeof(*p))` -> always `FALSE`. dxmt9 has no scanline source. Conformance accepts either `D3D_OK` or `E_FAIL` (`d3d9_device_misc.cpp:175`). |
...
| GetRasterStatus | ⚠️ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:2149` | `test_device_raster_status_bounds` |
| SetDialogBoxMode | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:2156` | logs only, returns S_OK |
| SetGammaRamp | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:2160` | logs only |
| GetGammaRamp | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:2164` | memset 0 |
...
| UpdateTexture | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:2473` | `test_update_texture_pool_copy_2d` |
| GetRenderTargetData | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:2504` | `test_get_render_target_data_policy` |
| GetFrontBufferData | ❌ | ❌ | ⚠️ | ❌ | `d3d9_pe_device.cpp:2537` | always returns `D3DERR_INVALIDCALL`; `test_visual_multisample_get_front_buffer_data_policy` exercises Ex/swapchain path |
...
| SetClipStatus | ❌ | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:3171` | returns `E_NOTIMPL` (matches Wine ddraw legacy) |
| GetClipStatus | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:3175` | memset 0 + S_OK |
...
| SetPaletteEntries | ⚠️ | ✅ | ✅ | ✅ | `d3d9_pe_device.cpp:3972` | PE shadow plus active-palette upload to bound 2D P8 textures; `test_set_palette_roundtrip`, `test_palette_*_policy`, `test_visual_p8_texture_sampler_policy`, `dxmt9-core-device-com-spec` |
| GetPaletteEntries | ⚠️ | ❌ | ✅ | ❌ | `d3d9_pe_device.cpp:4000` | mirrors shadow |
| SetCurrentTexturePalette | ⚠️ | ✅ | ✅ | ✅ | `d3d9_pe_device.cpp:4009` | PE shadow plus bound 2D P8 re-expansion; `test_palette_current_entry_isolation`, `test_visual_p8_texture_sampler_policy`, `dxmt9-core-device-com-spec` |
...
| ProcessVertices | ⚠️ | ✅ | ✅ | PE singleton | `d3d9_pe_device.cpp:6156` | device-lost gate plus fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW CPU transform with matching diffuse/specular/texcoord passthrough for FVF and simple source/destination declarations, directional/point/spot fixed-function lighting into destination diffuse and spot specular into destination specular, supported source attributes split across bound streams, and FLOAT3/normalized NORMAL/TANGENT/BINORMAL plus BLENDWEIGHT/BLENDINDICES inputs for simple programmable shaders plus FVF NORMAL input and normalized/half TEXCOORD decode; `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` flag acceptance are covered; limited programmable VS path executes DCL/DEFI/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4 over PE vertex shader float/integer constants, accepts `_PARTIALPRECISION` destination modifiers, and covers D3DSPSM source modifiers and simple IF/IFC/ELSE/ENDIF, REP/LOOP counts with LOOP `aL` initial/step source reads, MOVA address-register source reads, source relative addressing through `a0`/`aL`, temp destination relative addressing, BREAK/BREAKC, and CALL/CALLNZ/LABEL/RET and SETP/BREAKP predicate flow-control while preserving source FLOAT4 POSITION / XYZW w; unsupported shader/broader declaration variants return `D3DERR_INVALIDCALL`; `test_visual_process_vertices_xyzhw_policy` covers M4x4/MOV, fixed-function directional/point/spot lighting plus spot specular, MAD/MOV, NRM/DP3/MUL/ADD vector math, SLT/SGE/MIN/MAX/LRP compare-select math, RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP scalar-cross/transcendent math and M3x2 matrix math, `_PARTIALPRECISION`, D3DSPSM source modifiers, DEFI/CONSTINT REP/LOOP counts, LOOP `aL` initial/step source reads, MOVA address-register source reads, source relative addressing through `a0`/`aL`, temp destination relative addressing, `SHORT2N`/`USHORT2N`/`FLOAT16_2` TEXCOORD decode readbacks, `UBYTE4N`/`DEC3N` NORMAL decode readbacks, BREAK/BREAKC, CALL/CALLNZ/LABEL/RET, SETP/BREAKP predicate, and simple IF/IFC/ELSE/ENDIF flow-control, NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES input arithmetic, FVF NORMAL input arithmetic, FLOAT4 POSITION MOV, flag/offset handling, and extra source-attribute paths |
...
| DrawRectPatch | ❌ | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4079` | returns `D3DERR_INVALIDCALL` (patch tessellation unsupported) |
| DrawTriPatch | ❌ | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4080` | returns `D3DERR_INVALIDCALL` |
| DeletePatch | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4081` | returns S_OK |
| CreateQuery | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4084` | `test_query_get_data_size_policy`, `d3d9_queries.cpp` |

### D.5 IDirect3DDevice9Ex (15 additional methods)

| Method | PE-side | unix-side | conformance | native | Source | Notes |
|---|---|---|---|---|---|---|
| SetConvolutionMonoKernel | ❌ | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4098` | `return E_NOTIMPL` |
| ComposeRects | ❌ | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4099` | `return E_NOTIMPL` |
| PresentEx | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4104` | T2 device-lost gate; `test_visual_swapchain_flip_present_policy` indirect |
| GetGPUThreadPriority | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4122` | writes `*p=0` + S_OK |
| SetGPUThreadPriority | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4123` | S_OK |
| WaitForVBlank | ✅ | ✅ | ❌ | ❌ | `d3d9_pe_device.cpp:4125` | `dxmt9c_device_wait_for_vblank` |
| CheckResourceResidency | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device.cpp:4129` | S_OK |
| SetMaximumFrameLatency | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4132` | range [1..30] validated; `test_ex_frame_latency_state` |
| GetMaximumFrameLatency | ✅ | ❌ | ✅ | ❌ | `d3d9_pe_device.cpp:4140` | PE shadow read; `test_ex_frame_latency_state` |
| CheckDeviceState | ✅ | ✅ | ❌ | ❌ | `d3d9_pe_device.cpp:4147` | `dxmt9c_device_check_device_state` |
| CreateRenderTargetEx | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4152` | delegates to CreateRenderTarget; `test_create_rt_ds_failure_policy` |
| CreateOffscreenPlainSurfaceEx | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4164` | T4 shared-handle SYSTEMMEM 1-mip alias; `test_ex_user_memory_*` |
| CreateDepthStencilSurfaceEx | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4175` | `test_create_depth_stencil_surface_ex` |
| ResetEx | ⚠️ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4189` | T2 (2026-05-08): viewport/scissor reset, `deviceNotReset_=false`; `test_ex_create_reset_mode_validation` |
| GetDisplayModeEx | ✅ | ✅ | ✅ | ❌ | `d3d9_pe_device.cpp:4246` | `test_display_mode_ex_size_filter_smoke` |
...
| GenerateMipSubLevels | 🟡 | ❌ | ❌ | ❌ | texture `:869`; cube `:1132`; voltex `:1454` | no-op (autogen mipmap not driven from this entry point) |
...
| AddDirtyBox | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device_child_surface.cpp:1520` | returns S_OK; no upload coupling |
...
| GetFrontBufferData | ❌ | ❌ | ⚠️ | ❌ | `d3d9_pe_device_child_misc.cpp:506` | `return D3DERR_INVALIDCALL`; `test_visual_multisample_get_front_buffer_data_policy` exercises the policy edge |
...
| GetRasterStatus | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device_child_misc.cpp:564` | memset 0 + S_OK |
...
| Method | PE-side | unix-side | conformance | native | Source | Notes |
|---|---|---|---|---|---|---|
| GetLastPresentCount | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device_child_misc.cpp:628` | writes 0, returns S_OK |
| GetPresentStats | 🟡 | ❌ | ❌ | ❌ | `d3d9_pe_device_child_misc.cpp:635` | memset 0, returns S_OK |
...
1. **`ProcessVertices` breadth** — fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW path plus matching color/texcoord passthrough for FVF and simple source/destination declarations is covered by a conformance scaffold, including directional/point/spot fixed-function lighting plus spot specular, simple multistream source-declaration cases, FVF NORMAL programmable input, normalized/half TEXCOORD programmable input decode with `SHORT2N`/`USHORT2N`/`FLOAT16_2` readbacks, `UBYTE4N`/`DEC3N` NORMAL programmable input decode, `SrcStart`/`DestIndex` and `D3DPV_DONOTCOPYDATA` handling, plus limited vs_3_0 M4x4/MOV, MAD/MOV, NRM/DP3/MUL/ADD vector math, SLT/SGE/MIN/MAX/LRP compare-select math, RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP scalar-cross/transcendent math and M3x2 matrix math, `_PARTIALPRECISION`, D3DSPSM source modifiers, DEFI/CONSTINT REP/LOOP counts, LOOP `aL` initial/step source reads, MOVA address-register source reads, source relative addressing through `a0`/`aL`, temp destination relative addressing, BREAK/BREAKC, CALL/CALLNZ/LABEL/RET, SETP/BREAKP predicate, and simple IF/IFC/ELSE/ENDIF flow-control, NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES input arithmetic, and FLOAT4 POSITION MOV programmable cases for declaration and FVF XYZW source layouts. Broader destination relative forms, texture/sample opcodes, broader lighting edge cases, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred.
2. **Palette/P8 breadth** — 2D P8/A8P8 texture caps and 2D/cube/volume sampler backing are wired through palette expansion, including deferred locked-subresource palette switches; 2D P8/A8P8 fixed-function and ps_2_0 `texld` sampling have PE render-target readback scaffolds, including A8P8 texel alpha, level-1 `SetLOD` sampling, and palette updates while the texture is bound; direct core P8/A8P8 storage remains explicitly unsupported.
