...
| `ProcessVertices` / real software vertex processing | Partial. Device-lost gating remains first; fixed-function `D3DFVF_XYZ` or simple POSITION/COLOR/TEXCOORD source declaration input through WORLD/VIEW/PROJECTION to `D3DFVF_XYZRHW` or simple POSITIONT/COLOR/TEXCOORD destination declaration CPU transform is implemented, including matching diffuse/specular/texture-coordinate passthrough. Source declarations may read those supported attributes from multiple bound streams and feed FLOAT3 NORMAL/TANGENT/BINORMAL, FLOAT1-4 BLENDWEIGHT, and UBYTE4 BLENDINDICES to simple programmable shaders; FVF `D3DFVF_NORMAL` is covered as a programmable input source. `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` acceptance are covered. A limited programmable VS path now executes simple DCL/DEFI/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4 bytecode over PE vertex shader float/integer constants for `ProcessVertices`, including `_PARTIALPRECISION` destination-modifier acceptance, D3DSPSM source modifiers, simple IF/IFC/ELSE/ENDIF, count-only REP/LOOP, BREAK/BREAKC, and CALL/CALLNZ/LABEL/RET flow-control, and programmable source `FLOAT4 POSITION` / `D3DFVF_XYZW` reads that preserve clip-space w before viewport projection. The PE conformance scaffold covers vs_3_0 DCL + M4x4/MOV, MAD/MOV, NRM/DP3/MUL/ADD vector math, SLT/SGE/MIN/MAX/LRP compare-select math, RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP scalar-cross/transcendent math and M3x2 matrix math, `_PARTIALPRECISION`, D3DSPSM source modifiers, DEFI/CONSTINT count-only REP/LOOP, BREAK/BREAKC, CALL/CALLNZ/LABEL/RET, and simple IF/IFC/ELSE/ENDIF flow-control, NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES-input arithmetic, FVF NORMAL input arithmetic, and FLOAT4 POSITION MOV paths for both declaration and FVF XYZW source layouts; targeted Wine singleton validation passed for the earlier M4x4/MOV subset (`visual_process_vertices_xyzhw_policy`, 146 checks), while the current app-local runtime replay is blocked by the known Sikarugir `winemetal.dll` ABI attach failure. BREAKP flow control, full LOOP register iteration semantics, texture/sample opcodes, lighting, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred. `SetSoftwareVertexProcessing` now tracks the mutable PE shadow. |
