...
| Remaining validation work | Broader conformance Wine-run sweeps remain ongoing; the `ProcessVertices` M4x4 subset passed targeted PE singleton validation, while the expanded programmable scaffold and the P8/A8P8 fixed-function + ps_2_0 sampler readback scaffold, including cube `samplerCUBE` coverage, are build-covered and current app-local replay is blocked before D3D entry by the known Sikarugir `winemetal.dll` ABI attach failure. |
...
| UDEC3 | 13 | ✅ | dxmt9_ffp_shaders.hpp:56; dxmt9_ffp_shaders.cpp:34; d3d9_pe_device.cpp:289 | shader_transform_spec.cpp:testVs30VertexDeclarationUDec3Load; d3d9_conformance_visual_misc.c | size known; programmable VS declaration load path and ProcessVertices NORMAL readback covered |
...
- **D3DDECLTYPE**: 18 codes (0..17 incl. UNUSED). 18 fully tested or trivially covered (✅), 0 missing test coverage from the inventory tests. UDEC3 now has a programmable VS declaration load test plus ProcessVertices NORMAL readback, and DEC3N is covered by ProcessVertices NORMAL readback. 0 missing constants.
