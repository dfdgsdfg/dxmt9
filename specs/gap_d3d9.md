...
| ProcessVertices | ⚠️ | ✅ | ✅ | PE singleton | `d3d9_pe_device.cpp:5137` | device-lost gate plus fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW CPU transform with matching diffuse/specular/texcoord passthrough for FVF and simple source/destination declarations, including supported source attributes split across bound streams and NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES inputs for simple programmable shaders plus FVF NORMAL input; `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` flag acceptance are covered; limited programmable VS path executes straight-line DCL/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/M3x3/M3x4/M4x3/M4x4 over PE vertex shader constants and preserves source FLOAT4 POSITION / XYZW w; unsupported shader/broader declaration variants return `D3DERR_INVALIDCALL`; `test_visual_process_vertices_xyzhw_policy` covers M4x4/MOV, MAD/MOV, NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES input arithmetic, FVF NORMAL input arithmetic, FLOAT4 POSITION MOV, flag/offset handling, and extra source-attribute paths |
