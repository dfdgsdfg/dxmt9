...
| `ProcessVertices` / real software vertex processing | Partial. Device-lost gating remains first; fixed-function `D3DFVF_XYZ` or simple POSITION/COLOR/TEXCOORD source declaration input through WORLD/VIEW/PROJECTION to `D3DFVF_XYZRHW` or simple POSITIONT/COLOR/TEXCOORD destination declaration CPU transform is implemented, including matching diffuse/specular/texture-coordinate passthrough. Source declarations may read those supported attributes from multiple bound streams and feed FLOAT3 NORMAL/TANGENT/BINORMAL, FLOAT1-4 BLENDWEIGHT, and UBYTE4 BLENDINDICES to simple programmable shaders; FVF `D3DFVF_NORMAL` is covered as a programmable input source. `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` acceptance are covered. A limited programmable VS path now executes simple straight-line DCL/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4 bytecode over PE vertex shader constants for `ProcessVertices`, including `_PARTIALPRECISION` destination-modifier acceptance, D3DSPSM source modifiers, simple IF/IFC/ELSE/ENDIF flow-control, and programmable source `FLOAT4 POSITION` / `D3DFVF_XYZW` reads that preserve clip-space w before viewport projection. The PE conformance scaffold covers vs_3_0 DCL + M4x4/MOV, MAD/MOV, NRM/DP3/MUL/ADD vector math, SLT/SGE/MIN/MAX/LRP compare-select math, RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP scalar-cross/transcendent math and M3x2 matrix math, `_PARTIALPRECISION`, D3DSPSM source modifiers, simple IF/IFC/ELSE/ENDIF flow-control, NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES-input arithmetic, FVF NORMAL input arithmetic, and FLOAT4 POSITION MOV paths for both declaration and FVF XYZW source layouts; targeted Wine singleton validation passed for the earlier M4x4/MOV subset (`visual_process_vertices_xyzhw_policy`, 146 checks), while the current app-local runtime replay is blocked by the known Sikarugir `winemetal.dll` ABI attach failure. Loop/rep/call/break flow control, texture/sample opcodes, lighting, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred. `SetSoftwareVertexProcessing` now tracks the mutable PE shadow. |
...
| `ProcessVertices` / SWVP execution | D.4 | Partial. Fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW CPU transform/readback is implemented for FVF and simple POSITION/COLOR/TEXCOORD source declarations, with matching diffuse/specular/texcoord passthrough into FVF or POSITIONT/COLOR/TEXCOORD declaration destinations; source declarations may split supported attributes across bound streams and feed NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES into simple programmable shaders, and FVF `D3DFVF_NORMAL` is covered as a programmable input source. `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` acceptance are covered. Limited programmable VS execution covers simple straight-line DCL/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4 bytecode and `_PARTIALPRECISION` destination-modifier acceptance over PE vertex shader constants, plus programmable `FLOAT4 POSITION` / `D3DFVF_XYZW` source w preservation. The targeted PE Wine singleton passed before the MAD/FLOAT4-POSITION/NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES-input expansion (`visual_process_vertices_xyzhw_policy`, 146 checks); current replay is blocked before D3D entry by the known Sikarugir app-local `winemetal.dll` ABI attach failure. Loop/rep/call/break flow control, texture/sample opcodes, lighting, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred. |
...
2. **Broad `ProcessVertices` / real SWVP execution** — fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW CPU transform plus matching color/texcoord passthrough is implemented for FVF and simple source/destination declarations, including simple multistream source declarations and NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES inputs for simple programmable shaders; `SrcStart`/`DestIndex` and `D3DPV_DONOTCOPYDATA` are covered; limited straight-line programmable VS execution covers DCL/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4, accepts `_PARTIALPRECISION` destination modifiers, and covers D3DSPSM source modifiers and simple IF/IFC/ELSE/ENDIF flow-control while preserving source FLOAT4 POSITION / XYZW w. Loop/rep/call/break flow control, texture/sample opcodes, lighting, clipping, instanced stream-frequency behavior, and broader declaration/multistream forms remain deferred.
...
| ProcessVertices | ⚠️ | ✅ | ✅ | PE singleton | `d3d9_pe_device.cpp:5483` | device-lost gate plus fixed-function WORLD/VIEW/PROJECTION XYZ→XYZRHW CPU transform with matching diffuse/specular/texcoord passthrough for FVF and simple source/destination declarations, including supported source attributes split across bound streams and NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES inputs for simple programmable shaders plus FVF NORMAL input; `SrcStart`/`DestIndex` placement and `D3DPV_DONOTCOPYDATA` flag acceptance are covered; limited programmable VS path executes straight-line DCL/DEF/MOV/basic arithmetic/comparison/DP/MAD/LRP/NRM/RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP/M3x2/M3x3/M3x4/M4x3/M4x4 over PE vertex shader constants, accepts `_PARTIALPRECISION` destination modifiers, and covers D3DSPSM source modifiers and simple IF/IFC/ELSE/ENDIF flow-control while preserving source FLOAT4 POSITION / XYZW w; unsupported shader/broader declaration variants return `D3DERR_INVALIDCALL`; `test_visual_process_vertices_xyzhw_policy` covers M4x4/MOV, MAD/MOV, NRM/DP3/MUL/ADD vector math, SLT/SGE/MIN/MAX/LRP compare-select math, RCP/RSQ/FRC/ABS/POW/CRS/SGN/SINCOS/EXP/LOG/LIT/DST/EXPP/LOGP scalar-cross/transcendent math and M3x2 matrix math, `_PARTIALPRECISION`, D3DSPSM source modifiers, simple IF/IFC/ELSE/ENDIF flow-control, NORMAL/TANGENT/BINORMAL/BLENDWEIGHT/BLENDINDICES input arithmetic, FVF NORMAL input arithmetic, FLOAT4 POSITION MOV, flag/offset handling, and extra source-attribute paths |
