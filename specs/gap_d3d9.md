...
| Remaining validation work | Broader conformance Wine-run sweeps remain ongoing; the `ProcessVertices` M4x4 subset passed targeted PE singleton validation, while the expanded programmable scaffold is build-covered and current app-local replay is blocked before D3D entry by the known Sikarugir `winemetal.dll` ABI attach failure. |
