...
the latest targeted app-local `visual_mvp_software_vp_policy` replay reaches
D3D with the Sikarugir ABI handshake OK, but currently reports 185
render-target readback check failures in the later clipping, stream-frequency,
pixel-shader, and texture sections.
