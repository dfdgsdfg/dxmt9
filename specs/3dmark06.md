...
| Source relative addressing | implemented | Indexed constant reads lower to clamped `cFloat` / `cInt` / `cBool` array access, and temp source reads lower to clamped `r[]` access. |
| Destination relative addressing | implemented for valid mutable arrays | Indexed constant destinations, temp destinations, and texcoord-output destinations lower to clamped writes. Output/input/predicate/address relative destinations outside those mutable arrays remain rejected with a deterministic destination-addressing diagnostic. |
