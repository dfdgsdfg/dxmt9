---
type: "Spec"
title: "D3D9 Layer Spec"
description: "D3D9 spec, ownership, ordering, and evidence mapping."
tags: [specs, d3d9, spec]
---

# D3D9 Layer Spec

Detailed subtopic specs live in:

- `caps/spec.md` for concrete `D3DCAPS9` reported values.
- `formats/spec.md` for D3D9-to-Metal format mapping tables.
- `queries/spec.md` for deferred query sequencing and backend query records.
- `wsi/spec.md` for HWND-to-CAMetalLayer resolution and presentation lifecycle.
- `shader/spec.md` for shader IR, analysis passes, MSL emission, FFP key,
...
`shader/spec.md` §3.1 alongside the rest of the translator's semantic
...
1 / vp.Height)` live in `shader/spec.md` §3.2 (half-pixel offset, NDC
...
`shader/spec.md` §3.4. Requirement IDs: `R-CORE-SHADER-2.8`, `R-CORE-SHADER-2.9`.
