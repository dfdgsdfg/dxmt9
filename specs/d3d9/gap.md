...
| Windowed WSI acquisition / failure contract (`R-CORE-WSI-1.1`, `R-CORE-WSI-1.2`, `R-CORE-WSI-4.1`) | ✅ implementation / ⚠️ runtime breadth | PE-side ExtEscape acquisition, the fixed-width cold adoption call, exact-qualified legacy fallback, fail-closed unsupported/no-client-view handling, serialized Presenter replacement/readers, generation-qualified binding snapshots, and logical claims for Wine's aliased non-retained `WineMetalView` are implemented; no raw owning Presenter accessor remains. The bounded replacement model and native/source-order pins pass. Legacy x64 evidence on 2026-08-25 is stable without queue tracing: reset HRESULT/resources 5/5, lockable-backbuffer/multisample reset 5/5, desktop no-host fail-closed 6/6, and one full 235-case run completed 234 pass / 1 fail with only the pre-existing `visual_process_vertices_xyzhw_policy` failure; the trace-on reset control also passed. Open evidence is WoW64 repetition, a Wine build carrying ExtEscape, distinct-wrapper identity behavior on other macdrv ABIs, and Wine-compatible desktop/NULL-window success through a real owned headless target rather than a null-Presenter no-op. Protocol/runtime detail is tracked in [winemetal/gap](../winemetal/gap.md). |
...
| PE recorder state/transaction closure (`R-CORE-REC-*`) | ⚠️ | The target [requirements](recorder/requirements.md) and [design](recorder/spec.md) have detailed status in [recorder gap](recorder/gap.md). The host-buildable production transition algebra, exhaustive native truth table, bounded `PeRecorderTransition` model plus expected counterexample, exact normal/oversized/inline/constant append acceptance, and fixed render/TSS/sampler/transform/constant state-block tracked sets are implemented. The bounded commit/StateBlock settlement refinement and model/code binding are also implemented, including generated-table freshness and expected-failure controls. Open: bridge/capture fault injection, remaining identity/model binding, instruction-level disabled-path breadth, and Wine/wild runtime promotion evidence; those runtime gaps remain incomplete. |
...
| Vendor FOURCC / dependent-texture / mipmap upload policy | ⚠️ | scaffolded via 7 PE conformance entries (`tests/conformance/d3d9/d3d9_conformance_vendor_policy.c`). FETCH4 now has an unadvertised raw-token runtime path plus Metal readback coverage, while `CheckDeviceFormat(GET4)` intentionally remains `D3DERR_NOTAVAILABLE`; TEXBEM/TEXDEPTH/test_mipmap_upload remain deferred. `test_miptree_layout_lock_pitch_policy` captures the public-ABI LockRect pitch + GetLevelDesc round-trip (the value-level math is in `tests/native/core/core_d3d9_miptree_layout_spec.cpp`). |
...
(`src/d3d9/d3d9_pe_device_impl.hpp` `applyCurrentPaletteToTexture`), a third corrective writer. The fix removes the race
...
> (`src/d3d9/d3d9_pe_device_com_cold.cpp` `UpdateTexture`) compares `Surface::GetDesc` formats, and
...
packs them into `D9CCommandChunkWireUpdateSurface`, and the replay sink passes
