...
| Windowed WSI acquisition / failure contract (`R-CORE-WSI-1.1`, `R-CORE-WSI-4.1`) | ❌ target path | Current code uses the legacy unix-side macdrv symbol route and can preserve a no-window path. The target requires PE-side `ExtEscape` acquisition, a dedicated cold opaque-token adoption call, and `D3DERR_NOTAVAILABLE` when neither ExtEscape nor an exact qualified legacy path is available. Protocol implementation and lifetime evidence are tracked in [winemetal/gap](../winemetal/gap.md). |
