...
|---|---|---|
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-CORE-REC-7.6`–`7.8`) | mandatory all-family production semantic owner, admission-time canonical D9C V2 role backing, opt-in PE→Unix fixed-region transport, and bounded direct-Arena `CpuReadySemanticTransfer` owner implemented | One prepared admission witness now combines qualified identity dedup, retention deltas, capacity, and exact role frontiers. Accepted records materialize immutable record/handle/payload roles once, so seal does not replay semantic slots. Segmented transport aliases the roles; contiguous ExactFixed remains the default/capture representation and performs the unavoidable tight-layout role gather because the final payload offset depends on aggregate record and handle counts. The negotiated 144-byte segmented descriptor carries the immutable producer interval and remains selected only by `DXMT9_PE_SEGMENTED_TRANSPORT`. CPU-ready pooled storage and broader Wine/GPU/locality evidence remain separate promotion work. |
