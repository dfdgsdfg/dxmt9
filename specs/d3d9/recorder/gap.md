...
|---|---|---|
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-CORE-REC-7.6`–`7.8`) | opt-in PE→Unix fixed-region transport and host immutable-batch/ExactFixed binding implemented; CPU-ready Tape role integration and promotion remain open | Negotiation preserves the 32-byte exchange, defaults to contiguous V2, and selects the 112-byte segmented descriptor only under `DXMT9_PE_SEGMENTED_TRANSPORT`. The PE builder lends immutable record/handle/payload vectors; generated native/WoW64 thunks validate the three fixed roles and 32-bit address bound; Unix copies into typed owned regions, validates and retains all identities before one adoption, and imports without a contiguous gather. ABI/POD/offset pins build on PE32, PE64, and Unix. Native all-family coverage proves segmented canonical concatenation equals contiguous V2; the separate immutable owner performs checked exact planning and ExactFixed emission for the same validated 21-family batch. `SegmentedTransportV1.tla` plus six negative configs covers incomplete reservation, partial adoption, fallback-after-effect, lost checkpoint, partial ledger, and missing wake. | Connect the immutable owner and `SemanticBatchSettlement` to production PendingDelta/capture/resource settlement and CPU-ready Tape Reserve/Adopt/Emit owners. Add bounded Wine allocation/retain/bridge/capture faults, then GPU/readback and supervised wild/locality/performance evidence before changing either default. |
