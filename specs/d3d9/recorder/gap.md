...
|---|---|---|
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-CORE-REC-7.6`–`7.8`) | opt-in PE→Unix fixed-region transport, default-off all-family production semantic owner, and host immutable-batch/ExactFixed binding implemented; CPU-ready Tape role integration and promotion remain open | Negotiation preserves the 32-byte exchange, defaults to contiguous V2, and selects the 112-byte segmented descriptor only under `DXMT9_PE_SEGMENTED_TRANSPORT`. The semantic owner lends its immutable typed record/handle/payload regions; generated native/WoW64 thunks validate the three fixed roles and 32-bit address bound. Capture uses contiguous ExactFixed, while normal production uses owner-backed segmented roles only when negotiated. The persistent production transaction spans all-family PendingDelta generation, CapacityPre/Post, retainer checkpoint, seal/bridge, capture disposition, and reset. Native all-family coverage proves segmented canonical concatenation equals contiguous V2; `SegmentedTransportV1.tla` plus six negative configs covers incomplete reservation, partial adoption, fallback-after-effect, lost checkpoint, partial ledger, and missing wake. | Connect the semantic owner to CPU-ready Tape Reserve/Adopt/Emit owners. Add bounded pooled-storage ownership model, Wine allocation/retain/bridge/capture faults, then GPU/readback and supervised wild/locality/performance evidence before changing either default. |
