...
|---|---|---|
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-CORE-REC-7.6`–`7.8`) | mandatory all-family production semantic owner, admission-time canonical D9C V2 role backing, opt-in PE→Unix fixed-region transport, and bounded direct-Arena `CpuReadySemanticTransfer` owner implemented | One prepared admission witness now combines qualified identity dedup, retention deltas, capacity, and exact role frontiers. Accepted typed semantic arenas are materialized once into immutable canonical record/handle/payload roles at admission; seal does not replay semantic slots. The segmented alias overload exposes those canonical roles directly, while the external-region overload copies the same roles into caller-provided regions. Contiguous ExactFixed is a separate canonical-roles→contiguous-wire gather: it writes the final header/layout and copies the already-materialized roles because aggregate record and handle counts determine tight offsets. The negotiated 144-byte segmented descriptor carries the immutable producer interval and remains selected only by `DXMT9_PE_SEGMENTED_TRANSPORT`. CPU-ready pooled storage and broader Wine/GPU/locality evidence remain separate promotion work. |
...
| Calibrated semantic-owner phase observer (`DXMT9_PE_SEMANTIC_OWNER_PHASE_SPLIT`) | ✅ implemented as a nullable `PeDiagnosticsState` observer, qualified by nonzero `DXMT9_PE_STATS_DECIMATION`; one deterministic parent sample and explicit null clock-pair calibration cover append admission/rollback/outcomes, ExactFixed plan/role-copy, external segmented copy/role-copy, true segmented alias view, and settle/clear. Native coverage pins nested/stale-state restoration, child gating, malformed-vs-header outcomes, and owner-vs-canonical all-family final-wire bytes. A 2026-09-04 GT2 `N=127` direction check completed with zero rejects/errors and attributed about `1.045 ms/P` to prepare, `1.255 ms/P` to sparse/variable copy, `1.098 ms/P` to canonical materialization, and only `0.099 ms/P` to the inclusive ExactFixed parent after null-pair correction. The corrected prepared wire witness now records first wire-visible promotion order in the admission identity pass, uses a proof-qualified bounds-only append, resets only its active frontier, and removes the later handle-order traversal. Native canonical-byte/refcount tests include direct non-wire surface retention followed by duplicate texture and promoted surface wire order. A clean follow-up attributed `1.262`, `1.094`, `0.913`, and `0.091 ms/P` to prepare, sparse/variable copy, canonical materialization, and ExactFixed; the five admission child phases changed from `4.090` to `3.987 ms/P`. | ⚠️ diagnostic evidence only: the initial OFF/ON healthy populations were 1,594/1,587 and the witness comparison populations were 1,587/1,624, so repeat matched runs remain required for a promotion claim. One rebuild-contaminated run is excluded. The evidence demotes ExactFixed-only removal and leaves sparse typed-copy/canonical construction as the larger PE target; no default transport or rendering semantic change is authorized by this observer. |
...
| Replay carrier retirement (`R-BACK-2.84`, `R-BACK-2.99`) | implementation complete / fresh runtime evidence open | `DrawRunSubmission`, its state lane and grouping helpers, snapshot/batch Device and queue APIs, source-payload adapter, replay scratch vector, carrier counters, and carrier-only test seams are absent from production. Direct/Arena and pre-effect ordinary fallback all use synchronous borrowed-state ingress and final `ChunkSlot` storage. The old ledger enum positions remain explicitly reserved so ordinal stability does not masquerade as a live carrier class. Native source audits and ordinary/direct command/state/completion differentials cover the removal boundary; `DirectChunkSlotContinuation.tla` still owns admission, rollback, commit, and no-retry behavior. | Re-run the owner-qualified materialization ledger, cross builds, GPU readback, and matched wild locality/performance gates. Complete broader ordered-control/resource/capture evidence. |
