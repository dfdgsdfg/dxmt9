...
|---|---|---|
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-CORE-REC-7.6`–`7.8`) | opt-in PE→Unix fixed-region transport, default-off all-family production semantic owner, and bounded direct-Arena `CpuReadySemanticTransfer` owner implemented; pooled storage/copy removal and promotion remain open | Negotiation preserves the 32-byte exchange, defaults to contiguous V2, and selects the 112-byte segmented descriptor only under `DXMT9_PE_SEGMENTED_TRANSPORT`. The semantic owner lends its immutable typed record/handle/payload regions; generated native/WoW64 thunks validate the three fixed roles and 32-bit address bound. Capture uses contiguous ExactFixed, while normal production uses owner-backed segmented roles only when negotiated. The direct-Arena path now composes the imported move-only `RawCommandChunk` with `CpuReadyArenaBuildLease` through explicit Reserve/Adopt/Emit/Publish-or-Abort states, retaining the existing bridge ABI and release authority. Recoverable pre-effect fallback is restricted to batch leases; the queue's single-source abort remains terminal fail-stop. Native transfer coverage and `CpuReadySemanticTransfer.tla` bind ownership/order, but do not claim a raw-wire-to-Tape zero-copy conversion. | Add bounded pooled-storage ownership and Wine allocation/retain/bridge/capture faults, then GPU/readback and supervised wild/locality/performance evidence before changing either default. |
| PE final wire blob (`R-CORE-REC-7.1`–`7.2.1`) | `planExactCommandChunkLayout` and exact `CommandChunkBuilder` remain the legacy default; the default-off bounded `PeSemanticBatchOwner` admits all 21 producer rows into typed record slots, copies only constant/UP bytes plus typed sparse and rectangle arenas, and acquires kind-qualified physical pins with source/record checkpoints. Admission rolls back every pin and arena frontier atomically on generation collision or bounded overflow, maintains O(1) record-local handle/payload frontiers for cadence evidence, and reserves the full record walk for final emission. Valid sparse rows preserve nullable handle semantics for unbind without acquiring a pin. Successful settlement advances the warm-retainer epoch, preserves recently used pins, and clears only used owner ranges; Reset/discard releases all pins. Typed `appendOwnedRecord` and const emission visitors perform checked record-local dedup and identical payload alignment for segmented fixed roles and contiguous ExactFixed output; native owner coverage proves region conservation, nullable-binding byte identity, and input-mutation isolation. The older `PePrewireChunkTransaction` and `PeOwnedRecordCandidate` remain call-local Present/Readback test/pilot compatibility paths; the production gate selects one owner/retainer lane per chunk. | CPU-ready Tape ownership and broader Wine/pixel evidence remain required before promotion. Current-toolchain x64/x86 PE and x86_64 provider compilation plus focused native owner/batch tests pass. Sikarugir GT2 now completes through both contiguous and negotiated segmented owner routes with zero chunk rejects and zero GPU command-buffer errors; this is mechanism evidence, not default promotion. Current ledger evidence prices surviving legacy PE builder+seal work at `0.944-1.018ms/Present` in matched GT2 and `0.322ms/Present` in SFIV. The 256-record/256-pin production alias matches the promoted default cadence; larger debug `DXMT9_PE_CHUNK_MAX_RECORDS` overrides exceed the owner bound and fail closed under the owner gate rather than silently falling back. |
...
| Heterogeneous semantic projection (`R-CORE-REC-7.4`–`7.5`) | The existing default-off `DXMT9_PE_SCALAR_SEMANTIC_OBSERVER` gate now also allocates a separate all-family cold ledger without changing `PeRecorderState`, `D3D9DeviceImpl`, child wrappers, or the pointer-free ABI. One production table covers 21 exact record producers: sparse/indexed draw, both UP forms, APPLY_STATE, six constant kinds, Clear, StretchRect, ColorFill, UpdateTexture, UpdateSurface, QueryIssue, Readback, RESZ, GenerateMipmaps, and Present. The common append envelope issues a source ordinal before CapacityPre, preserves pre-effect rejection, and after builder acceptance copies exact category/record-key/payload value and every full wire identity into bounded cold arenas with the exact record ordinal and payload-arena-relative byte range. Effect-unknown bridge failure retains accepted tokens until discard; capture materialized/rejected/skipped settles only after acceptance. The table generates `PeRecorderSemanticProjectionTable.tla`; the owning model plus ten counterexample configs and exhaustive native all-row/field/differential/lifecycle tests execute production predicates. The StateBlock `bridge_pre` fixture remains the only Wine fault seam and its prior x64/x86 results are unchanged, not generalized. | Typed chunk reserve/unique-retain/capture-throw injection, new x64/x86 PE compilation under the current toolchain, bounded Wine all-family faults, and wild observer evidence remain open. An observer-enabled single record above the 16 MiB proof arena or above 64 qualified identities intentionally fails stop; the cold oracle does not prove PE COM behavior, bridge ABI effects, allocator failure, or pixels. |
