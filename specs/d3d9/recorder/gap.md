...
|---|---|---|
| PE final wire blob (`R-CORE-REC-7.1`–`7.2.1`) | The pure, pointer-width-independent `planExactCommandChunkLayout` and an internal exact `CommandChunkBuilder` mode now allocate the canonical D9C V2 header/table/table/arena layout once. Exact mode writes final record and handle tables plus arena directly, keeps legacy wire record/handle/payload vectors empty, stores arena-relative record offsets, and uses monotone used-prefix counts with `handleObjects_`/final-table reads for dedup and rollback. The bounded immutable-batch native fixture proves byte identity with legacy, retain rollback, underfilled-seal retry, repeated seal, Reset reuse, and that only physically absent PE temporary/seal ledger classes disappear. Normal production still uses the legacy builder. | No production owner currently has the replayable producer transaction required by `R-CORE-REC-7.2.1`: `appendRecord` owns one call, future whole-chunk counts are unknown, and its CapacityPre/CapacityPost decisions define cadence. Add an immutable whole-batch owner that can plan without effects and replay the same owned inputs without retaining call-local borrows or changing cadence; then close all-family differential, fallible rollback, x64/x86 Wine, and wild evidence. **Item 6 performance promotion remains open.** |
