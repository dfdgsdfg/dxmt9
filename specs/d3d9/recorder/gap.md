...
The physical residual is now bounded: `d3d9_pe_device_impl.hpp` is 2,997
physical lines. State, draw, scene, and present implementations belong to
`d3d9_pe_device_hot.cpp`; no state implementation fragment remains. The cold
...
| Hot/cold and disabled observer boundary (`R-CORE-REC-5.*`) | ✅ declaration shell and real source owners / ⚠️ Wine observer evidence | Generic StateBlock writer, live-binding, and child call-scope callbacks remain compile-time nothrow capabilities. Six independent plain contexts (StateBlock, Buffer, SurfaceTexture, Query, Presentation, ShaderDeclaration) each hold one device pointer and expose only family-consumed `noexcept` operations; concrete wrappers hold one nullable context pointer. The broad recorder facade and child-visible object-definition/state-shadow invalidation operations are removed; QueryInterface is the deliberate out-of-line key function and FlushPeRecorderForChild is private nonvirtual. The 2,997-line declaration shell has zero state implementation fragments; `d3d9_pe_device_hot.cpp` owns hot state/draw/scene/present definitions. | `d3d9_pe_device_child.hpp` is now a thin compatibility umbrella and ownership includes are narrowed. Broader Wine/wild observer evidence remains open. |
