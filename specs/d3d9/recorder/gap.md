...
|---|---|---|
| PE final wire blob (`R-CORE-REC-7.1`–`7.2.1`) | The pure, pointer-width-independent `planExactCommandChunkLayout` and exact `CommandChunkBuilder` mode allocate the canonical D9C V2 header/table/table/arena layout once and keep the legacy wire vectors empty. `prepareExactFinalLayout` now replans the persistent production builder transactionally while preserving its warm retainer. Present and synchronous Readback are production-selected because each forces an empty chunk, appends one record, and commits before return; allocation failure uses the untouched legacy fallback, accepted commit restores legacy mode after the ordinary reset, and entered bridge failure keeps sealed exact bytes under the existing poison path. One 21-row typed policy names those two exact singletons and the fallback reason for every residual family. Native coverage compares legacy/exact bytes and successful import for all 21 single-record families, injects an under-planned exact payload for atomic rollback/retain release on every row, and covers production singleton restoration, repeated seal, Reset/retry, and discard. Canonical x64/x86 PE and x86_64 provider builds pass. | The other 19 rows are not safely promotable: sparse draw/APPLY_STATE and six constants need deferred PendingDelta settlement; UP/Clear need owned variable inputs; copy/mipmap rows need deferred autogen side effects; UpdateTexture needs capture closure; Query/RESZ need cross-call ordering ownership. Canonical compact D9C V2 direct-final publication remains blocked until a complete immutable semantic batch supports side-effect-free count/dedup followed by one `ExactFixed`-style emission, or a new bounded scatter/gather ABI is introduced. That owner must preserve CapacityPre/CapacityPost cuts, append order, retain multiplicity, capture/resource settlement, and retry/fail-stop across allocation/retain/bridge/capture faults. Bounded x64/x86 Wine fault and wild evidence remain open. **Item 6 performance promotion remains open.** Current ledger evidence (`docs/perfomance/present-pacing/present-pacing-copy-materialization-ledger.237.md`) prices the surviving PE builder+seal rows at `0.944-1.018ms/Present` in matched GT2 and `0.322ms/Present` in SFIV; this supports a bounded direct-final-layout investigation, not promotion.
...
The semantic row's original seven field/settlement counterexamples are now
joined by three latest-issuance controls (A/B/A, wrong producer, and wrong
source), for ten total in this semantic workstream; the broader verification
inventory count remains owned by its current rollup.
