...
|---|---|---|
| PE final wire blob (`R-CORE-REC-7.1`–`7.2`) | `CommandChunkBuilder::seal` now grows and reuses the payload allocation as the final wire blob, preserving retry and Reset semantics without allocating a second final-blob buffer. The copy ledger and 160-handle native fixture pin byte-stable repeated seals and exact record/handle/payload accounting. This is a partial direct builder: final table offsets depend on the completed record and handle counts, so sealing still moves the payload within that allocation and copies the temporary record and handle tables. | Eliminate the three residual seal materializations only after an exact-count batch preplan, fixed-region layout, or schema-approved scatter/gather representation can preserve the contiguous wire ABI. Then close full command-family equivalence, every fallible rollback seam, bounded Wine faults, and wild evidence before promotion. |
...
legacy/direct oracle; the final-layout transaction precedes builder A/B; typed
capabilities precede any asynchronous code review claim; formal/native evidence
precedes PE/Wine evidence, which precedes wild counter and performance evidence.
One design choice remains intentionally unresolved: every producer may provide
exact record/unique-handle/payload counts in a bounded prepass, or the builder
may run a common two-pass visitor. Conservative unused handle capacity cannot
shift the payload offset because exact wire bytes are required, and a
post-build compaction copy would recreate the class this workstream removes.
The current partial builder changes allocation ownership only; it does not
change the wire ABI, recorder cadence, or D3D9 semantics.
