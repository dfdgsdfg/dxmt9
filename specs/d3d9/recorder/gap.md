...
|---|---|---|
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-CORE-REC-7.6`–`7.8`) | mandatory all-family production semantic owner, opt-in PE→Unix fixed-region transport, and bounded direct-Arena `CpuReadySemanticTransfer` owner implemented | The semantic owner is now the sole production final-wire transaction. Contiguous ExactFixed remains the default/capture representation; the negotiated 112-byte segmented descriptor remains selected only by `DXMT9_PE_SEGMENTED_TRANSPORT`. CPU-ready pooled storage and broader Wine/GPU/locality evidence remain separate promotion work. |
| PE final wire blob (`R-CORE-REC-7.1`–`7.2.1`) | `PeSemanticBatchOwner` is the sole production owner for all 21 producer rows, exact typed pins, PendingDelta settlement, buffer hazards, Render Tape pending leases, ExactFixed capture, and negotiated segmented emission. Device construction fails closed when its bounded storage cannot be established; append/seal/transport failure has no legacy production fallback. `CommandChunkBuilder`, `PePrewireChunkTransaction`, and `PeOwnedRecordCandidate` remain differential or Bootstrap test oracles only. | Focused owner/batch/commit/capture native tests and current-toolchain x64/x86 PE builds pass for this promotion. Bounded Wine all-family fault evidence, supervised GT1/GT2/GT3/SFIV correctness/locality runs, and a fresh copy/materialization ledger are still required before claiming the expected `0.944-1.018ms/Present` GT2 builder+seal cost has actually disappeared. Larger debug `DXMT9_PE_CHUNK_MAX_RECORDS` values above the fixed 256-record owner bound fail closed. |
