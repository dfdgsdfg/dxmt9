...
The remaining physical residual is explicit: at this base
`d3d9_pe_device_impl.hpp` is 5,226 lines and has nine production includers.
The four diagnostic/tape helper fragments and tape value fragment have been
folded into real owner headers/TUs. The cold StateBlock Prepare/Commit pair,
implicit-FVF resolver, constant range/read helpers, and all six explicitly
noinline VS/PS constant slow wrappers now belong to
`d3d9_pe_device_com_cold.cpp`; the three state fragments measure 1,010 residual
lines against the 1,650-line baseline. The remaining state setter bodies stay
inline until a true builtin x64/x86 instruction audit proves an out-of-line
owner preserves their codegen; matched strict baseline/candidate audits now
prove zero-delta default-hot VS/PS constant-F fast paths.
...
| Hot/cold and disabled observer boundary (`R-CORE-REC-5.*`) | ⚠️ typed context boundary landed; device-header residual/evidence open | Generic StateBlock writer, live-binding, and child call-scope callbacks remain compile-time nothrow capabilities. Six independent plain contexts (StateBlock, Buffer, SurfaceTexture, Query, Presentation, ShaderDeclaration) each hold one device pointer and expose only family-consumed `noexcept` operations; concrete wrappers hold one nullable context pointer. The broad recorder facade and child-visible object-definition/state-shadow invalidation operations are removed; QueryInterface is the deliberate out-of-line key function and FlushPeRecorderForChild is private nonvirtual. Cold StateBlock/FVF/constant validation and noinline constant slow bodies now live in the COM-cold owner; strict matched x64/x86 builtin probes pass with unchanged bridge schema and measured zero-delta default-hot VS/PS constant-F fast paths. | `d3d9_pe_device_child.hpp` is now a thin compatibility umbrella and ownership includes are narrowed, while the audit reports 1,010 residual state-fragment lines; split further only with safe declaration-order/codegen evidence, and broaden before/after symbol/instruction and Wine/wild observer evidence. |
