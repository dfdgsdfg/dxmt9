...
|---|---|---|
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-CORE-REC-7.6`–`7.8`) | opt-in PE→Unix fixed-region transport, production chunk settlement owner, and host immutable-batch/ExactFixed binding implemented; CPU-ready Tape role integration and promotion remain open | Negotiation preserves the 32-byte exchange, defaults to contiguous V2, and selects the 112-byte segmented descriptor only under `DXMT9_PE_SEGMENTED_TRANSPORT`. The PE builder lends immutable record/handle/payload vectors; generated native/WoW64 thunks validate the three fixed roles and 32-bit address bound. Unix performs three scatter copies into one aligned move-only pooled extent, validates and retains all identities before one adoption, binds exact ledger release to the admitting sink, and imports without a contiguous gather. The persistent production transaction spans all-family PendingDelta generation, CapacityPre/Post, retainer checkpoint, seal/bridge, capture disposition, and reset. Native all-family coverage proves segmented canonical concatenation equals contiguous V2; the separate immutable owner performs checked exact planning and ExactFixed emission for the same validated 21-family batch. `SegmentedTransportV1.tla` plus six negative configs covers incomplete reservation, partial adoption, fallback-after-effect, lost checkpoint, partial ledger, and missing wake. | Connect the immutable semantic owner itself to production final-wire construction and CPU-ready Tape Reserve/Adopt/Emit owners. Add a bounded pooled-storage ownership model, Wine allocation/retain/bridge/capture faults, then GPU/readback and supervised wild/locality/performance evidence before changing either default. |
| PE final wire blob (`R-CORE-REC-7.1`–`7.2.1`) | `planExactCommandChunkLayout` and exact `CommandChunkBuilder` allocate canonical D9C V2 once without temporary wire vectors. Present and synchronous Readback are production-selected exact singletons; all 21 families have legacy/exact byte/import/rollback coverage and current PE32/PE64/Unix builds. The new segmented ABI removes the final contiguous PE gather for opt-in ordinary chunks while preserving the same canonical geometry. | The other 19 exact-singleton rows still cannot move independently because PendingDelta, variable input ownership, autogen/capture effects, and ordered control settle at batch scope. The host immutable-batch owner proves value/byte algebra but is not connected to those production settlements. Add CapacityPre/Post, capture/resource/retain/bridge fault binding and bounded Wine/wild evidence before broader direct-final or segmented promotion. Current ledger evidence prices surviving PE builder+seal work at `0.944-1.018ms/Present` in matched GT2 and `0.322ms/Present` in SFIV; this supports investigation, not promotion. |
