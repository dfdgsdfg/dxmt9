...
| CheckDeviceFormat | ⚠️ | ✅ | ✅ | ⚠️ | `d3d9_pe_factory.cpp` | T3 rejects VB/IB rtype with INVALIDCALL. R-FORMAT-16 promotes supported AUTOGENMIPMAP formats to `D3D_OK`; public level count remains one and the backend owns the generated hidden chain. The 2026-08-27 builtin AUTOGEN conformance lane passes 4/4 tests (55 checks), and the native RGBA16Float Metal reduction oracle passes. The UI-confirmed HDR1 Canyon Flight `hdr1-gui-selected-ordered-autogen-r120` wild artifact completes without timeout, GPU command-buffer error, chunk reject, or missing-pipeline draw; sampled water/reflection frames no longer show the former crop/checker pattern or black interval. Ordered generation removes the prior PE drain/standalone-CB wait: `sync_waits` falls from 1,350 (`1,840.242 ms`) in `hdr1-autogen-fix-r115` to zero, while command buffers fall from 9,284 to 7,845 despite 2.7% more submitted draws. The Advanced Edition ignores the nominal `-hdr1` selector, so r120 was qualified by clearing the GUI selection and verifying `SM2=0/2`, `CPU=0/2`, `HDR=1/2` before launch; lane metadata alone is insufficient evidence. The broader factory row remains partial for the independent adapter/display/resource-type validation gaps. |
...
| GenerateMipSubLevels | 🟡 | ✅ | ✅ | ✅ | texture `:917`; cube `:1204`; voltex `:1538` | non-AUTOGEN no-op; explicit AUTOGEN calls retain the synchronous public path, while sampling-triggered AUTOGEN uses the texture-qualified ordered chunk/backend command pinned by native wire, replay, source-payload, framegraph, WMT Metal, and `AutogenMipGeneration.tla` evidence |
