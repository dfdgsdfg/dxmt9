...
|---|---|---|
| TLA+ specs: CommandQueue, QueueLifecycleRefinement, PresentFrameLatency, ResourceLifetime, BufferBackingVersioning, EncoderLifecycle, QuerySeqId, ConcurrentProgressSignals, DrawableToken, WireHandleGeneration, WireObjectRegistryV2, PresentIdAba, EncodeSessionCompletion, DceChunkLookahead, CpuReadySessionProgress | ✅ | ConcurrentProgressSignals added 2026-05-09 (T2 closes G1: pacing-axis independence under R-ARCH-6.8/6.9). DrawableToken / WireHandleGeneration / PresentIdAba added 2026-05-18 (close drawable-token handoff race, PE→unix generation-stamp zombie, and (slot, generation) ABA-safety respectively). EncodeSessionCompletion covers one Metal session tail expanding into ordered per-source `seqId` completion. DceChunkLookahead covers the R-BACK-32.10 one-held-source, prefix ownership, already-ready FIFO proof, and immediate no-ready fail-open refinement. CpuReadySessionProgress covers the bounded scheduling subset of R-VERIF-2.14: tentative-prefix rollback, suffix visibility, ordered action-covered release fences, generation-current pressure acknowledgement, completion/reclaim, and a forward-looking terminal shutdown action. The terminal action is model-only today: production shutdown still uses direct drain and has no `requestTerminal` caller. BufferBackingVersioning covers concrete backing snapshot/use watermarks and safe MANAGED upload rotation. Traceability matrix in `specs/verification/spec.md` §7. |
| Encode-scheduling model extensions (`R-VERIF-2.13`–`2.15`) | ⚠️ | `CpuReadySessionProgress.tla` now covers the bounded coordinator/release-fence subset of R-VERIF-2.14 and passed TLC on 2026-08-05 with 1,889,134 generated / 314,463 distinct states at depth 34. It deliberately abstracts payload/page allocation geometry, descriptor/page generation reuse, control-reason payloads, device-loss failure semantics, and Metal command effects; those R-VERIF-2.14 obligations remain open. Also missing are the generalized bounded ready-prefix DCE proof (R-VERIF-2.13) and ordered parallel-child or Metal 4 joint completion refinement (R-VERIF-2.15). |
| All fourteen specs model-checked by TLC — zero errors | ✅ | The full `verify_tla.sh` run completed on 2026-07-25. `DceChunkLookahead` generated `286` states / `124` distinct states at depth `16`; the pre-existing models also completed without invariant or temporal-property errors. |
