...
|---|---|---|
| TLA+ specs: CommandQueue, QueueLifecycleRefinement, PresentFrameLatency, ResourceLifetime, BufferBackingVersioning, EncoderLifecycle, QuerySeqId, ConcurrentProgressSignals, DrawableToken, WireHandleGeneration, WireObjectRegistryV2, PresentIdAba, EncodeSessionCompletion, DceChunkLookahead, CpuReadySessionProgress | ✅ | ConcurrentProgressSignals covers pacing-axis independence. DrawableToken / WireHandleGeneration / PresentIdAba cover drawable handoff, PE→unix generation stamps, and ABA safety. EncodeSessionCompletion covers ordered per-source completion and DceChunkLookahead covers the one-held-source proof window. CpuReadySessionProgress covers the pre-`R-BACK-2.65` bounded scheduling subset of R-VERIF-2.14: tentative rollback, suffix visibility, ordered semantic/pressure fences, pressure acknowledgement, completion/reclaim, and a forward-looking terminal action. It does not prove session lease/headroom, deterministic cap-only grouping, or no pressure-created release. BufferBackingVersioning covers backing snapshot/use watermarks. Traceability is in `specs/verification/spec.md` §7. |
| Encode-scheduling model extensions (`R-VERIF-2.13`–`2.15`) | ⚠️ | `CpuReadySessionProgress.tla` passed TLC on 2026-08-05 with 1,889,134 generated / 314,463 distinct states at depth 34. It abstracts page geometry, generation reuse, control payloads, device loss, and Metal effects, and models the rejected pressure-release predecessor. Extend it with the `R-BACK-2.65` capacity lease, successor reserve, completion-schedule-independent grouping, and no-pressure-release invariant. The generalized ready-prefix DCE proof and parallel/Metal 4 completion refinement also remain open. |
| All fifteen specs model-checked by TLC — zero errors | ✅ | The full `dxmt9-verify-tla` run completed on 2026-08-05 after adding `CpuReadySessionProgress`; all modules completed without invariant, deadlock, or liveness error. |
