...
|---|---|---|
| TLA+ specs: CommandQueue, QueueLifecycleRefinement, PresentFrameLatency, ResourceLifetime, BufferBackingVersioning, EncoderLifecycle, QuerySeqId, ConcurrentProgressSignals, DrawableToken, WireHandleGeneration, PresentIdAba, EncodeSessionCompletion | ✅ | ConcurrentProgressSignals added 2026-05-09 (T2 closes G1: pacing-axis independence under R-ARCH-6.8/6.9). DrawableToken / WireHandleGeneration / PresentIdAba added 2026-05-18 (close drawable-token handoff race, PE→unix generation-stamp zombie, and (slot, generation) ABA-safety respectively). EncodeSessionCompletion adds R-BACK-2.49 coverage for one Metal session tail expanding into ordered per-source `seqId` completion. BufferBackingVersioning adds R-BACK-5.11 coverage for concrete backing snapshot/use watermarks and safe MANAGED upload rotation. Traceability matrix in `specs/verification/spec.md` §7. |
| All twelve specs model-checked by TLC — zero errors | ✅ | EncoderLifecycle now uses exact `lastReadHandles` / `lastWriteHandles : SUBSET Handles` instead of Boolean `hazardFlag` (T1 closes G2 / R-VERIF-4.4); `AtMostOneEncoder` tautology removed (T1 closes G3 / R-VERIF-4.1); `BloomNeverForcesSplit` invariant proves Bloom signal cannot trigger an encoder split (T4 closes G4 / R-BACK-2.28). EncodeSessionCompletion adds 2,226 distinct states covering R-BACK-2.49. BufferBackingVersioning checks 11,085 generated / 4,104 distinct states with zero errors. |
