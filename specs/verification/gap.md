...
|---|---|---|
| TLA+ specs: CommandQueue, QueueLifecycleRefinement, PresentFrameLatency, ResourceLifetime, BufferBackingVersioning, EncoderLifecycle, QuerySeqId, ConcurrentProgressSignals, DrawableToken, WireHandleGeneration, WireObjectRegistryV2, PresentIdAba, EncodeSessionCompletion, DceChunkLookahead | ✅ | ConcurrentProgressSignals added 2026-05-09 (T2 closes G1: pacing-axis independence under R-ARCH-6.8/6.9). DrawableToken / WireHandleGeneration / PresentIdAba added 2026-05-18 (close drawable-token handoff race, PE→unix generation-stamp zombie, and (slot, generation) ABA-safety respectively). EncodeSessionCompletion covers one Metal session tail expanding into ordered per-source `seqId` completion. DceChunkLookahead covers the R-BACK-32.10 one-held-source, prefix ownership, already-ready FIFO proof, and immediate no-ready fail-open refinement. BufferBackingVersioning covers concrete backing snapshot/use watermarks and safe MANAGED upload rotation. Traceability matrix in `specs/verification/spec.md` §7. |
| Encode-scheduling model extensions (`R-VERIF-2.13`–`2.15`) | ❌ | The current one-successor `DceChunkLookahead.tla` and serial-tail `EncodeSessionCompletion.tla` remain valid baselines. Missing: bounded already-ready prefix proof, CPU-ready/session admission progress, and ordered parallel-child or Metal 4 joint completion refinement. No new TLA files were added by the scheduling spec consolidation. |
