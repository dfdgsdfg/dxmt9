...
|---|---|---|
| TLA+ specs: CommandQueue, QueueLifecycleRefinement, PresentFrameLatency, ResourceLifetime, BufferBackingVersioning, EncoderLifecycle, QuerySeqId, ConcurrentProgressSignals, DrawableToken, WireHandleGeneration, WireObjectRegistryV2, PresentIdAba, EncodeSessionCompletion, DceChunkLookahead, CpuReadySessionProgress, SessionCapacityLease, PostEncodePayloadRetirement | ✅ | `PostEncodePayloadRetirement` models the synchronous Encode gate, receipt activation, locked detach, out-of-lock owner destruction, finish-time page/generation release, mixed legacy/receipt completion, exactly-once device-loss settlement, retained resources, and Present/completed-Present ordering. `SessionCapacityLease` separates physical residency from encoded work and preserves a deterministic work-cap boundary independent of reclaim or GPU completion. Traceability is in `specs/verification/spec.md` §7. |
...
| All eighteen specs model-checked by TLC — zero errors | ✅ | The full `dxmt9-verify-tla` run completed on 2026-08-06 after the post-encode retirement changes; all modules completed without invariant, deadlock, or liveness error, including the new `PostEncodePayloadRetirement` model and the split-accounting `SessionCapacityLease` refinement. |
