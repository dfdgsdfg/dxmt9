...
| R-BACK-2.85–2.89 / R-VERIF-2.23 (direct construction and end-to-end ownership progress) | bounded queue/pipeline refinement, Arena payload transaction, draw differential, and observer validator implemented; outer transaction/live emitter binding partial | `CpuPipelineOwnership.tla` conserves two FIFO identities through the abstract stages with one-credit admission pressure and a Present-only tail; four expected-failure configs remove wake, complete publication, join-before-completion, and completion-before-reclaim independently. `dxmt9_pipeline_lifecycle.hpp` supplies the fixed-capacity reducer and shared pure predicates exercised by `dxmt9-pipeline-lifecycle-observer-spec`. `TransactionalChunkSlotAssembler` provides sticky failure, constructed-prefix rollback, commit-only publication, and production outer page/control/sequence/resource-generation witnesses for single/batch Arena builders, while the A-B-A fixture compares the direct draw and carrier results. The queue owner emits actual publication/dequeue/inline-finish/reclaim evidence. PE/import/parallel-owner binding, same validated raw mixed-command/resource/failure/completion equivalence, live lease/resource-retain witness validation, and the remaining PE seal materializations remain open. |
...
| `CpuPipelineOwnership.tla` | Bounded end-to-end CPU ownership/progress refinement with two FIFO sources, one queue credit, and a Present-only tail | shared `queue::{pipelinePublicationMayCommit,pipelineCompletionMayPublish,pipelineOwnerMayReclaim,pipelineTransitionRequiresAdmissionWake,pipelineAdmissionWaitSatisfied}` plus the fixed-capacity reducer in `src/dxmt9/dxmt9_pipeline_lifecycle.hpp`; concrete Arena assembler outer witnesses and the queue publication/dequeue/inline-finish/reclaim owner are bound, while PE/import/parallel emitters remain open | `TypeOK`, `OccupancyExact`, `PublicationIsComplete`, `BorrowIsSynchronous`, `CompletionAuthorityAfterJoin`, `NoPrematureReclaim`, `CompletedPrefix`, `PresentCompletionOrdered`, `AdmissionReleaseNotifies`; four mutation configs fail independently | `EverySourceEventuallyReclaimed`, `PresentOnlySourceEventuallyCompletes`, `AdmissionWaitEventuallyRetries` under weak fairness | `tests/native/backend/pipeline_lifecycle_observer_spec.cpp` drives exact identity/generation, completion/present sequences, occupancy, condition-variable admission/reclaim wake, rollback, state-only, shutdown, early retirement, and stale/mutated negatives without sleeps or Metal timing |
...
The completed native lifecycle observer must consume events emitted by the same
production owners mapped by the formal actions. The current reducer accepts the
required identity, generation, stage, payload, ownership, borrow, and
disposition fields, but its fixture emits them from `FakePipelineQueue`; no
production transition owner calls the sink yet. The fixture therefore tests the
reducer and missing-wake/partial-publication counterexamples without sleeps, but
does not drive the real queue condition variables or close R-VERIF-7.6.
R-VERIF-7.3 remains the effective-command oracle and must eventually be joined
by identity/ordinal rather than expanded into a second ownership authority.
