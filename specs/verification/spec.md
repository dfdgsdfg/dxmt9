...
| `backend/encode-scheduling/requirements.md` R-BACK-2.67 (composed scheduling progress) | `tla/EncodeSchedulingProgress.tla` composes the published interfaces of the admission, capacity, wake, session, retirement, completion, and Present models; queue weak fairness and GPU environment fairness remain explicit and separate | `render/encode_scheduling_progress.hpp`, `SchedulingProgressWatchdog`, `QueueLifecycleController`, `CommandQueue`, and `tests/native/backend/encode_scheduling_progress_spec.cpp` |
...
| `EncodeSchedulingProgress.tla` | Bounded composed scheduling pipeline from accepted admission through capacity lease, publication/generation wake, FIFO session continuation, optional deferred retirement, completion expansion/release, and distinct Present publication-or-skip plus settlement | `render/encode_scheduling_progress.hpp`, `SchedulingProgressWatchdog`, `QueueLifecycleController`, and `CommandQueue` | `TypeOK`, `BoundedStores`, `OwnershipConservation`, `FifoSessionAndCompletion`, `PayloadRetirementSafety`, `PresentDecisionSeparation`, `StickyObligations`, `CapacityLostWakeupFreedom`, `EncoderLostWakeupFreedom`, plus temporal `StickyTracking` | `EveryAcceptedSourceReleased`, `EveryPresentDecided`, `EveryPresentSettled`; weak fairness covers queue-owned enabled actions only, while `GpuSettlementAssumption` is explicitly environmental | `tests/native/backend/encode_scheduling_progress_spec.cpp`, `tests/native/backend/present_ordinal_boundary_spec.cpp` |
