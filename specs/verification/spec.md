...
| R-BACK-2.85–2.89 / R-VERIF-2.23 (end-to-end CPU pipeline owner lifecycle) | `tla/CpuPipelineLifecycle.tla` imports generated `PipelineLifecycleTable.tla`; the bounded safety cfg checks generation/occupancy/owner/wake/present/no-GPU predicates and independent failure counterexamples, while `CpuPipelineLifecycle.progress.cfg` checks weak-fair normal completion, reclaim, Present settlement, and selected-parallel join | `QueueLifecycleController`, `QueueSubmissionRecord`, `CommandQueue`, `DeviceImpl`, `Device::resetValidated`, `SchedulingProgressWatchdog`, `pipelineReplayFailureMayRollback`, and focused native backend specs; the shared rollback predicate binds the model's pre-effect retry/post-effect fail-stop cut to `DirectChunkSlotReplayLease` and its native truth table |
...
| R-BACK-2.85–2.89 / R-VERIF-2.23 (direct construction and end-to-end ownership progress) | Arena/ordinary final-storage transactions, same-raw differential, production Metal oracle, and observer validator implemented; queue/pipeline refinement partial | `CpuPipelineOwnership.tla` conserves two FIFO identities with one-credit admission pressure and a Present-only tail, explicitly separating `GPUInFlight`, `Completed`, and `Reclaimed`; its independent expected-failure configs cover wake, publication, join, skipped completion, reclaim-before-completion, premature reclaim, and fabricated GPU milestones. `dxmt9_pipeline_lifecycle.hpp` supplies the fixed-capacity reducer plus the production `pipelineReplayFailureMayRollback` cut, whose native truth table is consumed by `DirectChunkSlotReplayLease`; `include/dxmt9/progress_predicates.hpp` owns the PE/Unix shared pure progress predicates. `TransactionalChunkSlotAssembler` provides sticky failure, constructed-prefix rollback, fallible prepare, typed move-only live transaction evidence, and evidence-gated commit for Arena and ordinary final `ChunkSlot` builders. The queue sink records actual publication/dequeue/inline-finish/reclaim owner callbacks; zero-command-buffer inline work emits the explicit `NoGpuTerminal` disposition without fabricated GPU milestones, and poison-stop raw/encoding edges are owner-qualified to `DeviceLoss`. `cpu_ready_production_routing_spec` compares immutable validated pointer-free raws across ordinary Direct/Legacy mixed Draw/Clear/copy cadence, exact draw payload, resources, Present closure, completion identity/digest, typed pre-effect fallback, and injected post-effect no-retry; the companion whole-raw oracle retains Clear/copy/Present and the Readback Inline barrier. `direct_chunk_slot_metal_oracle_spec.mm` executes the real device/D9C replay/Metal readback path in fresh env `0`/`1` processes, proves Direct commit ingress, and compares exact BGRA8 output plus command/resource/completion identity. The 2026-08-31 same-build GT1/GT2/GT3/SFIV wild matrix and GT2 copy ledger close the ordinary default-promotion gate. PE/import/parallel-owner formal binding and broader bounded x64/x86 Wine faults remain open. |
...
| `CpuPipelineOwnership.tla` | Bounded CPU ownership/progress model with two FIFO sources, one queue credit, and a Present-only tail; concrete completion/reclaim refinement partial | shared `queue::{pipelinePublicationMayCommit,pipelineCompletionMayPublish,pipelineOwnerMayReclaim,pipelineTransitionRequiresAdmissionWake,pipelineAdmissionWaitSatisfied}` plus the fixed-capacity reducer in `src/dxmt9/dxmt9_pipeline_lifecycle.hpp`; concrete Arena assembler prepare/typed-live-evidence/commit witnesses and queue publication/dequeue/reclaim callbacks are partially bound, while PE/import/parallel emitters remain bounded by the existing observer abstractions. The model now separates `GPUInFlight -> Completed -> Reclaimed`, and the production inline sink emits `NoGpuTerminal` without fabricated GPU milestones. Queue-issued `GenerationQualifiedSourceBorrow` / `SynchronousSourceBorrowBatch` capabilities freeze and prevalidate exact Tape generations under the queue mutex, pin them through unlocked work, and revalidate after relock before epoch deactivation. Unlocked payload projections use only the atomic epoch and frozen identity; callbacks receive a noncopyable `SynchronousSourcePayloadBorrow`, and `WorkerOwnedSourceSnapshot` is the supported value-owned handoff. This prevents ordinary copy-by-value capability escape and stale capability access, but a caller can deliberately retain addresses from command/span projections; C++ cannot make that hostile lifetime violation impossible. Production DCE/session lookahead now runs under the same borrow batch; local planner spans are not retained. | `TypeOK`, `OccupancyExact`, `PublicationIsComplete`, `BorrowIsSynchronous`, `CompletionAuthorityAfterJoin`, `NoPrematureReclaim`, `NoReclaimBeforeCompletion`, `NoSkippedCompletion`, `CompletedPrefix`, `PresentCompletionOrdered`, `AdmissionReleaseNotifies`; independent mutation configs fail at their distinct invariants. `CompletedPrefix` is a sequence-set property, while `Completed` is a separate abstract ownership phase. | `EverySourceEventuallyReclaimed`, `PresentOnlySourceEventuallyCompletes`, `AdmissionWaitEventuallyRetries` under weak fairness | `tests/native/backend/pipeline_lifecycle_observer_spec.cpp` drives exact identity/generation, completion/present sequences, occupancy, condition-variable admission/reclaim wake, rollback, state-only, shutdown, early retirement, and stale/mutated negatives without sleeps or Metal timing; `tests/native/backend/queue_completion_sources_spec.cpp` adds forged-construction compile-time pins plus nested-reentry, concurrent-reclaim, post-validation mismatch, stale-after-reclaim, and call-local resolved-view negatives; `tests/native/backend/cpu_ready_production_routing_spec.cpp` adds the same-raw Direct/Legacy oracle |
...
| `SegmentedTransportV1.tla` | Fixed-role three-stage semantic-batch transport: pure exact count/unique-handle proof, complete reservation, atomic adoption, contiguous ExactFixed CPU Tape emission, fallback/poison, FIFO settlement, and reclaim wake | Opt-in PE/WoW64/Unix segmented bridge binds fixed-region validation and all-or-nothing Unix ownership; host-only `ImmutableSemanticBatchOwner`/`SemanticBatchSettlement` binds the value transitions, while production CPU-ready Tape does not yet execute the three queue roles | `TypeOK`, `PureCountsAndDedup`, `WireExtentBounded` (32 MiB total-wire cap), `RoleOrder`, `AdoptionIsAtomic`, `NoPartialPublication`, `ReservationIsComplete`, `CapacityConserved`, `LedgerConserved`, `ExactFixedContiguous`, `NoFallbackAfterEffect`, `RollbackRestoresCheckpoint`, `FIFOSettlement`, `WakeOnReclaim`, `NoRetryAfterEffect` | `EventuallySettledOrFallbackOrPoisoned`, `EventuallySettlementOrPoison` under weak fairness | `dxmt9-pe-semantic-batch-spec`, segmented validator/registry/replay specs, ABI codegen tests, and PE32/PE64/Unix layout assertions; `scripts/check/verify_tla.sh` executes six independent failures for incomplete reservation, partial adoption, fallback-after-effect, lost checkpoint, partial ledger, and missing wake |
...
| `PeRecorderSemanticProjection.tla` | All 21 production PE record families: source ordinal before CapacityPre; exact accepted record ordinal, byte range, payload and qualified identities; retry, effect-unknown poison, and three capture dispositions | `DXMT9_PE_SEMANTIC_PRODUCER_TABLE`, `planCommittedPeSemanticProjection`, the common `D3D9DeviceImpl::appendRecord` envelope, `CommandChunkBuilder::visitLastCommittedRecord*`, and the cold `PeAllFamilySemanticTokenLedger` | `TypeOK`, `ExactProjection`, `RetryStable`, `CaptureAfterAccept`, `BridgeEffectUnknownFailStop`; ten independent expected-failure configs remove source, record, range, value, identity, A/B/A, wrong-producer, wrong-source, fail-stop, or capture conservation | _(finite safety model)_ | `tests/native/bridge/pe_semantic_projection_spec.cpp` exhausts every producer and exact-field combination, immutable legacy/direct facts, full identities, retry, poison, capture, and discard. `gen_pe_semantic_producer_table.py --check` rejects model/table drift. PE/Wine faults remain open. |
...
| `QueueT2dReserveCopyCommit.tla` (deferred gate) | Bounded reserve/copy/commit decision model for one queue writing slot: pre-existing prefix checkpoint, private reservation bytes, capacity charge, frozen generation/ticket, complete prefix-plus-reservation publication, rollback/wake, and recycle/commit hazards | Prospective `CommandQueue::submitDrawRunBatchImpl` append; current production append remains under `CommandQueue::mutex_` and no model transition is enabled in production | `TypeOK`, `CapacityConserved`, `NoHalfAppendedSlot`, `NoStaleReservationWrite`, `CompletePublication`, `ReservationIsolation`, `RollbackRestoresCharge`, `RollbackWakesWaiters`, `RollbackRestoresPrefix`; production config passes, while `.half-appended-slot.counterexample.cfg`, `.stale-reservation.counterexample.cfg`, and `.lost-prefix-rollback.counterexample.cfg` fail their named invariants | _(bounded safety/preflight model; no fairness claim)_ | No production concurrency implementation is claimed. Existing `dxmt9-producer-interleaving-spec` / TSan / source-order evidence and matched qmutex profile are recorded in append decomposition `.36`; the model's deliberate counterexamples are executed by `scripts/check/verify_tla.sh`. |
...
This is the evidence owner for R-VERIF-2.23 and R-VERIF-7.6–7.7; architecture
