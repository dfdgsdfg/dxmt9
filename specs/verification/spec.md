...
| R-BACK-2.85–2.89 / R-VERIF-2.23 (direct construction and end-to-end ownership progress) | Arena payload transaction, draw differential, and observer validator implemented; queue/pipeline refinement partial | `CpuPipelineOwnership.tla` conserves two FIFO identities with one-credit admission pressure and a Present-only tail, explicitly separating `GPUInFlight`, `Completed`, and `Reclaimed`; its independent expected-failure configs cover wake, publication, join, skipped completion, reclaim-before-completion, premature reclaim, and fabricated GPU milestones. `dxmt9_pipeline_lifecycle.hpp` supplies the fixed-capacity reducer, while `include/dxmt9/progress_predicates.hpp` owns the PE/Unix shared pure progress predicates exercised by `dxmt9-pipeline-lifecycle-observer-spec`. `TransactionalChunkSlotAssembler` provides sticky failure, constructed-prefix rollback, fallible prepare, typed move-only live transaction evidence, and evidence-gated commit for single/batch Arena builders. The queue sink records actual publication/dequeue/inline-finish/reclaim owner callbacks; zero-command-buffer inline work emits the explicit `NoGpuTerminal` disposition without fabricated GPU milestones, and poison-stop raw/encoding edges are owner-qualified to `DeviceLoss`. `cpu_ready_production_routing_spec` compares one immutable validated pointer-free raw fixture across Direct/Legacy command order, explicit payload digest/layout, normalized ordinals, resource identities/conservation, Clear/copy/Present, the Readback Inline barrier, failure dispositions, and completion identity. PE/import/parallel-owner formal binding and GPU/wild equivalence remain bounded/open. |
