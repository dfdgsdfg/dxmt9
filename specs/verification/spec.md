...
| R-BACK-2.60 / R-VERIF-2.14 (CPU-ready/session admission progress) | missing `CpuReadySessionProgress` model | planned CPU-ready store and encode coordinator |
...
| CPU-ready/session progress (missing) | Bounded source admission, fail-open prefix submission, and pressure independence (`R-VERIF-2.14`) | planned CPU-ready store and encode coordinator | bounded occupancy, represented-source ownership, no hidden consumed prefix | admission pressure cannot prevent releasing completion | missing queue-observer spec |
