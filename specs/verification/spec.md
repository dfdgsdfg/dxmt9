...
|---|---|---|
| `d3d9/recorder/requirements.md` R-CORE-REC-1.1–3.1, 6.1–6.3 | `PeRecorderTransition.tla` plus its expected `ConsumeOnPrepare` counterexample and `dxmt9-pe-transition-algebra-spec` bind state-write and append settlement; builder/registry/capture tests cover adjacent transactions | `planRecorderStateWrite` / `settleRecorderAppend` in `d3d9_pe_transition_algebra.hpp`; typed owners in `d3d9_pe_state_shadow.hpp`; production settlement in `d3d9_pe_producer.cpp` and `D3D9DeviceImpl`; seal/bridge/capture-journal settlement remains a separate open refinement |
...
| `PeRecorderTransition.tla` | PE qualified state writes, fixed state-block tracking, pre-effect recorder failure, and prepare/accept/fail/discard append settlement; post-effect Apply failure is explicitly outside the model | `planRecorderStateWrite`, `settleRecorderAppend`, `acceptPreparedSparseState`, the four normalized keyed setters, six constant adapters, and PE state-block wrapper snapshot | `TypeOK`, `PreparedIsNonConsuming`, `OnlyAcceptedConsumes`, `AcceptedExactlyRepresented`, `FailedRetryStable`, `NoLostPending`, qualified-key/fixed-set/prior-value invariants; `ConsumeOnPrepare` must violate `NoLostPending` | `PreparedEventuallySettles`, `AcceptedEventuallyReplayed`, `InputStopEventuallySettledOrRetained` under weak fairness | `tests/native/bridge/pe_transition_algebra_spec.cpp`, `pe_shadow_native_spec.cpp`, `pe_producer_differential_spec.cpp`, `tests/native/core/core_stateblock_restore_spec.cpp` |
