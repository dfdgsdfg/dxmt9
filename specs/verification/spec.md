...
| R-BACK-2.60 / R-BACK-2.65 / R-VERIF-2.14 (CPU-ready/session admission progress) | `tla/CpuReadySessionProgress.tla` covers the bounded pressure-release predecessor; session lease/headroom and no-pressure-release refinement remain missing | `CpuReadyTape` and `QueueLifecycleController` in `src/dxmt9/dxmt9_cpu_ready_tape.hpp` / `src/dxmt9/dxmt9_queue.*` |
...
| `CpuReadySessionProgress.tla` plus `R-BACK-2.65` refinement gap | Current model covers bounded admission, pre-effect rollback, ordered semantic/pressure fences, pressure progress, and a model-only terminal extension; it does not cover the required session lease, successor headroom, deterministic cap-only grouping, or no-pressure-release invariant | `CpuReadyTape`, tentative `QueueLifecycleController` APIs, and the command-queue release coordinator; capacity-lease and terminal production wiring remain planned | current: exact FIFO partition, bounded storage, suffix-stays-Ready, fence-respecting representation, action-covered acknowledgement, no completion before submit; missing: completion-schedule-independent grouping and no pressure-created release | current pressure fences eventually acknowledge; required refinement must let pressure delay lease/publication without choosing a session boundary | `tests/native/backend/cpu_ready_tape_spec.cpp`, `tests/native/backend/queue_completion_sources_spec.cpp`, `tests/native/backend/cpu_ready_session_join_spec.cpp`; missing lease/headroom queue-observer spec |
