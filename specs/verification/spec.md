...
| R-BACK-2.60 / R-BACK-2.65 / R-VERIF-2.14 (CPU-ready/session admission progress) | missing `CpuReadySessionProgress` model | planned CPU-ready store and encode coordinator |
...
| CPU-ready/session progress (missing) | Bounded source admission, fixed session lease/headroom, deterministic cap submission, and pressure independence (`R-VERIF-2.14`) | planned CPU-ready store and encode coordinator | bounded occupancy, represented-source ownership, no hidden consumed prefix, completion-schedule-independent grouping, no pressure-created release | admission pressure cannot prevent completion/reclaim or consume the successor reserve | missing queue-observer spec |
