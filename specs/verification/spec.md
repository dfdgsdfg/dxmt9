...
| `EncodeSchedulingProgress.tla` | Bounded composed scheduling pipeline plus a seeded older-Submitted/Ready denied-first-lease state in which Arena admission pressure must spend two distinct exact-head serial-progress tokens before the complete batch-control predicate becomes Ready, without changing capacity generation | `classifyFirstLeaseCapacityWait` in `render/encode_scheduling_progress.hpp`, `SchedulingProgressWatchdog`, `QueueLifecycleController`, and `CommandQueue` | Existing pipeline invariants plus `FirstLeaseWaitActionEnabled`, `PressureEscapeIsStandalone`, and `PressureEscapeConservation`; `PressureSerialEscape` leaves session-release state absent, preserves FIFO ownership, cannot join the escaped source to a session, and cannot repeat an escaped identity | Existing release/Present/terminal properties plus `SeededDeniedLeasePressureProgress`, `EveryPressureEscapedSourceReleased`, `DeniedFirstLeaseAdmissionCycleBroken`, and `ChangedHeadRearmsWithoutCapacityGeneration`; GPU settlement remains explicitly environmental | `tests/native/backend/encode_scheduling_progress_spec.cpp` exhausts the production action/identity truth table; `tests/native/backend/cpu_ready_session_join_spec.cpp` drives the real production loop, multi-source admission waiter, and CV transitions without polling or sleeps; `tests/native/backend/present_ordinal_boundary_spec.cpp` retains Present coverage |
