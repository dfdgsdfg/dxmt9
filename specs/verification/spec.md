...
| `DirectSlotAggregateCapacityLease.tla` (R-BACK-2.104 aggregate refinement) | Two bounded physical payloads are a symmetry reduction of the production 64-payload compatibility owner; staged replacement charges old plus new and then atomically adopts all 30 allocation dimensions or rolls staged credit back, with generation/serial-qualified settlement tickets and explicit stale/double rejection actions | `reduceDirectSlotCapacityLease`, `CpuReadyTape::compatibilityPayloadRetainedBytes`, physical compatibility-payload index resolution, and `provisionEmptyDirectSlotStorage` | retained/staged credit conservation, transient aggregate bound, capacity-generation conservation, ticket qualification/no-double-settlement, all-or-none adoption | terminal stutter after successful adoption or allocation rollback | `tests/native/backend/direct_slot_provisioning_spec.cpp` injects every production allocation and checks exact live topology, 64-payload compatibility sizing, over-limit observation, and lease rollback/stale settlement |
| `DirectSlotCapacityProvisioning.tla` (R-BACK-2.103–2.105 capacity/reuse refinement) | Bounded adjacent source shapes share persistent payload capacity across publish/reclaim generations; an empty reclaimed payload either reuses complete retained storage or takes the failure-atomic provision path, while incomplete detached owner storage and stale ledger credit fail closed | `directSlotStorageTransition`, `directSlotEmptyStorageReusable`, `directSlotCapacityLedgerQualifiesReuse`, `restoreDirectSlotLookupTables`, and the `ChunkSlot::detachResourceOwners` / `restoreResourceOwnerStorage` round trip | boundary credits against a same-capacity serial reference, no populated growth, no redundant reprovision, complete-retention premise, exact ledger premise, and unchanged exact-fit lane | finite safety model; terminal stutter after rotation/provision/reuse | `tests/native/backend/direct_slot_provisioning_spec.cpp` binds the reducer, current allocation inventory, per-dimension insufficient-capacity matrix, lookup restore, address/capacity stability, reclaim round trip, and stale-ledger rejection; three dedicated counterexample configs remove the reuse premises |
