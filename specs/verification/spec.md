...
| R-ARCH-7.11–7.24 / R-BACK-2.85–2.100 / R-VERIF-2.23 and R-VERIF-7.9–7.10 (direct construction and end-to-end ownership progress) | Arena/ordinary final-storage transactions, same-raw differential, production Metal oracle, queue/Tape batch projection, and PE-to-reclaim identity composition implemented; source-wide replay-state transaction, carrier retirement, materialization-floor evidence, and promotion evidence partial | `CpuPipelineOwnership.tla` separates `GPUInFlight`, `Completed`, and `Reclaimed`; `CpuPipelineLifecycle.tla` owns the immutable producer interval, contiguous GPU-completion frontier, and finish-thread waterline composition. `dxmt9_pipeline_lifecycle.hpp` supplies the strict production reducer, storage-generation sidecar, physical batch id/index/count, and replay rollback cut. Production emits compatibility and Arena arrival/publication, every multi-source dequeue/submit/completion identity, finish advance, actual reclaim, inline no-GPU terminal, and pending-completion fail-stop. `TransactionalChunkSlotAssembler` retains sticky pre-effect rollback/post-effect fail-stop and direct continuation extends only adjacent PE intervals. Native PE owner/import/Tape/observer tests bind the same range relation. The model/code binding does not yet distinguish a valid Unix `RawOwned` copy from an invalid retained PE span or unowned same-address alias, and it does not yet atomically bind working-state commit to final emission. `DrawRunSubmission` and its APIs/tests/counters remain in production, and no fresh ledger yet proves the four-boundary floor. Broader all-family direct projection, resolved-sidecar content coverage, C++ atomic ordering, drawable-token/WSI cleanup, and Wine/GPU/wild equivalence remain open. |
