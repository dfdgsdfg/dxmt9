...
|---|---|---|
| `d3d9/recorder/requirements.md` R-CORE-REC-1.1–3.5, 5.1.3–5.1.4, 6.1–6.3 / R-VERIF-6.7 | `PeRecorderTransition.tla` binds the production semantic pending tuple; `PeRecorderCommit.tla` and composed `PeRecorderSettlement.tla` bind generic acceptance/count/ordinal and CapacityPost dispositions through bridge/capture, drain/reset, and recovery without inventing a semantic tuple from wire type/size. `PeStateBlockTransitionTable.tla`, `PeStateBlockTransaction.tla`, and repeated-value `PeStateBlockValues.tla` bind lifecycle rows, monotonic Recording epochs and stale-capability rejection, frozen tracked sets, repeated Capture/Apply, and staged-ref cardinality | `planRecorderStateWrite` / `settleRecorderAppend` / `settleRecorderCommit` / `planPeStateBlockTransition` / `planPeStateBlockValue`; production settlement remains in `d3d9_pe_producer.cpp`, `D3D9StateBlockImpl`, and `D3D9DeviceImpl`. Exact same-slot COM reference conservation is native fake-object evidence, and semantic cross-projection through the heterogeneous append envelope remains open. |
...
| R-BACK-2.85–2.89 / R-VERIF-2.23 (direct construction and end-to-end ownership progress) | Arena payload transaction, draw differential, and observer validator implemented; queue/pipeline refinement partial | `CpuPipelineOwnership.tla` conserves two FIFO identities with one-credit admission pressure and a Present-only tail; four expected-failure configs independently remove wake, complete publication, join-before-completion, and completion-before-owner-release. The model still moves directly from `GPUInFlight` to `Reclaimed`, so it does not refine the concrete `Completed` owner separately. `dxmt9_pipeline_lifecycle.hpp` supplies the fixed-capacity reducer, while `include/dxmt9/progress_predicates.hpp` owns the PE/Unix shared pure progress predicates exercised by `dxmt9-pipeline-lifecycle-observer-spec`. `TransactionalChunkSlotAssembler` provides sticky failure, constructed-prefix rollback, fallible prepare, typed move-only live transaction evidence, and evidence-gated commit for single/batch Arena builders. The queue sink records actual publication/dequeue/inline-finish/reclaim owner callbacks, but its inline zero-command-buffer callback currently fabricates GPU-bearing milestones instead of the required explicit no-GPU terminal row. `cpu_ready_production_routing_spec` compares one immutable validated pointer-free raw fixture across Direct/Legacy command order, explicit payload digest/layout, normalized ordinals, resource identities/conservation, Clear/copy/Present, the Readback Inline barrier, failure dispositions, and completion identity. Split completion/reclaim, a real inline terminal disposition, PE/import/parallel-owner formal binding, GPU/wild equivalence, and the remaining PE seal materializations remain open. |
...
The native lifecycle observer consumes events from production queue owners,
but it is not yet isomorphic to the formal actions. `CpuPipelineOwnership.tla`
combines GPU completion and reclaim, while the production inline path expands a
zero-command-buffer transition into fabricated GPU milestones instead of the
explicit no-GPU terminal row required by R-BACK-2.88.
