...
| R-ARCH-7.11–7.19 / R-BACK-2.85–2.95 / R-VERIF-2.23 and R-VERIF-7.9 (end-to-end CPU pipeline owner lifecycle) | `tla/CpuPipelineLifecycle.tla` imports generated `PipelineLifecycleTable.tla`; the bounded safety cfg checks generation/occupancy/owner/wake/present/no-GPU predicates and independent failure counterexamples, while `CpuPipelineLifecycle.progress.cfg` checks weak-fair normal completion, reclaim, Present settlement, and selected-parallel join. The existing model covers Unix stages; the PE-accept-to-reclaim composed identity trace required by R-VERIF-7.9 remains open. | `QueueLifecycleController`, `QueueSubmissionRecord`, `CommandQueue`, `DeviceImpl`, `Device::resetValidated`, `SchedulingProgressWatchdog`, `pipelineReplayFailureMayRollback`, and focused native backend specs; the shared rollback predicate binds the model's pre-effect retry/post-effect fail-stop cut to `DirectChunkSlotReplayLease` and its native truth table |
...
| R-ARCH-7.11–7.19 / R-BACK-2.85–2.95 / R-VERIF-2.23 and R-VERIF-7.9 (direct construction and end-to-end ownership progress) | Arena/ordinary final-storage transactions, same-raw differential, production Metal oracle, and identity-qualified queue/Tape batch projection implemented; full PE-to-reclaim composition partial | `CpuPipelineOwnership.tla` separates `GPUInFlight`, `Completed`, and `Reclaimed`; `CpuPipelineLifecycle.tla` owns the contiguous GPU-completion frontier and finish-thread waterline composition. `dxmt9_pipeline_lifecycle.hpp` supplies the fixed reducer, physical batch id/index/count, and replay rollback cut. Production emits Arena arrival/publication, every multi-source dequeue/submit/completion identity, finish advance, actual reclaim, inline no-GPU terminal, and pending-completion fail-stop. `TransactionalChunkSlotAssembler` retains sticky pre-effect rollback/post-effect fail-stop and admits the narrow single-source optional-Clear + non-UP Draw/APPLY_STATE/constants + terminal Present shape as `DirectWithPresentTail`; Present is appended inside the assembler transaction and publication/boundary remain coordinator-owned. Existing `CpuPipelineLifecycle` ownership/completion refinement and `cpu_ready_production_routing_spec.cpp` source/cadence/completion pins cover the publication identity; drawable-token/WSI side-effect cleanup and wild visual/locality equivalence remain open until dedicated evidence is added. Exact PE import, selected-parallel child/join, receipt detail, sidecar generation, C++ atomic ordering, and broader Wine/GPU/wild equivalence remain open. |
...
| `DirectChunkSlotContinuation.tla` | Small populated-slot continuation transaction: strict structural/capacity admission for DrawRun plus state-only APPLY_STATE/constants, private staged append, exact pre-effect rollback, commit, and post-effect fail-stop without retry | `core::directContinuationAdmission` is the shared pure typed predicate used by `CommandQueue::beginDirectChunkSlotReplay`; a populated slot remains on the ordinary Direct lane only when final vector/arena and lookup capacity is already present. Clear, Present, ordered control/readback, resource mutation, UP, `TriangleFan`, and malformed shapes fail closed to Legacy before effects. | `TypeOK`, `AdmissionSound`, `LegacyBeforeEffects`, `PrefixPreservedOnRollback`, `CommitRequiresEffects`, `NoRetryAfterPostEffectFailure`, `NoHalfPublishedPrefix`, `StagedCandidateIsPrivate` | Bounded safety model; native truth-table and populated-slot replay pin the same C++ predicate and queue routing. It does not prove Metal pixels, unbounded command grammar, or wild workload behavior. | `tests/native/backend/cpu_ready_production_routing_spec.cpp`, `src/dxmt9/dxmt9_direct_continuation.hpp` |
...
This is the evidence owner for R-VERIF-2.23 and R-VERIF-7.6–7.9; architecture
...
The composed trace uses the architecture's `EndToEndSourceIdentity` as its join
key and records physical representation, storage generation, lease/borrow
count, sidecar generation, effect cut, completion projection, reclaim
authority, and capacity-wake generation. Component models expose only these
published fields to the composition. Native model/code binding must drive the
real PE accept/import transition and the existing direct/compatibility replay,
serial/selected-child encode, completion, and reclaim owners; a test-only
transition producer cannot close the requirement.
