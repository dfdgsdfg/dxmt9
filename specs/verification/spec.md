...
|---|---|---|---|---|---|
| `AutogenMipGeneration.tla` | D3D9 AUTOGEN public-level/hidden-pyramid write-to-sample ordering | `AutogenMipmapState` / `transitionAutogenMipmap`, `D3D9DeviceImpl::generateBoundAutogenMipmaps`, PE surface-write dirty transitions, and `TextureRecord::physicalMipLevels` | `PublishedNeverAhead`, `GeneratedNeverAhead`, `CleanCoversLatestWrite`, `SampleNeverUsesStaleMip`, `ReadyHasExactGeneration`; generation failure preserves Dirty | `RequestEventuallySettles` under fair publish/generate/sample settlement; a failed request settles without sampling and remains retryable | `tests/native/core/core_d3d9_generate_mipmaps_spec.cpp`, `tests/native/backend/wmt_generate_mipmaps_metal_spec.mm`, plus the 3DMark06 HDR1 water-reflection wild oracle |
