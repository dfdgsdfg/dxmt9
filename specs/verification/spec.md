...
| `EncodeSchedulingProgress.tla` | Bounded composed scheduling pipeline plus a seeded older-Submitted/Ready denied-first-lease state in which Arena admission pressure must spend one exact serial-progress credit before completion/reclaim | `classifyFirstLeaseCapacityWait` in `render/encode_scheduling_progress.hpp`, `SchedulingProgressWatchdog`, `QueueLifecycleController`, and `CommandQueue` | Existing pipeline invariants plus `FirstLeaseWaitActionEnabled` and `PressureEscapeIsStandalone`; `PressureSerialEscape` leaves session-release state absent, preserves FIFO ownership, and cannot join the escaped source to a session | Existing release/Present/terminal properties plus `SeededDeniedLeasePressureProgress`, `EveryPressureEscapedSourceReleased`, and `DeniedFirstLeaseAdmissionCycleBroken`; GPU settlement remains explicitly environmental and reclaim advances the capacity generation | `tests/native/backend/encode_scheduling_progress_spec.cpp` exhausts the production action truth table; `tests/native/backend/cpu_ready_session_join_spec.cpp` drives the real production loop/admission waiter without sleeps; `tests/native/backend/present_ordinal_boundary_spec.cpp` retains Present coverage |
