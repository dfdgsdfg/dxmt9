...
| R-ARCH-7.11–7.24 / R-BACK-2.85–2.100 / R-VERIF-2.23 and R-VERIF-7.9–7.10 (direct construction and end-to-end ownership progress) | Arena/ordinary final-storage transactions, source-wide state checkpoint/settlement, same-raw focused differential, queue/Tape batch projection, PE-to-reclaim identity composition, and concrete carrier retirement are implemented; the complete final-slot/direct differential, fresh materialization-floor evidence, and promotion evidence remain open | `CpuPipelineOwnership.tla` separates `GPUInFlight`, `Completed`, and `Reclaimed`; `CpuPipelineLifecycle.tla` owns the immutable producer interval, contiguous GPU-completion frontier, and finish-thread waterline composition. `ReplayProjectionTransaction.tla` owns source projection through exact destination receipt; production and native tests share `advanceReplayTransaction`, while a fixed-capacity typed undo journal contains in-place working-state mutations until settlement. `dxmt9_pipeline_lifecycle.hpp` supplies the strict production reducer, storage-generation sidecar, physical batch id/index/count, and replay rollback cut. The bounded Direct production projection begins at successful admission-witness consumption; RawOwned import and planning are abstract states, not fabricated events. Production emits compatibility and Arena arrival/publication, every multi-source dequeue/submit/completion identity, finish advance, actual reclaim, inline no-GPU terminal, and pending-completion fail-stop; Direct terminal events additionally bind Encode, Complete, Detach, Restore-or-Poison, and Reclaim with full locators, and their complete exactly-one-owner sibling batch reduces against scratch state before the queue ledger or sink is exposed. `TransactionalChunkSlotAssembler` retains sticky pre-effect rollback/post-effect fail-stop and direct continuation extends only adjacent PE intervals. The model/code binding still does not distinguish a valid Unix `RawOwned` copy from an invalid retained PE span or unowned same-address alias. `DrawRunSubmission`, its APIs/adapters/scratch, and carrier-specific counters/tests are absent from production; ordinary fallback now uses synchronous borrowed-state ingress into final `ChunkSlot` storage. The stable `OwnedRawFinalChunkSlot` topology is represented by `FinalOwned` → `Encoding` in `CpuPipelineLifecycle.tla`, with the dedicated encode thread as the stable execution owner. Focused routing tests cover effective-stream and selected final-storage/resource/completion/failure shapes, but the R-BACK-2.103 all-family composed differential is not closed. A fresh four-boundary ledger, broader C++ atomic ordering, and Wine/GPU/wild equivalence remain open; compatibility-only ranges remain compatibility-owned and outside the carrier-free claim. |
| `backend/encode-scheduling/requirements.md` R-BACK-2.101–2.105 / R-VERIF-2.25 (source-range plan and lease lifecycle) | `ReplayEmissionPlanIslands.tla` retains the minimal span slice. `DirectSourceLifecycle.tla` projects the shared production/native reducer over two sources/two slots, separator/ordered-control modes, Present, pre-admission rotation/reuse, behavioral physical-credit ownership for shared semantic spans, weak-fair settlement, exact FIFO/once/no-fallback/completion-before-reclaim/credit invariants, and fourteen independent expected failures. RawOwned import and planning are abstract model states; production observation begins at successful witness admission and has a distinct FIFO row/frontier invariant and reorder counterexample. Nominal transitions exercise tail-only pre-effect cancellation as production-row erasure plus frontier restoration. | `reduceDirectSourceLifecycle` / `reduceDirectSourceLifecycleBatch`, production owner observers, native lifecycle truth tables, existing `dxmt9-replay-emission-plan-spec` / `dxmt9-replay-emission-plan-islands-spec` multi-cut coverage, and the observer-enabled Direct Metal Draw/Clear/Draw/Present exact-pixel/completion/fail-stop oracle; production terminal events bind Encode/Complete/Detach/Restore-or-Poison/Reclaim with full locators and transactionally preflight each exactly-one-owner sibling batch before exposing the ledger or sink, while poisoned detached tokens remain retained on mismatch. Production queue pins cover a compatibility prefix followed by Direct append and rollback followed by compatibility Publish. Non-tail rollback is rejected byte-identically; complete sibling cancellation is preflighted in reverse tail order through the batch reducer. A separate observer-disabled source-contract pin proves one cached branch with no event construction, sink-facade copy, or ledger allocation; it does not claim Metal execution. Native Direct lifecycle and queue-routing cases, the exact Metal oracle, and the complete 161-configuration TLC inventory pass. Ordered-control driver execution and supervised wild locality remain open. |
...
| `DirectSlotAggregateCapacityLease.tla` (R-BACK-2.104 aggregate refinement) | Two bounded physical payloads are a symmetry reduction of the production 64-payload compatibility owner; staged replacement charges old plus new and then atomically adopts all 30 staged allocation dimensions or rolls staged credit back, with generation/serial-qualified settlement tickets and explicit stale/double rejection actions | `reduceDirectSlotCapacityLease`, `CpuReadyTape::compatibilityPayloadRetainedBytes`, physical compatibility-payload index resolution, `provisionEmptyDirectSlotStorage`, and the compile-time role-tagged dimension schema | retained/staged credit conservation, transient aggregate bound, capacity-generation conservation, ticket qualification/no-double-settlement, all-or-none adoption | terminal stutter after successful adoption or allocation rollback | `tests/native/backend/direct_slot_provisioning_spec.cpp` asserts 32 semantic / 31 physical / 30 staged rows, injects every staged allocation, gives coverage-only readback non-zero demand, and checks exact live topology, 64-payload compatibility sizing, over-limit observation, and lease rollback/stale settlement |
| `DirectSlotCapacityProvisioning.tla` (R-BACK-2.103–2.105 capacity/reuse refinement) | Bounded adjacent source shapes share persistent payload capacity across publish/reclaim generations; an empty reclaimed payload either reuses complete retained storage or takes the failure-atomic provision path, while incomplete detached owner storage and stale ledger credit fail closed. | `directSlotStorageTransition`, `directSlotEmptyStorageReusable`, `directSlotCapacityLedgerQualifiesReuse`, `restoreDirectSlotLookupTables`, typed `DirectSpanAdmissionWitness`, and the `ChunkSlot::detachResourceOwners` / typed-allocation `DetachedCompatibilityOwnerToken` round trip | boundary credits against a same-capacity serial reference, no populated growth, no redundant reprovision, complete-retention premise, exact ledger premise, and unchanged exact-fit lane | finite safety model; terminal stutter after rotation/provision/reuse | `tests/native/backend/direct_slot_provisioning_spec.cpp` binds the reducer and shared schema, all 31 physical coverage rows, all 30 staged fault rows, lookup restore/address stability, full-capacity admission-witness move/stale/ABA/wrong-issuer/double-consume, explicit receipt/token move/destroy/restore/abandon, ledger conservation, reclaim round trip, and stale-ledger rejection; `DirectSourceLifecycle` owns composed linear lifecycle failures |
| `DirectSourceLifecycle.tla` (Phase B composed Direct lifecycle) | Two exact Direct sources across two slots, retained-capacity reuse plus pre-admission staged/adopted rotation, separator/ordered-control modes, Present, effect cut, receipt/publication, encode/completion, and reducer-only detach/restore-or-poison/reclaim; semantic sibling spans acquire one behavioral physical credit owner at import, and the witness binds the final slot plus slot/source/storage generations. RawOwned import and planning are abstract model states; production observation begins at successful witness admission and separately tracks its FIFO row sequence and admission frontier. Pre-effect cancellation releases the Direct destination into `CompatibilityReady`; a distinct FIFO-gated compatibility settlement advances publication/reclaim order without fabricating Direct completion or restore. | `reduceDirectSourceLifecycle`, `reduceDirectSourceLifecycleBatch`, `CommandQueue::DirectChunkSlotReplayLease` observer edges, and `PipelineLifecycleObserver` projection of real admission, publication, Encoding, Completed, Detach, Restore/Poison, and Reclaim edges | exact import/publication/reclaim FIFO plus production-admission FIFO, exactly-once publication/witness use, tail-only pre-effect rollback row erasure/frontier restoration, no fallback after effect, completion-before-reclaim, retained+staged+detached conservation, restore-before-reclaim, atomic adoption and sibling reduction, generation freshness, no post-effect destination change, no dual slot owner, ordered Present | weak-fair lifecycle progress (`DirectSourceLifecycle.progress.cfg`) | `tests/native/backend/direct_source_lifecycle_spec.cpp`, `cpu_ready_production_routing_spec.cpp`, and `direct_chunk_slot_metal_oracle_spec.mm`; fourteen expected-failure configs are wired, including production-admission reorder, post-effect destination mutation, and phantom/leaked/duplicate credit controls. Production terminal events carry full locators and preflight the complete exactly-one-owner sibling batch against scratch reducer state; production pins cover compatibility-prefix physical ownership and rollback-to-compatibility publication, while poisoned detached tokens remain retained on mismatch. Native rollback truth tables reject non-tail cancellation without mutation and accept a complete tail-ordered sibling group atomically. Native lifecycle/queue cases and the observer-enabled concrete Metal pixel oracle pass. Observer-disabled cost is owned separately by the native source-contract pin. The complete 161-configuration TLC inventory also passes, including all fourteen Direct expected-failure configurations. |
...
and capacity wake remain bound by the existing owner transitions. The bounded
production projection begins at successful `DirectSpanAdmissionWitness`
consumption; `ImportRaw` and `Plan` remain abstract model states and are not
reported as runtime events. From admission onward, the owner trace binds the
destination receipt, Tape-sealed publication, Encode, Complete, Detach,
Restore-or-Poison, and Reclaim transitions with full locators and atomic sibling
reduction: the complete exactly-one-owner batch preflights against scratch
lifecycle state before either ledger or sink is exposed. Broader
resolved-sidecar contents, C++ atomic ordering, Wine/GPU
output, and promotion evidence remain layered requirements rather than missing
identity joins; address equality alone is not a `SourceLease` transition.
