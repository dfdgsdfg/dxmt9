...
|---|---|---|
| **PREREQUISITE: encode-side versioned byte readers snapshot-sourced (R-BACK-44.4a)** | implemented (2026-08-25); clean-machine runtime A/B still owed | The encode index staging call now carries the `!indexSnapshot` guard mirroring the vertex path, the two trace-gated diagnostic index readers source bytes snapshot-first, and `StreamIbStagingCache::findOrStage` asserts the no-versioned-record invariant (`dxmt9_draw_encoder_draw.mm`, `dxmt9_encode_session_storage_internal.hpp`). Full audit of every `->shadow`/`->contents` site in the encoder/encode-session files found all remaining readers already snapshot-first. 19 encoder/hazard/session native specs pass. The pre-existing-race framing stands: the R-BACK-2.51(d) drain waits on `lastReplayedSeq` only and never covered encode-time reads. Runtime evidence debt: the first GT2 smoke ran concurrently with an unrelated heavy native-test workload on the host and is not a valid perf sample; a clean-machine GT2 A/B (vs `discard-range-off-gt2-r1`, harmonic `27.775`) plus GT1 visual remain owed before the offload mode builds on this. |
