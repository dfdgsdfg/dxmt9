...
| Parallel render-pass executor (`R-BACK-2.63`) | pass-local candidate extraction implemented / production static eligibility unresolved / real execution default-off | Parallel requests run the validated production planner. With perf observation enabled, a fixed value-owned producer scans the final effective replay order before effects and records up to 16 source-local pass candidates with two to 16 validated DrawRun children each. Clear, Present, partial Clear, and attachment changes delimit spans without entering children. Static eligibility is a separate step requiring proof-owner callbacks that canonicalize resource identities and resolve every draw to one fixed non-Unknown route, plus explicit coordinator proofs for a nonzero action epoch, query/update absence, capture inactivity, initializer independence, ordered-control absence, and sidecar-observation absence. Missing proofs retain candidate/sealed counts and emit precise fail-closed reasons; no bool can relabel raw identities canonical. Snapshots own only source-qualified locators and resolved values, with no borrowed payload/span/page/Metal state. The current production call site supplies pool-backed alias identity and route resolvers but cannot prove the coordinator facts at its pre-effect seam, so it publishes zero eligible passes and consumes the original plan serially. Proof-complete native fixtures pin Clear -> large DrawRun -> Present eligibility, active replay order, partial Clear epochs, multi-pass/attachment splitting, canonical alias hazard and resolver failure, unknown route, zero/mismatched epoch, unresolved coordinator facts, exact 16-pass capacity/overflow, 2..16 child bounds, unchanged serial coverage, and perf-off zero work. The fake-child executor continues to prove ownership, ordering, arbitrary join, exactly-once replay, pre-effect fallback, and post-effect fail-stop. Counters distinguish attempts, extracted candidates, boundary-sealed candidates, proof-complete eligible passes, and grouped rejections. The earlier 2026-08-11 GT2 whole-source shadow run remains negative evidence only and does not measure this producer. | Next plumbing gap: move the observation seam to, or add an immutable coordinator-owned pre-effect proof snapshot at, the point where the real action epoch and query/update/capture/initializer/ordered-control/sidecar facts are complete. Then add residency-pinned native-state re-resolution, the WMT parent/child adapter, and bounded worker pool. Cross-source/carried-session sealing, formal/refinement, Metal-backed visual/locality/failure evidence, and a fresh pass-local counter run remain missing. No promotion or performance improvement is claimed; the provider remains default-off and selects no Metal parallel work. |
