...
| Parallel render-pass executor (`R-BACK-2.63`) | production source-local WMT parent/child execution implemented / deterministic correctness ladder green / explicit opt-in | The post-preamble pre-effect seam publishes coordinator proofs and extracts up to 16 source-local logical passes with two to 16 ordered children. The producer keeps Clear/Present on the coordinator and owns source-qualified locators, canonical resource sets, route/action facts, and first-draw snapshots. Production re-resolves locators under the residency pin; WMT creates ordered children; a bounded executor joins before parent end and completion. The serial and child encoders now share an immutable VS/PS/FFP payload transition, each child retains its own previous identity, and the coordinator rejects an actual prefetched slot-30 table PSO before parent creation when the child emits direct Stage 1 bindings. `ParallelDrawBinding.tla`, native truth tables, fake-child tests, and a two-child Metal additive A→B→A readback (100 byte-equal iterations plus stale negative control) are green. Two earlier GT1 runs had exposed black polygons with the pre-fix child path, so the old wild claim remains withdrawn. | Keep `identity` as the default until GT1/GT2/GT3/SFIV are rerun opt-in and the prior GT1 artifact is absent. Resolve the existing 5.3% matched-GT2 throughput regression. Cross-source/carried-session sealing, UP/tile/table child ownership, and Metal 4 joint completion remain open. |
