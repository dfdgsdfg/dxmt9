...
| Final WSI session drain (`R-BACK-2.83`) | production terminal release and native end-to-end proof implemented behind the default-off Tape lane | `beginFinalWsiQuiescence` now fixes the final writer fence, requests the existing typed Shutdown release when the session coordinator is the selected worker, wakes it, waits for acknowledgement/submission and exact sequence completion, then requires Ready FIFO and Tape residency to reach zero. The native production-loop fixture holds an open session, publishes a later suffix ending in Present, proves the terminal latch blocks before release, supplies completion, and checks lease/residency/page/Ready zero plus final Present ordinal completion. A second Tape+DCE fixture pins the current fail-closed composition rule: Tape selects the session coordinator, so neither an unowned latch nor the uncomposed Direct-Arena DCE path is reachable. This binds the existing `CpuReadySessionProgress` shutdown trace to production while preserving the Tape-off DCE and compatibility lanes. | Native fixtures invoke the selected production session loop directly; the constructor-owned `startThreads` dispatch is code-audited but not independently pinned without a real Metal queue. Metal/Wine teardown and repeated wild reset/device-destroy evidence remain open; the default-off gate is unchanged. Explicit DCE+session composition remains future work. |
