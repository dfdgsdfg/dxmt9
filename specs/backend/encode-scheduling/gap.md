...
| EncodeSession semantic lifecycle (`R-BACK-2.42`–`2.49`, `R-BACK-2.65`) | bounded multi-source serial planning and replay plus fresh-frontier retained Ready-head lookahead implemented behind the default-off Direct gate / fresh wild locality gate pending | The coordinator snapshots at most eight already-Ready compatible sources, resolves and retains the complete selected prefix, builds one combined alias-resolved RAW/WAR/WAW graph seeded from the active render pass, and accepts only a complete DrawRun-only cross-source permutation. It preflights source ranges, maximal runs, FIFO completion capacity, admission, and carrier capacity before the first Metal effect. Replay uses source-qualified fragments while completion sources remain registered exactly once in natural FIFO order; post-effect failures are fail-stop. Only a validated reordered plan enters the exact fragment transaction; `NaturalAfterMerge`, no-merge, and rejected outcomes retain source-local fallback. Present may remain as the natural final source in the same session and command buffer after a planned pre-Present prefix. Exact `sourceOrdinal` and `seqId` adjacency identify the Ready prefix; optional `rawOrdinal` values are only monotonic bookkeeping because StateOnly or Legacy raw entries may interpose without publishing sources. Only at a fresh sessionless frontier may a sole compatible Ready head be retained pre-admission when an exact ordered-tail Writing successor exists. It is restored ahead of that successor before ordinary two-source planning, while ordered release, producer wait, initializer, stop, writer loss, or admission/writer pressure restores first and forces exact replay. An exact Ready successor wins over simultaneous pressure. An active session consumes a sole Ready head immediately. Native production-path evidence covers Legacy `A` plus Arena `B,A`, fresh retained Arena `A|B` plus Writing `A`, immediate active-session consumption, reordered fragment replay with FIFO completion and one carrier, StateOnly raw interposition, ordered SessionRelease fencing, retained-head stop/release progress, the eight-source bound with a ninth natural successor, and `B,A,Present`; lifecycle and optimizer suites cover rejection and attribution contracts. Exact encoder attachment, hazard, initializer, and tile checks remain authoritative. Session storage now exposes a typed replay-frontier state: a clean closed encoder may accept a valid same-live-set moved head without an active seed, while pending clear, active blit, unproved active render, and sessionless injection retain natural order. Native lifecycle and queue fixtures cover fresh and prior-work clean sessions, ordered ClosePass on one command buffer, pending-clear command provenance, injected fallback, FIFO completion, and finalization. Composite planning now reserves `TentativeRepresented`, releases scheduling for planner/resource work, exactly revalidates before commit or exact Ready restore, and invokes the transaction observer post-effect exactly once with scheduling released. The pure lifecycle fixture carries one active encoder across more than eight manually injected sources, and the production fixture proves an eight-source planning bound with a ninth natural successor; neither currently proves logical-pass actions across that exact production window edge. | Add a production 8+1 window-edge fixture that keeps one compatible render pass open across reselection or park/wake on the same session and command buffer, asserts no intermediate pass/action/sidecar/completion publication, resolves any provisional Store exactly once at a later semantic command, and preserves FIFO completion and reclaim. Re-run strict GT2 tape-off/on locality first, then GT1, GT3, SFIV and repeated-run promotion evidence. Explicit Flush, pressure-fallback-specific native injection, and multi-segment Present end-to-end evidence remain. The existing `CpuReadySessionProgress.tla` proves tentative restore and pressure/release safety but does not yet model the exact retained-Writing identity park as a separate liveness branch. Continuity after encoder end or command-buffer commit remains Metal 4 work under `R-BACK-2.64`. Keep the gate default off. |
