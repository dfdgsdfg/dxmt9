...
|---|---|---|
| `TransactionalChunkSlotAssembler` (`R-BACK-2.85`) | A bounded Arena payload transaction now wraps `ArenaSourcePayloadBuilder`: failed construction is sticky, rollback destroys constructed prefixes, and publication occurs only through `commit`. Production single- and batch-segment build contexts now bind a value-only outer witness carrying raw/source/sequence/build, source/page generations, control index, segment extent, and planned bytes. The `CpuReadyArenaBuildLease` still owns page/control/sequence/resource obligations, and the witness cannot verify live lease state or resource-retain counts by itself. | Bind the outer lease and resource retains to the same transaction/evidence, prove commit infallibility after successful build, inject every capacity/validation/publication failure, and compare pre-effect rollback versus post-effect fail-stop. |
| Arena direct replay to `ChunkSlot` (`R-BACK-2.86`–`2.87`) | Eligible Arena draw records now call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`; compatibility, inactive, and trace-capture lanes retain the carrier. A native A-B-A fixture compares final draw state/layout/params/payload bytes with the carrier path, and now appends an ordered mixed Clear/copy/Readback/Present suffix through the same transactional assembler APIs; partial-construction rollback remains covered. | The differential still does not start from the same validated raw chunk and does not yet compare original raw ordinals, resource retention, barriers/readback effects, injected failure disposition, or completion identity. GPU readback, bounded Wine faults, and wild promotion are also open; the ordinary Direct gate remains default off. |
| Pipeline ownership/progress observer (`R-BACK-2.88`) | `CpuPipelineOwnership.tla` and `dxmt9_pipeline_lifecycle.hpp` implement the bounded abstract model and reducer; QueueLifecycleController now emits bounded exact-identity owner evidence, and the native completion-source fixture drives publication/dequeue/inline-finish through real Tape/CV methods. | Extend the sink to PE/import/receipt/selected-parallel/device-loss detail and add deterministic injected missing-wake, partial-publication, premature-reclaim, and completion-before-join cases at those actual owners. Until then the test does not satisfy R-VERIF-7.6's full counterexample coverage clause. The 2026-08-30 absolute-path retry enabled this observer for 3DMark05 GT2 and retained completion span/occupancy evidence; the final child-current 3DMark06 GT2 retry now also records 1,860 valid presents and clean chunk/no-pipeline/shader-reject counters, but is timeout-allowed and has no benchmark-owned `.3dr`; STALKER remains blocked at WSI `layer_acquisition=unavailable` before render. End-to-end owner coverage and promotion remain open. See `experiments/output/current-head-attribution-20260830/measurement-retry.md`. |
| Typed replay/encode borrows (`R-BACK-2.89`) | The ordinary Unix queue encode API now passes a non-copyable/non-movable `GenerationQualifiedSourceBorrow` plus a trivially-copyable `WorkerOwnedSourceSnapshot`; the callback receives the borrow only while the represented Tape source is pinned, and cross-thread state can carry only the value snapshot. Native compile-time checks pin non-copyability and the queue fixture checks exact source/storage/sequence generations. DCE/session/partition lookahead and FrameGraph option spans still use `ResolvedPublishedSource` call-local views by convention, so no uniform capability yet prevents an asynchronous closure or observer from retaining those resolved spans. | Inventory every replay, serial, parallel-child, observer, and completion callback; extend the capability boundary to lookahead/session/parallel APIs; add stale-generation, reclaim-during-borrow, callback-escape, and exact-conservation negatives before claiming end-to-end type-level closure. |
