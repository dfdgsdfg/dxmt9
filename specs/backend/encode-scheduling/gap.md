...
| Stable render scheduling provider modes (`R-BACK-2.66`) | contract defined / canonical resolver and selectors not implemented | The current runtime exposes only `DXMT9_CPU_READY_TAPE`, which selects the source-delivery axis while production partition execution remains identity-serial and Metal 4 is absent. The partition interface, serial identity/explicit native fixtures, and default-off Tape fixtures provide implementation prerequisites. | Add one queue-immutable `RenderSchedulingProviderConfig`, canonical source/partition/segment selector parsing with legacy Tape alias precedence, requested/resolved startup and perf reporting, source-kind-neutral explicit serial planning, and mode-matrix native coverage. Keep the selectors out of the descriptive environment-variable rules until the runtime honors them. Parallel and Metal 4 implementation remain owned by their rows below. |
