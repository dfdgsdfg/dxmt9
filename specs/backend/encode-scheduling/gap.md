...
| Arena/ordinary direct replay to final storage (`R-BACK-2.86`–`2.87`) | Eligible Arena and ordinary synchronous draw records call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`. Ordinary replay does not require `DXMT9_CPU_READY_TAPE`: a pure whole-raw classifier admits one validated logical source, including a multi-segment aggregate layout, into a fresh writing `ChunkSlot`, preserving Legacy source/CB/pass cadence. Draw → APPLY_STATE → Draw is pinned carrier-free. Ordinary production selection follows one default-on policy constant plus `DXMT9_DIRECT_CHUNK_SLOT_REPLAY`, is disabled under `DXMT_TRACE_RENDER`, and retains exact `0` as the stable rollback. Oversized sources now have one additional carrier-free envelope: exactly one source containing at least one non-UP Draw and only Draw/APPLY_STATE/constants may use `DirectOversized`; Present, Clear/copy/fill/resolve/mipmap, ordered controls, UP, capture/trace, multi-source, populated destinations, and an active build remain typed pre-effect Legacy/reject exclusions. The source uses the existing immutable aggregate capacity/layout and one generation-qualified completion identity; no queue hot record or PE producer changes. Existing ordinary and Present-tail routes remain unchanged. Native planning/differential/lifecycle tests pin exact raw order, locators, retain/completion identity, fresh-slot admission, rollback, and the `DirectOversized` disposition. Exact telemetry is `direct_chunkslot_replay_direct_oversized_{raws,records,wire_bytes}` plus the shared `direct_chunkslot_replay_outcome_{not_attempted,import_rejected,plan_rejected,begin_legacy_pre_effect_failure,begin_fail_stopped,replay_failed,commit_failed,committed}`; residual populations remain `legacy_oversized`, `legacy_segmented`, `legacy_up_draw`, `legacy_present`, `legacy_capture_or_trace`, ordered Inline, and DrawRun/Arena carriers. The replay worker remains for PE/Unix overlap, but the target serial provider performs direct cursor Replay and Metal encode on that same worker rather than handing a final Arena to a second encode thread. The current owner ledger measures the compatibility carrier at `15.852 MB/Present` and `0.643 ms/Present` in GT2; this is a removable materialization class, not a promise that the same time lies on the frame critical path. | Extend carrier-free direct-cursor consumption through the highest-frequency residual Draw/APPLY_STATE families while retaining explicit fail-closed dispositions for ordered control, UP, capture, and unsupported resource operations. Add fused-worker lifecycle/progress and Metal-owner evidence. Broader Wine allocation/retain/bridge/capture fault injection, oversized direct wild visual/locality, and WSI evidence remain open. CPU-ready Tape is independently default off. |
...
| End-to-end source composition, carrier retirement, and materialization floor (`R-BACK-2.95`–`2.100`, `R-ARCH-7.11`–`7.24`) | Production PE descriptors carry a closed event/source interval through authenticated Raw storage, Arena or compatibility publication, queue residency generations, synchronous replay/encode borrows, early payload retirement, completion, and reclaim. Import closes asynchronous ownership through one complete Unix `RawOwned` copy; replay queues move leases rather than source bytes. Every source now owns one fixed-capacity sparse state checkpoint and one shared pure transaction phase relation. Compatibility, ordinary Direct, and Arena replay preserve exact record order, restore state plus private destination before the effect cut, require a destination receipt before state commit, and fail-stop without retry after an ordered-control/command/publication effect. The bounded TLA model and native truth table include A→B→A, exact attribution, stale generation, optimizer isolation/proof, partial publication, and retry-after-effect negative controls. `DrawRunSubmission`, queue-wide final SoA, and the Replay-worker-to-command-queue-encode-thread handoff remain current compatibility representations. | Extend the universal fused replay/encode direct cursor across segmented/oversized/UP/trace/capture and mixed controls, then delete the carrier, complete final-SoA handoff, and dedicated serial encode thread. Add the complete Legacy/direct effective-stream, next-state, final-SoA, resource/completion, and failure differential. Limit compact indexed SoA construction and a separate encode coordinator to an already accepted explicit-parallel pass. Re-run the owner-qualified ledger and prove only the default-serial R-ARCH-7.24 materialization floor remains. Add C++ atomic, Wine/GPU, wild, and sidecar audit evidence. |
...
| Exclusive encode execution provider (`R-BACK-2.66`, `R-BACK-42.13`–`42.18`) | target policy specified / current mechanisms and thread topology differ | Device creation currently resolves `DXMT9_RENDER_PARTITION_MODE=identity|serial|parallel`; Tape and segmentation have separate controls. Every production lane still uses a Replay offload worker followed by a distinct command-queue encode thread. Identity is the runtime default and explicit parallel adds child workers behind typed pre-effect serial fallback. | Implement one queue-immutable `serial-direct|long-session-direct|parallel-compact-soa` resolver. Direct modes fuse Replay and serial Metal encode on one Unix worker with no downstream encode thread. Only the compact-SoA mode retains a Replay-to-encode coordinator boundary and bounded children. Add requested/resolved topology, fallback, and differential mode-matrix evidence. |
