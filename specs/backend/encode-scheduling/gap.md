...
| Pipeline ownership/progress observer (`R-BACK-2.88`) | `CpuPipelineOwnership.tla` implements a bounded ownership model, but its `CompleteGpu` action still combines `Completed` and `Reclaimed`; `dxmt9_pipeline_lifecycle.hpp` has the distinct concrete stages. `QueueLifecycleController` emits bounded exact-identity evidence, and the native completion-source fixture drives publication/dequeue/inline-finish through real Tape/CV methods. The inline fixture currently expands one zero-command-buffer transition into fabricated `Encoding -> GPUInFlight -> Completed` milestones, contrary to R-BACK-2.88's explicit no-GPU terminal-row rule, so this is partial diagnostic evidence rather than a model-code refinement proof. | Split completion from reclaim in `CpuPipelineOwnership.tla`; emit a real state-only/zero-GPU terminal disposition from the production inline owner; then extend the sink to PE/import/receipt/selected-parallel/device-loss detail and add deterministic injected missing-wake, partial-publication, premature-reclaim, and completion-before-join cases at those actual owners. Until then the test does not satisfy R-VERIF-7.6's model-code isomorphism or full counterexample coverage clauses. The 2026-08-30 absolute-path retry enabled this observer for 3DMark05 GT2 and retained completion span/occupancy evidence; the final child-current 3DMark06 GT2 retry now also records 1,860 valid presents and clean chunk/no-pipeline/shader-reject counters, but is timeout-allowed and has no benchmark-owned `.3dr`; STALKER remains blocked at WSI `layer_acquisition=unavailable` before render. End-to-end owner coverage and promotion remain open. See `experiments/output/current-head-attribution-20260830/measurement-retry.md`. |
| Typed replay/encode borrows (`R-BACK-2.89`) | The ordinary Unix queue encode API now passes a non-copyable/non-movable `GenerationQualifiedSourceBorrow` plus a trivially-copyable `WorkerOwnedSourceSnapshot`. FrameGraph planning and observation receive only queue-issued `SynchronousSourceBorrowBatch` values: construction is private, issuance freezes and prevalidates exact source/storage generations while holding the queue mutex, and an active epoch pins those locators against reclaim. Unlocked payload/command projections check only the atomic epoch and frozen identity, never the queue-owned Tape; the caller relocks and revalidates every exact Tape generation before deactivation. The visitor and Ready-prefix selector paths use allocation-free templates/thunks. `GenerationQualifiedSourceBorrow` no longer returns a `SourcePayloadView`; its checked `visitPayload` callback receives the noncopyable `SynchronousSourcePayloadBorrow`, whose public surface is limited to checked synchronous projections. Native pins reject public/default/forged construction and capability copying, and cover nested reentry, concurrent reclaim rejection, post-callback generation mismatch, and stale-after-reclaim rejection. This is runtime protocol enforcement, not a claim that C++ can prevent deliberately retaining an address obtained from a returned command/span projection; such escape remains a contract violation and undefined lifetime use. DCE/session/partition lookahead option spans remain private synchronous call-local views, so their helper audit and selected-child/completion callsites remain open. | Keep the inventory pinned across replay, serial, DCE/lookahead, session, partition, selected-parallel, observer, and completion callbacks; remove the remaining raw lookahead spans or bind them to the same epoch before claiming end-to-end type-level closure. Add production PE/import/receipt/device-loss owner evidence. |
