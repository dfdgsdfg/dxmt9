...
| End-to-end source composition, carrier retirement, and materialization floor (`R-BACK-2.95`–`2.100`, `R-ARCH-7.11`–`7.24`) | Production PE descriptors carry a closed event/source interval through authenticated Raw storage, Arena or compatibility publication, queue residency generations, synchronous replay/encode borrows, early payload retirement, completion, and reclaim. Import closes asynchronous ownership through one complete Unix `RawOwned` copy; replay queues move leases rather than source bytes. Replay-core and optional optimizer policy are separate stages. `DrawRunSubmission`, queue-wide final SoA, and the Replay-worker-to-command-queue-encode-thread handoff remain current compatibility representations. | Implement the working-state transaction and universal fused replay/encode direct cursor across segmented/oversized/UP/trace/capture and mixed controls, then delete the carrier, complete final-SoA handoff, and dedicated serial encode thread. Limit compact indexed SoA construction and a separate encode coordinator to an already accepted explicit-parallel pass. Re-run the owner-qualified ledger and prove only the three default-serial R-ARCH-7.24 large materializations remain. Add topology/progress model-code binding, C++ atomic, Wine/GPU, wild, and sidecar audit evidence. |
