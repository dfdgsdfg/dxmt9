...
| Arena/ordinary direct replay to final storage (`R-BACK-2.86`–`2.87`) | Eligible Arena and ordinary synchronous draw records call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`. Ordinary replay does not require `DXMT9_CPU_READY_TAPE`: a pure whole-raw classifier admits one validated logical source, including a multi-segment aggregate layout, into a fresh writing `ChunkSlot`, preserving Legacy source/CB/pass cadence. Draw → APPLY_STATE → Draw is pinned carrier-free. Ordinary production selection follows one default-on policy constant plus `DXMT9_DIRECT_CHUNK_SLOT_REPLAY`, is disabled under `DXMT_TRACE_RENDER`, and retains exact `0` as the stable rollback. A narrow optional leading Clear plus non-UP Draw/APPLY_STATE/constants plus exactly one terminal Present shape is typed `DirectWithPresentTail`; coordinator-owned Present metadata/token is staged, appended inside `TransactionalChunkSlotAssembler` so evidence covers its resource closure, then the same slot is published and its boundary applied only after evidence commit, with deferred boundary drain before boundary application. Multiple logical sources, populated destinations, UP draws, ordered controls/readback, standalone/nonterminal/duplicate Present, capture/trace, oversize, structural reject, and pre-effect admission failure have explicit Legacy/Inline/reject dispositions; post-effect failure is fail-stop. Native tests pin classifier truth-table, direct Present payload capacity, and production differential cadence/completion; stable perf-report disposition/outcome names and units are documented, while drawable-token failure injection, exact runtime telemetry aggregation, and WSI failure evidence remain residual. | Broader Wine allocation/retain/bridge/capture fault injection remains useful residual evidence; the new direct-tail route still needs wild visual/locality evidence. CPU-ready Tape is independently default off. |
