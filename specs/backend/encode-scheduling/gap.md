...
| Parallel render-pass executor (`R-BACK-2.63`) | production source-local WMT parent/child execution implemented / correctness promotion blocked / explicit opt-in | The post-preamble pre-effect seam publishes coordinator proofs and extracts up to 16 source-local logical passes with two to 16 ordered children. The producer keeps Clear/Present on the coordinator and owns source-qualified locators, canonical resource sets, route/action facts, and first-draw snapshots. Production re-resolves locators under the residency pin; WMT creates ordered `MTLParallelRenderCommandEncoder` children; a bounded executor uses distinct child shadows and joins before parent end and completion. Native proof/failure specs and a Metal validation-layer fixture pass, but those layers did not cover production per-draw binding transitions. Two GT1 opt-in runs exposed intermittent large black polygons before the final scene while Metal/GPU error counters stayed zero; the earlier GT1/GT2/GT3/SFIV wild-smoke correctness claim is therefore withdrawn. Same-build 60-second GT2 had also measured identity versus parallel at 1,573 versus 1,490 Presents, unchanged CB/pass shape, `encode_chunk_cpu_ms/present` 19.08 versus 15.32, worker peak 8, and 3,022 batches/37,601 tasks. | Keep `identity` as the default. Before another promotion run, add the `R-VERIF-1.5`–`1.8` evidence chain: bounded serial-refinement model for child binding/order including `A -> B -> A`, model-to-code transition/isomorphism pins, and an alternating-state offscreen GPU readback. Then resolve the existing 5.3% matched-GT2 throughput regression. Cross-source/carried-session sealing and `R-VERIF-2.15` joint completion also remain open. |
