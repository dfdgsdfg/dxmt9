...
| End-to-end source composition (`R-BACK-2.95`, `R-ARCH-7.11`–`7.19`) | Individual production types cover PE immutable ownership, authenticated raw storage, queue residency generations, synchronous replay/encode borrows, direct final construction, early payload retirement, completion, and reclaim. They now share one normative identity/facade/sidecar vocabulary. | Bind the exact PE event to raw/source/seq/storage/completion identity in one production observer; audit every retained sidecar and cross-thread range; add R-VERIF-7.9 model/code counterexamples; retain typed compatibility dispositions for record families not admitted by direct projection. |
| Replay projection, carrier retirement, and materialization floor (`R-BACK-2.96`–`2.100`, `R-ARCH-7.20`–`7.24`) | The contract now separates synchronous PE borrows from asynchronous `RawOwned` ownership, replay-core projection from optional optimization policy, and the four allowed large materialization boundaries. `DrawRunSubmission` remains the current synchronous compatibility adapter. | Implement the working-state transaction and universal assembler across segmented/oversized/UP/trace/capture and mixed controls, then delete the carrier and its APIs/adapters/counters/tests. Re-run the owner-qualified ledger and prove that no queue/session/partition gather or final-region recopy remains and that Metal command/reference costs are accounted separately from GPU-visible writes. Add model/code, C++ atomic, Wine/GPU, wild, and sidecar audit evidence. |
