...
| Parallel render-pass executor (`R-BACK-2.63`) | production source-local WMT Stage 1/Stage 2b execution implemented / deterministic correctness ladder green / economics shadow-only / explicit opt-in | The post-preamble pre-effect seam publishes coordinator proofs and extracts up to 16 source-local logical passes with two to 16 ordered children. The producer keeps Clear/Present on the coordinator and owns source-qualified locators, canonical resource sets, route/action facts, and first-draw snapshots. Production re-resolves every locator and draw under the residency pin, proves one pass-wide Stage 1 or Stage 2b direct ABI, and rejects missing PSOs, slot-30 tables, resource arrays, mixed ABIs, PSO-rebuilding overrides, UP, and tile FFP before parent creation. WMT creates ordered children with independent all-dirty binding shadows; Stage 2b binds direct constant payloads at slots 0/3 without the queue argument encoder, table, or cache. A bounded executor joins before parent end and completion. `ParallelDrawBinding.tla`, native truth tables/classifier tests, fake-child tests, and a two-child Stage 2b Metal slots-0/3 additive A→B→A readback (100 byte-equal iterations plus stale negative control) are green. Perf-gated counters conserve selected Stage 1 plus Stage 2b passes, report exact binding rejects and Stage 2b volume, and classify child-size/transition economics without enforcing the result. Two earlier GT1 runs had exposed black polygons with the pre-fix child path, so the old wild claim remains withdrawn. | Keep `identity` as the default until GT1/GT2/GT3/SFIV are rerun opt-in and the prior GT1 artifact is absent. Run a matched GT2 identity/parallel pair and record Stage 1/Stage 2b selected passes, Stage 2b children/draws, all binding rejects, economics accept/reject reasons and volume, minimum-child and child-count buckets, PSO/uniform transitions, worker CPU/wall, throughput, and unchanged command-buffer/pass/tile shape. No speedup or promotion claim exists yet. Cross-source/carried-session sealing, UP/tile/table child ownership, and Metal 4 joint completion remain open. |
