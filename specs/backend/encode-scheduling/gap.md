...
|---|---|---|
| `TransactionalChunkSlotAssembler` (`R-BACK-2.85`) | The transaction now owns either a bounded Arena payload block or the ordinary queue-owned compatibility `ChunkSlot`. The latter checkpoints every final SoA/payload range, including the aggregate nested Clear rect prefix, and uniform locator; reserves the planner's complete upper bound before replay; enforces that bound on each typed append; transfers owned direct Clear rect vectors without post-reservation allocation; and rebuilds uniform lookup state on rollback. Production commits bind private move-only `ArenaCommitEvidence` to the exact raw/source/sequence/build/storage/control witness and the seq-qualified resource closure digest; pre-effect partial construction restores the exact prefix, while post-effect identity or append failure abandons rollback and fail-stops. | GPU-side readback and broader bounded Wine fault injection remain open. |
| Arena/ordinary direct replay to final storage (`R-BACK-2.86`–`2.87`) | Eligible Arena and ordinary synchronous draw records call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`. Ordinary replay does not require `DXMT9_CPU_READY_TAPE`: a pure whole-raw classifier admits only a one-segment eligible raw into a fresh writing `ChunkSlot`, preserves that slot in `Writing`, and therefore keeps the Legacy source/CB/pass cadence. Ordinary production selection is behind default-off `DXMT9_DIRECT_CHUNK_SLOT_REPLAY` and is also disabled under `DXMT_TRACE_RENDER`. Populated destinations, UP, ordered controls, Present, capture/trace, oversize, structural reject, and pre-effect admission failure have explicit Legacy/Inline/reject dispositions; post-effect failure is fail-stop. Native all-family tests compare final command order/fields, draw payload bytes, resource closure, stable vector locality, rollback, fail-stop, and completion digest against Legacy; these are implementation/failure-support evidence, not promotion evidence. | GPU readback and broader multi-workload wild equivalence remain open; keep the ordinary selector default off and retain Legacy as the default rollback lane. The CPU-ready Tape provider is independently default off. |
...
The ordinary fresh-`ChunkSlot` direct construction is an explicit opt-in
synchronous candidate; it changes no scheduling provider and falls back before
effects whenever the destination or raw family cannot satisfy the proof
envelope. `DXMT9_DIRECT_CHUNK_SLOT_REPLAY` defaults off and
