...
| Fixed-role `SegmentedTransportV1` and later `ExactFixed` CPU Tape emission (`R-BACK-2.90`–`2.94`) | opt-in bridge transport and bounded host owner/model binding implemented; production Tape roles/default promotion open | The PE/WoW64/Unix path preserves three fixed regions through synchronous validation and atomically adopts one typed Unix owner without a contiguous gather. Contiguous V2 remains the default and at-most-once fallback. The host immutable semantic owner freezes exact role extents, rejects non-canonical duplicate identities, plans checked final geometry, and emits a validated byte-identical 21-family ExactFixed extent. `tla/SegmentedTransportV1.tla` models the later CPU-ready Tape `ReserveRole -> AdoptRole -> EmitRole` transaction, with six independent failures for incomplete reservation, partial adoption, fallback-after-effect, lost checkpoint, partial ledger, and missing wake. | Bind PendingDelta/capture/resource settlement and real CPU-ready queue transitions to the immutable owner/state machine. Add Wine fault, GPU/readback, wild locality, and performance evidence before changing either `DXMT9_PE_SEGMENTED_TRANSPORT=0` or CPU-ready Tape defaults. |
| Arena/ordinary direct replay to final storage (`R-BACK-2.86`–`2.87`) | Eligible Arena and ordinary synchronous draw records call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`. Ordinary replay does not require `DXMT9_CPU_READY_TAPE`: a pure whole-raw classifier admits only a one-segment eligible raw into a fresh writing `ChunkSlot`, preserves that slot in `Writing`, and therefore keeps the Legacy source/CB/pass cadence. Ordinary production selection follows one default-on policy constant plus `DXMT9_DIRECT_CHUNK_SLOT_REPLAY`, is disabled under `DXMT_TRACE_RENDER`, and retains exact `0` as the stable rollback. Populated destinations, segmented raws, UP draws, ordered controls/readback, Present, capture/trace, oversize, structural reject, and pre-effect admission failure have explicit Legacy/Inline/reject dispositions; post-effect failure is fail-stop. Native same-immutable-raw tests compare ordinary Direct/Legacy mixed Draw/Clear/copy command cadence, draw payload bytes, resource closure, manual Present closure, completion identity/digest, pre-effect fallback, and post-effect no-retry; the companion whole-raw oracle pins Clear/copy/Present and the synchronous Readback Inline barrier. `dxmt9-direct-chunk-slot-metal-oracle-spec` runs the real production device and D9C replay in fresh env `0`/`1` processes, proves the Direct child reaches `commitDirectChunkSlotReplay`, and compares exact offscreen BGRA8 pixels plus command/resource/completion identity. | Promoted on 2026-08-31 after the same-build wild matrix and copy-ledger audit below. Exact `0` remains the permanent rollback. Broader Wine allocation/retain/bridge/capture fault injection remains useful residual evidence, not a claim made by this bounded promotion. The CPU-ready Tape provider is independently default off. |
| Pipeline ownership/progress observer (`R-BACK-2.88`) | `CpuPipelineOwnership.tla` now separates `GPUInFlight -> Completed -> Reclaimed`, and `dxmt9_pipeline_lifecycle.hpp` binds the queue-owned transition projection to shared owner/disposition predicates. The production `QueueLifecycleController` emits exact identity/generation evidence with `PeImport`, `Receipt`, `SelectedParallel`, `DeviceLoss`, or `Queue` owner qualification; its zero-command-buffer inline path emits `Encoding -> Reclaimed` with `NoGpuTerminal` and no fabricated GPU milestones. Native completion-source coverage drives real Tape/CV publication, dequeue, inline finish, and finish-waterline owners; lifecycle coverage includes deterministic missing-wake, reclaim-before-completion, skipped-completion, fabricated-GPU/no-GPU, partial-publication, and completion-before-join counterexamples. | PE/import, selected-parallel, receipt, and device-loss owner coverage is bounded to the existing observer abstractions; no new Metal behavior or ABI was added. Full asynchronous owner coverage, GPU/wild equivalence, and promotion remain open. |
