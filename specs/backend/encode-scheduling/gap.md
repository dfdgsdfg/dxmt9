...
| Composed end-to-end progress (`R-BACK-2.67`) | not started | Per-component temporal proofs exist (`CpuReadySessionProgress`, `SessionCapacityLease`, `ConcurrentProgressSignals`, `EncodeSessionCompletion`, `PostEncodePayloadRetirement`, all green under `dxmt9-verify-tla`) and the present-ordinal gate has an isomorphism pin. The 2026-08 tape lease/wake wedge is the motivating incident: each component was individually plausible while the composition parked forever. | Build the composition model spanning admission, lease/wake, publication, session continuation, completion release, and present pacing with leads-to and lost-wakeup-freedom properties under weak fairness; add isomorphism pins for the lease/wake and admission wait predicates; add runtime liveness watchdog counters distinguishing pending-obligation stall from idleness in no-gputrace wild runs. |
