...
| Arena/ordinary direct replay to final storage (`R-BACK-2.86`–`2.87`) | Eligible Arena and ordinary synchronous draw records call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`. Ordinary replay does not require `DXMT9_CPU_READY_TAPE`: a pure whole-raw classifier admits one validated logical source, including a multi-segment aggregate layout, into a fresh writing `ChunkSlot`, preserving Legacy source/CB/pass cadence. Draw → APPLY_STATE → Draw is pinned carrier-free. Ordinary production selection follows one default-on policy constant plus `DXMT9_DIRECT_CHUNK_SLOT_REPLAY`, is disabled under `DXMT_TRACE_RENDER`, and retains exact `0` as the stable rollback. Multiple logical sources, populated destinations, UP draws, ordered controls/readback, Present, capture/trace, oversize, structural reject, and pre-effect admission failure have explicit Legacy/Inline/reject dispositions; post-effect failure is fail-stop. Native tests separately pin single-source aggregate Direct and the same raw plus Present or multi-source segmentation as Legacy. | The existing ordinary promotion and permanent rollback remain unchanged. Broader Wine allocation/retain/bridge/capture fault injection remains useful residual evidence. CPU-ready Tape is independently default off. |
