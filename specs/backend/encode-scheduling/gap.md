...
| Parallel render-pass executor (`R-BACK-2.63`) | conservative production sealing shadow implemented / real execution default-off | Parallel requests run the validated production planner. With perf observation enabled, a fixed value-owned producer recognizes a complete fresh source-local pass containing two to sixteen DrawRun children plus an optional final Present. It rejects carried/deferred session boundaries, coordinator commands, capacity excess, attachment changes, attachment read hazards, and incomplete first-draw locators before effects. The snapshot stores no borrowed payload/span/page/Metal state. Production records eligibility and then typed `ParallelEncoderUnavailable` before consuming the original ranges serially. Native coverage pins sealing, final-Present exclusion from children, locator/attachment/hazard/capacity failures, and full first-draw validation. The existing fake-child executor proves ordered creation, arbitrary completion join, distinct child-local shadows, command/draw once, coordinator-only actions/sidecars/completion, join-before-parent-end, pre-effect fallback, and post-effect fail-stop. Enabled counters expose snapshot attempts, sealed/eligible passes, child/draw volume, grouped sealing rejections, execution consideration/eligibility/selection, and typed execution fallback. A 2026-08-11 no-gputrace GT2 shadow run completed 1,142 Presents with zero GPU command-buffer errors. It observed 1,142 attempts: 903 had no explicit subdivision plan and all 239 explicit-plan sources rejected on a coordinator command, yielding zero sealed or eligible snapshots. The artifact is `experiments/output/app-d3d9-3dmark05-sealed-pass-shadow-gt2-20260811/result.json`; the staged unix provider hash is `008b3c647349940f`. This is decisive evidence that whole-source sealing is only a lower-bound safety shape, not a useful estimator of pass-level parallelism. | Add a pass-local producer over the effective replay stream before interpreting eligibility: it must identify exact logical-pass begin/end, keep coordinator commands outside children, and preserve pass-action ownership. Extend sealing to proven Clear-start, carried-session, cross-source and multi-window logical passes; define a re-resolved complete native-state snapshot and residency pin. Then implement the real WMT `MTLParallelRenderCommandEncoder` parent/child adapter and bounded worker pool, plus formal/refinement and Metal-backed visual/locality/failure evidence. The provider remains default-off and currently selects no Metal parallel work. |
