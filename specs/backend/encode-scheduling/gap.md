...
| Parallel render-pass executor (`R-BACK-2.63`) | production source-local WMT Stage 1/Stage 2b execution implemented / economics enforced pre-effect / deterministic correctness ladder green / explicit opt-in | The sealed-pass builder now uses the existing 64-draw eligibility quantum independently of the serial planner: it chooses at most `floor(totalDraws / 64)` children within two to 16, evenly covers a single DrawRun, reduces complete-command groups until every child has at least 64 draws, and otherwise keeps the pass serial. Production re-resolves every locator, draw, ABI, PSO, uniform payload, route, and resource identity under the residency pin, then enforces the pure economics classifier before render-pass preparation or any parent/child Metal effect. Rejection returns to exact serial replay; accepted passes retain the existing coordinator ownership of Clear/Present, actions, sidecars, completion, and command-buffer state. Exact production counters conserve `considered = accepted + serial_fallback`, with typed reasons summing to fallback, while the binding, Stage 2b, child-size, transition, and worker families remain attributable. `ParallelDrawBinding.tla`, pure subdivision/classifier/accounting tests, fake serial-fallback replay, child executor tests, and the Stage 2b Metal A-B-A readback are green. The matched GT2 identity/parallel pair measured 21.087975 versus 19.729740 fps (-6.44%) with conserved CB/pass/tile shape; encode stage wall fell 8.9%, but summed `encode_draw` rose 14.22 to 31.92 ms/Present and worker CPU was 26.91 ms/Present. All 2,670 selected passes were safe Stage 2b, and the prior shadow classified all 33,244 children / 1,017,361 draws as `thin_child` (12.45 children/pass, 30.60 draws/child), with zero GPU/binding errors. | Keep `identity` as the default: the matched result is evidence for the enforced gate and coarser children, not a speedup or promotion claim. Fresh opt-in GT1/GT2/GT3/SFIV visual/locality/performance evidence remains required after this gate; the earlier pre-fix GT1 black-polygon artifact is not retracted by deterministic coverage. Cross-source/carried-session sealing, UP/tile/table child ownership, and Metal 4 joint completion remain open. |
