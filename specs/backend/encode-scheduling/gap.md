...
| Parallel render-pass executor (`R-BACK-2.63`) | production source-local WMT parent/child execution and bounded worker dispatch implemented / explicit opt-in | The post-preamble pre-effect seam publishes complete coordinator proofs and extracts up to 16 source-local logical passes with two to 16 ordered children. The producer derives contiguous complete-command spans independently of the explicit-serial subdivision, keeps Clear/Present on the coordinator, and owns only source-qualified locators, canonical resource sets, route/action facts, and first-draw snapshots. Immediately before effects, production re-resolves every locator under the source residency pin and rejects attachment/route drift, UP payloads, tile FFP, active sidecars, incomplete late Store actions, carried fragments, and unsupported split policy. WMT creates the ordered `MTLParallelRenderCommandEncoder` children; a persistent concurrent executor dispatches at most 16 tasks, each with a distinct binding/uniform/texture shadow, and the coordinator joins children before ending the parent and publishing pass actions/completion. Stage 2 serial candidates use an explicitly counted Stage 1 child variant instead of sharing the queue-owned argument encoder. Native proof/failure specs and a Metal validation-layer fixture pass. Opt-in wild smokes pass with zero GPU errors on GT1, GT2, GT3, and SFIV; SFIV selects zero children because its route is ineligible. Same-build 60-second GT2 measured identity versus parallel at 1,573 versus 1,490 Presents, unchanged CB/pass shape, `encode_chunk_cpu_ms/present` 19.08 versus 15.32, worker peak 8, and 3,022 batches/37,601 tasks. | Keep `identity` as the default: the parallel lane reduces coordinator encode wall time but regresses the matched GT2 end-to-end throughput by 5.3% and nearly doubles summed draw CPU. Add child-local Stage 2 argument-buffer packets, UP preupload ownership, tile-FFP and diagnostic-sidecar policy only if profiles justify them. Cross-source/carried-session sealing, repeated visual/pixel evidence, and `R-VERIF-2.15` parallel completion refinement remain open. |
