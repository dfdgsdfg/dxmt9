...
| Arena/ordinary direct replay to final storage (`R-BACK-2.86`–`2.87`) | Eligible Arena and ordinary synchronous draw records call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`. Ordinary replay does not require `DXMT9_CPU_READY_TAPE`: a pure whole-raw classifier admits one validated logical source, including a multi-segment aggregate layout, into a fresh writing `ChunkSlot`, preserving source/CB/pass cadence. Draw → APPLY_STATE → Draw is pinned carrier-free. Oversized sources retain the bounded non-UP Draw/APPLY_STATE/constants Direct envelope; Present, Clear/copy/fill/resolve/mipmap, ordered controls, UP, capture/trace, multi-source, populated destinations, and an active build remain typed pre-effect exclusions. Those dispositions now use synchronous borrowed-state ingress into ordinary final storage when they emit draws; no DrawRun/Arena compatibility carrier remains. The replay worker remains for PE/Unix overlap, but the target serial provider performs direct cursor Replay and Metal encode on that same worker rather than handing final queue storage to a second encode thread. The prior owner ledger measured the now-retired carrier at `15.852 MB/Present` and `0.643 ms/Present`; this is historical attribution, not current evidence or a critical-path claim. | Complete fused-worker lifecycle/progress and Metal-owner evidence while retaining explicit fail-closed ordered-control, UP, capture, and unsupported-resource dispositions. Re-run the owner-qualified ledger and ordinary/direct differential after carrier retirement. Broader Wine allocation/retain/bridge/capture fault injection, wild visual/locality, and WSI evidence remain open. CPU-ready Tape is independently default off. |
