...
| Promotion gates (`R-BACK-2.50`) | partial / current GT2 locality gate failed | Permanent counters now split retirement attempts/success/ineligibility, receipt failures/depth, physical residency released, encoded-work cap closes, and GPU-outstanding current/peak from existing wait, command-buffer, pass, load/store, and tile shape. The Tape gate remains default off and the legacy payload-owning path remains the rollback baseline. The 2026-08-30 triangle smoke and GT2 off/on pair are progress-, visual-, and error-safe; sampled FPS changes only `+0.78%`, while CB/pass/tile all increase. | Keep the gate default off. Close retained-suffix locality first, rerun the strict same-build GT2 comparison, and only then run GT1, GT3, SFIV, repeated-noise, and conservation gates. See `docs/perfomance/present-pacing/present-pacing-cpu-ready-tape-promotion-gate.237.md`. |
