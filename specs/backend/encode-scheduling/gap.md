...
| Arena/ordinary direct replay to final storage (`R-BACK-2.86`–`2.87`) | Eligible Arena and ordinary synchronous draw records call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`. Ordinary replay does not require `DXMT9_CPU_READY_TAPE`: a source-wide emission plan describes direct draw/state islands, typed coordinator locators, ordered-control cuts, and compatibility ranges. Production executes only the rotation-free subset whose first executable span is its sole Direct lease; multi-lease and ordinary-prefix plans fall back whole before effects. Draw → APPLY_STATE → Draw, Clear, a lease-trailing single Present, and supported copy/fill/resolve/mipmap coordinators remain carrier-free when they fit that subset. Capacity rejection never publishes the populated prefix. A non-trailing or duplicate Present remains a whole-raw typed pre-effect fallback, as do capture/trace, invalid, and overflowing plans. No DrawRun/Arena compatibility carrier remains. The stable provider publishes the completed `OwnedRawFinalChunkSlot` to the dedicated encode thread; a fused direct cursor remains an optional future placement. The prior owner ledger measured the now-retired carrier at `15.852 MB/Present` and `0.643 ms/Present`; this is historical attribution, not current evidence or a critical-path claim. | Prove a complete incoming first-draw/resource witness and a typed forced-capacity boundary before reconsidering rotation or multi-lease production execution. Add matched wild visual/locality/performance evidence for the rotation-free subset while retaining explicit fail-closed capture, invalid, overflow, and post-effect failure dispositions. Fused-worker lifecycle/progress and broader Wine allocation/retain/bridge/capture fault injection, WSI evidence, and the ordered-control end-to-end differential remain separate gaps. CPU-ready Tape is independently default off. |
...
### Source-range lease plan and rotation-free production subset (`R-BACK-2.101`/`R-BACK-2.102`, 2026-09-02)
...
| Production differential | `dxmt9-cpu-ready-production-routing-spec` -- a compatibility-cut raw against the Legacy lane, comparing the **concatenated per-command `effectiveCommandDigest` sequence** (ordered command kind, draw parameters, binding-override and binding-snapshot payload bytes, coordinator descriptors), not a command total; plus a populated identity-qualified slot followed by a fail-closed `Direct -> TriangleFan -> Direct` whole-raw fallback, the Present-tail and coordinator-bearing spans, both capacity-rejection fallbacks without prefix publication, and the non-terminal/duplicate Present whole-raw fallbacks with their published source order |
...
slot generation, an eventually safe capacity-rotation transition,
ordered-control session disposition,
...
2. **Rotation changed chunk cadence and rendering, so production rotation is disabled
   ...
   A later 2026-09-03 GT1 correctness isolation made the stronger failure
   observable: Direct with replay offload enabled and disabled produced the
   same dark, flickering, repeated geometry; whole-raw Direct-off was normal;
   and suppressing populated-slot capacity rotation restored normal rendering.
   The diagnostic no-rotation run also conservatively demoted multi-span and
   coordinator-bearing raws, so it isolates rotation but is not by itself a
   promotion run for the final admission subset. Production now passes
   `allowRotation = false` and admits only plans whose first executable span is
   their sole Direct lease. Multi-lease or ordinary-prefix plans and every
   capacity rejection fall back whole before effects. Native production pins
   require that the populated prefix remain unpublished.

   The final rotation-free production spot check is
   `app-d3d9-3dmark05-gt1-rotation-free-production-r1-20260903`. It reached 460
   Presents with 459 committed Direct leases, zero capacity rotation or
   rotation publication, zero GPU command-buffer errors, zero chunk rejects,
   and zero separator fail-stops; supervised rendering was normal. The harness
   result is `fail` only because automated capture was not recovered, so this
   is supervised correctness evidence rather than an automated pixel oracle or
   a promotion/performance run. `replay_span_soa_growth_after_reserve=26`
   remains a separate final-capacity-ledger gap and is not claimed closed by
   the visual result.
...
The ordinary Unix path now performs an allocation-free whole-range gate before
the Arena planner. A range containing only non-UP Draw and APPLY_STATE/constant
records receives one exact final-slot SoA count/reserve plan and one
transactional `ChunkSlot` emission; state transitions still execute through
the existing source-wide replay transaction. Valid Present, ordered-control,
resource-mutation, UP/TriangleFan, and mixed-boundary ranges remain on the
established typed coordinator/compatibility dispositions, while malformed
imported ranges fail closed before effects. Perf output distinguishes ranges
rejected by the cheap gate from a post-materialization rollback fallback.
Focused native gate coverage is present; full final-slot/direct effective-stream
and next-state differential, owner-qualified ledger reconciliation, and
Wine/GPU/wild evidence remain open.

The carrier-free statement above is limited to eligible Direct whole-range
replay. Compatibility-only or excluded ranges remain compatibility-owned, and
are not covered by that carrier-free claim.

The ordinary fresh-`ChunkSlot` direct construction is the default synchronous
representation for eligible whole raws; it
changes no scheduling provider and falls back before effects whenever the
destination or raw family cannot satisfy the proof envelope. The single backend
policy constant is on, exact `DXMT9_DIRECT_CHUNK_SLOT_REPLAY=0` is the
permanent rollback, and
`DXMT_TRACE_RENDER` overrides it off. CPU-ready Tape and parallel scheduling
remain separately configured providers and retain their existing promotion
requirements.

The completed supervised wild matrix is a same-build, fresh-process Legacy
(`DXMT9_DIRECT_CHUNK_SLOT_REPLAY=0`) versus Direct (`=1`) comparison across
3DMark05 GT1/GT2/GT3 and SFIV on 2026-08-31. Every run passed with
zero GPU command-buffer/chunk errors. CB/Present remained `3.99968/3.99968`,
`3.99943/3.99942`, `3.99958/3.99956`, and `3.92708/3.92875`; Pass/Present
remained `10.6806/10.6953`, `15.7594/15.7659`, `15.0137/14.9976`, and
`22.8029/22.8213` for Legacy/Direct respectively. The corrected copy ledger
shows the Direct lane is non-vacuous only in GT2 (41 final appends); the other
three workloads select typed Legacy fallbacks. GT2 carrier bytes are
`15,988,369` versus `15,848,585` bytes/Present, but the different timed frame
populations make that delta attribution-only, not an FPS claim. The
deterministic Metal readback oracle provides the exact route/pixel identity
...
### GT2 identity-v2 r15 changed-head progress counterexample

`experiments/output/render-tape-gt2-identity-v2-r15-post-progress-fix/r15-console.log`
confirms that the r14 fixes removed the batch-wait spin and wrap-shape false
ineligibility: admission held at one live waiter with 122 enters / 121 retry
exits, and both ordinary/high-water ineligibility counters stayed zero. It then
records a stable greater-than-30-second wedge after capture reserve began:
first-lease seq/source 7204 at unchanged generation 14416, replay raw one in
ArenaAdmission, pending publication 7205..7234, pressure-serial 113, and
credit-exhausted 42. The production audit confirmed that the escape token was
keyed only to capacity generation, so FIFO head advance could not rearm finite
progress for a SegmentSerial batch needing more than one control drain. The
implemented policy instead permits at most one standalone escape per exact
denied Ready-head identity; distinct FIFO heads may progress under one
generation, while identity, Present, non-Arena, ordinary/high-water, ownership,
and release rules remain unchanged. Deterministic native composition and the
`EncodeSchedulingProgress` refinement require at least two distinct head drains
before the complete real batch-control predicate becomes Ready. This is a
...
| EncodeSession semantic lifecycle (`R-BACK-2.42`–`2.49`, `R-BACK-2.65`) | bounded multi-source serial planning, fresh- and active-frontier Ready-head lookahead, exact replay-FIFO next-source intent, and exact fresh/carried deferred terminal-suffix join implemented behind the default-off Direct/Tape gate / GT2 locality gate failed | The coordinator bounds ordinary Ready planning to eight sources, preserves natural FIFO completion, and carries one active pass across the proven 8+1 window edge. Whole-head lookahead accepts either the exact ordered-tail Writing successor or a queue-local intent. The replay FIFO exposes only an already-adopted immediate planning raw; successful Direct semantic replay stages its ordinal, and Ready publication atomically advances the generation-stamped predecessor/raw/expected-source identity. Empty FIFO and immediate control items never promise. A non-Direct plan cancels before effect, the pure wake algebra observes intent replacement, and Direct admission pressure restores the older head. The distinct fresh/carried source-local lane recognizes exact `DrawRun(A), Clear(B), DrawRun(B) | DrawRun(A)`, retains bounded value proof, and joins or drains with unchanged active seed, lease, completion, callback, and capture ownership. Native evidence covers empty startup, adjacent-raw/control hint shape, exact intent successor, non-source cancellation wake, stop, ordered release, pressure, active-seeded planning, command-once, FIFO completion, and zero final residency. `CpuReadyActiveHeadLookahead` now composes absent/adopted raw, Direct/non-source disposition, cancellation, hold/restore, seed preservation, and progress; the complete TLA suite is green. | H239 closes the next-source intent contract but rejects promotion. The GT2 worker armed 5,053 intents and canceled 310, yet intent-selected/successor counters were zero because the successor was already Ready whenever the promise could participate. CB/pass/tile still increased by `0.34%`/`1.15%`/`4.40%`. The remaining exact owner is bounded replay/store-action equivalence: 11,038 late unknown actions resolve as 268 Clear/DontCare and 10,770 Store. A final-Present window variant was measured and reverted after it failed locality. API-originated ordered-control, capture-controller, admission-pressure-only, writer-pressure, and production multi-segment-plus-Present evidence remains open. Continuity after encoder end or command-buffer commit remains Metal 4 work under `R-BACK-2.64`. |
| Final WSI session drain (`R-BACK-2.83`) | production terminal release and native end-to-end proof implemented behind the default-off Tape lane | `beginFinalWsiQuiescence` fixes the final writer fence, requests the existing typed Shutdown release when the session coordinator is the selected worker, wakes it, waits for acknowledgement/submission and exact sequence completion, then requires Ready FIFO and Tape residency to reach zero. The original native production-loop fixture holds an open session, publishes a later suffix ending in Present, proves the terminal latch blocks before release, supplies completion, and checks lease/residency/page/Ready zero plus final Present ordinal completion. A follow-up model/code audit found a latent empty-state mismatch: the coordinator could sleep in the generic Ready-only dequeue predicate even though the model permits an already-covered empty terminal acknowledgement. The production idle wait now includes exact `sessionReleaseFenceCovered` state, and a deterministic native fixture first observes the idle wait entry, then proves final WSI terminal acknowledgement, zero Tape ownership, and return without stop. A second Tape+DCE fixture pins the current fail-closed composition rule: Tape selects the session coordinator, so neither an unowned latch nor the uncomposed Direct-Arena DCE path is reachable. | Tape-on GT2/SFIV startup failures on `ce0967b8` were ultimately a second placement-dependent defect, not this latent idle predicate: first-use shader generation on `dxmt9-encode` entered locale-backed float formatting and repeatedly faulted in `snprintf_l`. R-BACK-4.9 now closes that boundary. Post-fix artifacts `app-d3d9-3dmark05-tape-on-float-format-fix-gt2-20260828` and `app-d3d9-sfiv-benchmark-tape-on-float-format-fix-20260828` pass with 1,859 and 2,871 frames respectively, valid images, continuing draw/encode work, and zero GPU command-buffer errors. Retain repeated reset/device-destroy coverage. Explicit DCE+session composition remains future work, and the default-off gate is unchanged. |
| Source-qualified supply timing (`R-BACK-2.84`) | opt-in production observation and native identity separation implemented | Legacy and Arena sources report replay-entry-to-publish and publish-to-encode-dequeue count/total/max/P50/P95/P99 separately. A bounded queue-local ledger uses payload kind plus source/storage/control/sequence identity; misses and overflow remain visible, and the disabled path performs only the cached feature check. Direct-Arena replay owns a scoped attempt token that follows its unbound entry into every SegmentSerial sibling: every failure or fallback cancels only that attempt's unpublished entries before a retry, while successful publication leaves exact identities for dequeue. Token-reservation failure suppresses unowned exact children and becomes an explicit publication attribution miss. Native coverage proves one Legacy, one Arena, a three-source SegmentSerial group, a rolled-back three-sibling attempt whose cancellation preserves an unrelated successful Arena attempt, and the invalid-token overflow continuation without hidden attribution. | Collect controlled HDR1/GT measurements and use the split to decide whether residual supply latency is replay construction or Ready scheduling; no policy or promotion conclusion is made by these counters alone. |
| Promotion gates (`R-BACK-2.50`) | partial / current GT2 locality gate failed | Permanent counters now split retirement attempts/success/ineligibility, receipt failures/depth, physical residency released, encoded-work cap closes, GPU-outstanding current/peak, retained-head identity outcomes, replay-FIFO intent outcomes, command-buffer/pass/load/store shape, and tile preservation. The Tape gate remains default off and the legacy payload-owning path remains the rollback baseline. H239 is progress-, visual-, and error-safe, but Tape-on raises CB/pass/tile by `0.34%`/`1.15%`/`4.40%`. | Keep the gate default off. Do not add another local wait or identity carrier. Choose between deterministic frame-wide sealing and a bounded incremental replay/store-action equivalence contract before rerunning GT2; only a non-increasing locality result permits GT1, GT3, SFIV, repeated-noise, and conservation gates. See `docs/perfomance/present-pacing/present-pacing-cpu-ready-next-source-intent.239.md`. |
| Exclusive encode execution provider (`R-BACK-2.66`, `R-BACK-42.13`–`42.18`) | target policy specified / current resolver and mode matrix differ | Device creation currently resolves `DXMT9_RENDER_PARTITION_MODE=identity|serial|parallel`; Tape and segmentation have separate controls. Every production lane uses a Replay/materialization worker followed by a distinct command-queue encode thread, matching stable `OwnedRawFinalChunkSlot` + `DedicatedEncodeThread`. Identity is the runtime default and explicit parallel adds child workers behind typed pre-effect serial fallback. The typed `EncodeExecutionTopology` algebra also names future ReplayWorker and ParallelCoordinator placements without enabling them. | Implement one queue-immutable `serial-final-slot-thread|serial-direct|long-session-direct|parallel-compact-soa` resolver. Keep the stable final-slot topology as the current baseline. Fused direct modes may later combine Replay and serial Metal encode on one Unix worker; only the compact-SoA mode retains a Replay-to-encode coordinator boundary and bounded children. Add requested/resolved topology, fallback, and differential mode-matrix evidence. |
...
| Capture and denied-first-lease frontier (`R-BACK-2.82`) | diagnostic-only P0 implemented / wild diagnostic exercise pending | Capture-only PE breadcrumbs cover bootstrap, arm, pending-alias flush disposition, and captured-Present reserve disposition. Perf-gated production counters bind the exact first-lease eligibility/action classifiers, Arena admission transitions, replay drain/push/in-flight/stage frontier, and the existing watchdog threshold report. Native backend coverage composes a real multi-source offload item, admission waiter, capture drain, denied lease, production-valid SegmentSerial capture sources, exact FIFO pressure escapes for at least two distinct heads under one unchanged capacity generation, one token per exact identity, four ineligible siblings, full control-predicate readiness, and counter/ownership conservation. Existing native pending-alias coverage validates ownership and replacement ordering but does not execute or bind the PE logging callsites. `EncodeSchedulingProgress` retains its fairness/invariants and requires both exact head escapes before latch clear and replay drain return, plus bounded token conservation and the P0 safety/liveness properties. | Exercise the PE breadcrumb ordering and opt-in diagnostic frontier in a controlled no-gputrace wild run only after separate authorization; do not promote policy from diagnostic evidence alone. |
...
| `CpuReadyTape::SourceRef` | Generation-bearing queue locator; Tape owns the source/page storage | It remains only for ineligible legacy completion identities and synchronous planner/encoder resolution. Eligible post-encode session/submission entries are replaced by receipts. `SynchronousSourceBorrowBatch` exposes planner/observer sources only through immediate generation-qualified visits; asynchronous handoff uses `WorkerOwnedSourceSnapshot` | Required until the final synchronous borrow ends and receipt activation succeeds; stale locator resolution then fails after two-phase finish advances generations |
...
R14 and R15 evaluated a seedless active-session retained-head lookahead as an
...
`R-BACK-2.50`. Its separate physical-pass token, seedless shadow planner call,
candidate-shape/run diagnostics, and dedicated experimental counters were
removed rather than left on the production hot path. The current production
planner is materially different: it carries an `ActiveRenderComplete` seed and
has runtime evidence that every issued seed ticket is consumed without
mismatch. The active whole-head hold may therefore reuse the ordinary
`TentativeRepresented -> restore -> bounded planner` path, but remains subject
to a new same-build GT2 locality gate. Fresh and active attempts, holds, and
successor-ready exits are reported separately so that gate cannot pass on a
...
H238 supersedes the H237 retained-suffix interpretation. The production
coordinator now supports the same pre-effect whole-head hold behind a fresh
frontier or an `ActiveRenderComplete` session, restores before ordinary
planning, and preserves the active-render seed. The carried terminal-suffix
transaction also folds into the existing carrier without replacing older FIFO
completion sources, callbacks, capture ownership, or command-buffer state.
Native fixtures cover active successor join and natural stop drain;
`CpuReadyActiveHeadLookahead.tla` covers hold, restore, active-seed preservation,
exactly-once effects, and progress, with a seed-dropping expected failure.

The final same-build GT2 pair still rejects promotion. Tape-off versus Tape-on
measured command buffers `3.9994 -> 4.0092` per Present, render passes
`15.7488 -> 15.9400`, tile preservation `103.339 -> 108.194 MiB/Present`, and
GPU command-buffer time `2.042 -> 2.157 ms/Present`; sampled FPS
`25.075 -> 25.176` is noise. Both lanes returned normally with zero GPU errors
and chunk rejects. A follow-up diagnostic run conserves all 3,490 retained-head
attempts as 2,018 held, 1,245 Present rejects, and 227 missing ordered-tail
Writing-successor identities; every other typed rejection is zero, and every
held head reaches its exact successor. All six terminal-suffix fresh/carried
participation counters are zero in this GT2 shape.

The remaining locality owner is therefore outside the local hold predicate: in
227 observations no future source identity exists yet. `DXMT9_CPU_READY_TAPE`
remains default off, and no wider promotion matrix is warranted. The next
bounded design must provide an atomic, generation-stamped producer next-source
intent or publication reservation before the current source becomes Ready,
with bounded capacity, cancellation, wake, FIFO restore, and exactly-once
publication modeled before implementation. A heuristic or timeout wait without
identity is not an admissible liveness substitute. Full evidence is recorded in
[`present-pacing-cpu-ready-active-head-locality.238.md`](../../../docs/perfomance/present-pacing/present-pacing-cpu-ready-active-head-locality.238.md).

H239 implements that bounded replay-FIFO intent and refutes the H238 attribution
as the final GT2 owner. The worker exposes only an already-adopted immediate raw
successor, stages the ordinal after successful Direct replay, advances it to an
exact generation-stamped predecessor/source identity at Ready publication, and
cancels a non-Direct disposition before effects. Native truth tables and the
extended `CpuReadyActiveHeadLookahead.tla` model cover empty FIFO, immediate
control, adoption, cancellation, replacement wake, hold/restore, seed
preservation, exactly-once effects, and progress.

In the H239 Tape-on GT2 lane the worker armed 5,053 intents and canceled 310,
but intent-selected and intent-successor counters remained zero: whenever the
promise could participate, the successor was already Ready and ordinary Ready
selection won. The same-build pair still changes command buffers
`3.9995 -> 4.0131`, render passes `15.7691 -> 15.9505`, tile preservation
`103.537 -> 108.089 MiB/Present`, and GPU command-buffer time
`2.012 -> 2.135 ms/Present`. All 3,383 retained-head attempts conserve as
2,195 held, 955 Present rejects, and 233 missing Writing/usable-intent rejects.

The remaining measured boundary is replay/store-action knowledge. Tape-on
reports 11,568 render-pass suffix exhaustions. Of 11,038 late unknown color or
depth actions, exactly 268 resolve to Clear/DontCare and 10,770 resolve to
Store. A bounded final-Present-in-window experiment only moved suffix
exhaustion to `5.870/Present` and measured `4.0080` command buffers,
`15.9534` passes, and `108.342 MiB` of tile preservation per Present, so it was
reverted. The next admissible design must either seal a deterministic frame-wide
planning interval or prove an incremental replay/store-action equivalence
contract. More local waiting or source-identity carrying is not supported by
...
|---|---|---|
| Segment-hold attribution of the handoff sites + trimming the top holder | instrumentation complete; residual classified | `DXMT9_PERF_QUEUE_MUTEX_SPLIT` now reports acquire and segment-hold rows. Current-HEAD GT2 records `submit_draw_run_batch_impl/append=3.347 ms/present` as the dominant measured hold, while the worker mark segment is absent. The row is attribution, not an automatic rewrite license; T2d remains deferred in the producer-concurrency tracker. See [append decomposition .36](../../../docs/perfomance/state-churn-encode/state-churn-encode-append-decomposition.36.md). |
| Producer concurrency redesign (T2 lock-free marking / T3 decoupling per `docs/superpowers/specs/2026-08-20-producer-queue-concurrency-design.md`) | **T2a/T2b/T2c implemented and evidence-complete for their bounded contracts**. Both marking paths use the arena-stamp exception plus frozen-ticket restamp; capture and the map fast path use the arena-owned synchronization; `ProducerMarkReclaim.tla` has production plus pin/restamp/capture counterexamples; native predicate binding, deterministic real-thread interleavings, source-order audit, explicit TSan, restamp counters, and current-HEAD GT2 queue profiling are green. The 2026-08-25 profile has no `submit_draw_run_batch_impl/mark` row, producer mark acquire-wait `0.1500 ms/present`, total acquire-wait `1.0536`, and `1,069,104 / 0` restamp checks/fires. Cross-workload operational evidence remains the four-run sweep cited by `specs/backend/producer-concurrency/gap.md` | remaining scope: `CommandQueue::submitDrawRun` deliberately keeps its conservative mark under the mutex; the producer ticket acquire/frozen re-read is still required and needs a new reservation/sufficiency proof before removal; T2d reserve-copy-commit remains deferred. Current measurement and proof boundaries are in [append decomposition .36](../../../docs/perfomance/state-churn-encode/state-churn-encode-append-decomposition.36.md) |
