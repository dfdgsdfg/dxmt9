...
|---|---|---|
| `TransactionalChunkSlotAssembler` (`R-BACK-2.85`) | A bounded Arena payload transaction now separates fallible `prepare()` (final block validation/publication) from infallible `commit()`. Production single- and batch-segment contexts bind a private move-only `ArenaCommitEvidence` minted by `CommandQueue`; it carries the live context token, exact source/segment/page/control generations, planned extent, and the seq-qualified resource closure digest after pool `mark*Use` retention. Compatibility callers retain the old evidence-free `commit()` spelling, but queue production never uses it. Capacity, validation, construction, resource-retain, publication, and completion seams now have deterministic native pins; pre-effect batch failures roll back, while post-effect source/batch failures poison and fail-stop. | GPU-side readback, bounded Wine faults, and wild promotion remain open; the ordinary Direct gate remains default off. |
| Arena direct replay to `ChunkSlot` (`R-BACK-2.86`–`2.87`) | Eligible Arena draw records now call `submitDirectReplayDrawFromCurrentState` and construct final payload SoAs without `DrawRunSubmission`; compatibility, inactive, and trace-capture lanes retain the carrier. Native A-B-A and production same-raw fixtures compare typed effective command order, explicit payload fingerprints/layout, normalized raw/source ordinals, exact resource identities, Clear/copy/Readback/Present barriers, failure disposition, and completion/reclaim identity from one validated pointer-free `WireFixture`. The production oracle resolves and retains registered surface wrappers before running Direct and Legacy. The ledger differential now confirms `tryAppendDrawRunBatch` emits no `copy.replay.submission-carrier` event: its input carrier is read field-wise and only the real `materialize.queue-final` append/materialization rows are counted. The stable class identity remains reserved for a future proven whole-carrier copy. | GPU readback, bounded Wine faults, and wild promotion remain open; the ordinary Direct gate remains default off. |
