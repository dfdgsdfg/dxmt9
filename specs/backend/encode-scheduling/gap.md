...
| Parallel render-pass executor (`R-BACK-2.63`) | pass-local production sealing shadow implemented / real execution default-off | Parallel requests run the validated production planner. With perf observation enabled, a fixed value-owned producer now scans the final effective replay order before effects and independently records up to 16 complete source-local pass candidates with two to 16 validated DrawRun children each. Clear, Present, and attachment changes delimit spans without entering children; carried/incomplete fragments, helper/control inputs, non-child DrawRuns, stale locators, resource incompleteness, attachment sampling, epoch mismatch, and pass/child capacity fail closed. Snapshots own source-qualified boundary/child locators, unioned exact resource values, one pass-action epoch, and first-draw provenance, with no borrowed payload/span/page/Metal state. Production records each static eligibility as typed `ParallelEncoderUnavailable` and consumes the original plan serially. Native coverage pins Clear -> large DrawRun -> Present, multi-pass batching, attachment splitting, query/readback/update/control/initializer exclusion, carried/incomplete fragments, locator/resource/hazard/epoch failures, 2..16 bounds, unchanged serial coverage, and perf-off zero producer work. The existing fake-child executor still proves ordered creation, arbitrary completion join, distinct child-local shadows, command/draw once, coordinator-only actions/sidecars/completion, join-before-parent-end, pre-effect fallback, and post-effect fail-stop. Counters expose attempts, candidates, sealed/eligible pass volume and maxima, child/draw volume and maxima, grouped rejection, and typed execution fallback. The earlier 2026-08-11 GT2 whole-source shadow run remains useful negative evidence: 1,142 Presents, 239 explicit-plan sources, zero eligible whole-source snapshots, artifact `experiments/output/app-d3d9-3dmark05-sealed-pass-shadow-gt2-20260811/result.json`. It does not measure this new pass-local producer. | Extend sealing across carried-session, cross-source, and multi-window logical passes only with exact action ownership. Define and test the residency-pinned complete native-state re-resolution step, then implement the real WMT `MTLParallelRenderCommandEncoder` adapter and bounded worker pool. Formal/refinement, Metal-backed visual/locality/failure evidence, and a fresh pass-local counter run remain missing. No promotion or performance improvement is claimed; the provider remains default-off and selects no Metal parallel work. |
