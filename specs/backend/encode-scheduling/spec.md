...
| Composed end-to-end progress (`R-BACK-2.67`) | missing composition model spanning admission, lease/wake, publication, session continuation, completion release, and present pacing with leads-to and lost-wakeup-freedom properties under weak fairness; existing per-component liveness (`CpuReadySessionProgress`, `SessionCapacityLease`, `ConcurrentProgressSignals`, `EncodeSessionCompletion`, `PostEncodePayloadRetirement`) becomes the abstraction source. Isomorphism pins currently cover the present-ordinal gate only; lease/wake and admission wait predicates remain unpinned. Runtime liveness watchdog observability is missing. |
