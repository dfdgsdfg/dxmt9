...
| Writing slot | `CommandQueue::writingSlotOwnership_` | bound at `ensureWritingSlotUnlocked`, asserted or-locked at ordinary `submitDrawRun` final-slot append with `lock.owns_lock()` as witness. Structurally true today; it exists so a future unlocked append (T2d) cannot land without the model |
...
- **T2d reserve/copy/commit (deferred)**: the proposed bounded reservation
  ...
  shows zero append-segment wait. The measured batch carrier is now retired;
  ordinary `submitDrawRun` final-slot append remains mutex-protected until a
  real waiting victim clears the
