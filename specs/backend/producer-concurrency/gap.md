...
| C++ memory-order harness (R-BACK-43.6 residual) | implemented, current-HEAD gate green, with scope statement | `dxmt9-producer-interleaving-spec` drives production queue/pool symbols and translates the model traces. The scripted lane proves protocol order, not arbitrary memory order. TSan is configured by `tests/meson.build` with `TSAN_OPTIONS=halt_on_error=1:abort_on_error=1`; the explicit 2026-08-25 TSan run completed 395,109 production release/acquire observations with zero violations. Its relaxed control also reported zero violations on this M1, so the free-running negative lane had no detection power and is not counted as proof. Source-order audit + TSan + TLC counterexamples are the effective composition; exact commands and fixture exclusions are recorded in [append decomposition .36](../../../docs/perfomance/state-churn-encode/state-churn-encode-append-decomposition.36.md). |
...
| Restamp-fire observability | implemented; current-HEAD GT2 measured | `mark_ticket_restamp_checks` / `mark_ticket_restamp_fires` measure the frozen-ticket window. The 2026-08-25 GT2 run recorded `1,069,104 / 0`; no promotion or protocol weakening is derived from the zero wild count. See [append decomposition .36](../../../docs/perfomance/state-churn-encode/state-churn-encode-append-decomposition.36.md). |
