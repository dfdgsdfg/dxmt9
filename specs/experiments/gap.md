...
| 3DMark05/06 canonical benchmark lanes | ✅ named invocation and artifact identity; runtime evidence remains per lane | R-HARN-RUN-6.1–6.4. One shared preset table defines bounded scene, CPU diagnostic, score, feature, batch, and exhaustive suites for both products; both launchers reject unknown names, preserve raw arguments as an explicit `custom` lane, and `run_experiment.py` records the resolved identity in `result.json:benchmark_lane`. Native contract tests cover every preset and raw-override precedence. This closes reproducible selection, not scene correctness or performance evidence. |
| 3DMark06 external experiment lane runtime evidence | ⚠️ GT1 render path qualified; per-test result evidence open | The ignored external payload is installed locally and the public-key Advanced Edition reached the real GT1 render path under Sikarugir after replacing Wine's incomplete builtin `d3dx9_28` compiler with the native runtime. The perf-profile run completed rendering with no GPU command-buffer errors or chunk rejects, then remained at the UI until the positive timeout. The positional `.3dr` artifact was not emitted and the edition is not Professional, so this run does not yet prove accepted unattended per-test CLI/result behavior. HDR1/HDR2, GT2, CPU1/CPU2, screenshot oracles, and score/result export remain unqualified. R-HARN-RUN-6.* defines their reusable invocation identities without claiming those missing runtime gates. |
