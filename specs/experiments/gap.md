...
| Unified Render Tape `frame-tape` / `sequence-tape` profiles (`R-HARN-7.1`–`7.6`, `R-HARN-REPLAY-7.1`–`7.23`) | ⚠️ bounded refinement, production capture/provider replay, capture identity, reduction, executable projection, and parallel join | Production validation/provider replay retain unchanged canonical chunks, while texture/surface schema payloads use only the canonical V2 grammar; legacy texture and raw surface descriptors are Retired. The r57 Sikarugir GT2 bundle closes indexed production provider replay with an authenticated `output.rgba` sidecar and the bounded R-HARN-REPLAY-7.19 pixel envelope. The r65 capture closes original capture-authority source/pass identity, and r66 closes one `FULL_SNAPSHOT`-anchored executable GT2 projection through two candidate plus two strict fresh-provider runs. R-HARN-REPLAY-7.23 closes the bounded fresh-process identity/ExplicitParallel implementation join; its non-vacuous GT2 wild recipe remains unexecuted. Strict SHA equality remains the default promotion gate. Default sparse-delta start-state folding, PresentEx, controls/destruction in reducible input, prior-output loads, and broader interval counts remain open. |
| Render Tape authoritative identity (`R-HARN-REPLAY-7.21`) | ✅ bounded original-capture join and wild evidence | `device_c_render_tape_identity.*` owns the fixed pointer-free layout and fail-closed exact command/record coverage, monotonic source/sequence, bounded-token, and pass-range validation. Capture with an installed publisher selects a capture-only 8-MiB/512-page-per-source Arena profile; PE object/materialization preflight completes before the final event ordinal is assigned, and Direct Arena publication copies actual source/sequence plus frozen pre-reorder FrameGraph ranges. The r65 GT2 bundle validates 36 production sources and 51 pass ranges. Capture-off and ordinary streaming bounds are unchanged. Compatibility/control sources outside this bounded one-source-per-event grammar still fail closed rather than receiving inferred identity. |
| Render Tape v2 to executable GT2 draw-slice (`R-HARN-REPLAY-7.16`/`7.17`/`7.22`) | ⚠️ bounded GT2 executable lane proven; default delta start-state folding open | `device_c_render_tape_projection.*` validates one sidecar-proven pass, derives an APPLY_STATE bootstrap from a selected producer-emitted `FULL_SNAPSHOT`, rebuilds canonical Clear → contiguous Draw range → Present bytes, and conserves exact object/blob/mutation closure. GT2 r66 selects 46 indexed Draws from event 1042, records 5..50, then passes two oracle-free and two strict fresh-provider runs with projected SHA-256 `bb89749...`. Native and hermetic CLI tests cover malformed identity, pass crossing, conservation, execution order, deterministic output, and failure cleanup. Default delta captures such as r65 have no self-contained Draw anchor; folding BootstrapState plus preceding sparse deltas remains a separate formal/correctness task, so the path stays opt-in. |
| GT2 Render Tape parallel verification recipe | ⚠️ specified, unexecuted | Capture one bounded GT2 `frame-tape` with `DXMT9_RENDER_TAPE_CAPTURE=1`, `DXMT9_RENDER_TAPE_PROFILE=frame-tape`, an explicit `DXMT9_RENDER_TAPE_OUTPUT_ROOT`, `DXMT9_PE_DRAW_FULL_SNAPSHOT=1`, and `DXMT9_PE_CHUNK_MAX_RECORDS=256`; keep capture itself on `DXMT9_RENDER_PARTITION_MODE=identity`. After structural validation, run `python3 scripts/tools/run_dxmt9_render_tape.py parallel-verify <bundle>`; it launches at least two fresh processes per identity and ExplicitParallel mode and requires strict equality within each mode and across modes. Every parallel run must report `selected>0`, `children>=2`, `draws>0`, worker batches/tasks/active peak all greater than zero, and zero GPU command-buffer errors. CPU/wall timing is informational and is not a non-vacuity gate. This recipe is a future supervised GT2 capture gate, not wild evidence, and does not lower any threshold or change the default-off/identity policy. |
