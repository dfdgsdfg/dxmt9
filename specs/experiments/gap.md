...
| 3DMark06 external experiment lane runtime evidence | ❌ | The catalogue entry, bounded positive timeout, Sikarugir runtime selection, SM2 GT1 default, HDR/SM3 override path, positional `.3dr` output, unlocked-session guard, and pre-staging dry-run contract are implemented and covered by `dxmt9-3dmark06-lane`. No 3DMark06 payload is vendored or installed in the repository, so no wild result, screenshot oracle, or accepted per-test CLI selection is claimed. Most test-selection switches are a Professional Edition facility; the first wild qualification must record the installed edition and show that its selected test actually ran. |
