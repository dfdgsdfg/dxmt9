...
| Unified Render Tape `frame-tape` / `sequence-tape` profiles (`R-HARN-7.1`–`7.6`, `R-HARN-REPLAY-7.1`–`7.13`) | ⚠️ bounded production-provider identity slices, capture output oracle, and digest-matched Clear and non-uniform textured-UP wild replays complete | The provider still routes unchanged canonical chunks through production validation, replay, queue, lifetime, Present, and readback owners. Its fail-closed grammar admits either the established full-surface `Clear` → standard `Present` slice or exactly one `Clear` → fixed-function textured `DrawPrimitiveUP` → standard `Present` interval. The textured form preserves total order across actual command-chunk event boundaries and requires one inline `XYZRHW|DIFFUSE|TEX1` triangle, one bound single-level A8R8G8B8 texture with one full seed, and either the exact FVF state or production's byte-exact four-element FVF-generated vertex declaration; shaders, arbitrary declarations, VB/IB, PresentEx, controls/destroys, partial/compressed content, prior-output loads, and multi-frame input remain rejected before effects. Focused native tests cover combined and split chunk shapes, declaration near-misses, and 2/2 or 3/3 object conservation. The Apple M1 Metal fixture produces the non-uniform 16×16 SHA-256 `3dc6ca2708ccbb285106dea4b1cba42e6d67dd69ffe72ab003d87d7d8250b72e` through both state representations. The 2026-08-13 Sikarugir bundle `experiments/output/render-tape-wild-textured-r2/tapes/frame-99975454225700-1` closes non-uniform production identity: 8 events, 3 records in 2 chunks, 3 definitions, one 64-byte texture mutation, a 32-byte immutable declaration, 2/2 blob references, 3/3 object conservation, and matching capture/replay 256×256 SHA-256 `866e45bc5527c590f7cbf1deb9ca8fd5aa3ac2eddcd6746bdaf0572848a78c17` with `output_non_degenerate=true`. `sequence-tape`, reducer/bisect tooling, tape-to-draw-slice projection, and repeated captured-frame identity remain open. |
