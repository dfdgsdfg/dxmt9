...
| Render Tape v2 to executable GT2 draw-slice (`R-HARN-REPLAY-7.16`/`7.17`/`7.22`) | ⚠️ bounded GT2 executable lane proven; default sparse fold closed below wild evidence | `device_c_render_tape_projection.*` validates one sidecar-proven pass, folds its canonical BootstrapState plus ordered sparse APPLY_STATE/Draw sections and standalone constants through the selected first Draw, emits one deterministic APPLY_STATE|FULL_SNAPSHOT bootstrap in the existing grammar, rebuilds canonical Clear → contiguous Draw range → Present bytes, and conserves surviving generation-qualified object/blob/mutation closure. Pure adversarial native coverage pins exact last-write behavior and fail-closed validation; the default-delta hermetic fixture validates, materializes, and passes repeated fresh production-provider replay without `DXMT9_PE_DRAW_FULL_SNAPSHOT`. GT2 r66 remains the only production executable-projection oracle evidence: it selects 46 indexed Draws from event 1042, records 5..50, then passes two oracle-free and two strict fresh-provider runs with projected SHA-256 `bb89749...`. A production default-delta GT2 projection/oracle run remains unproven, and PresentEx, reducible controls/destruction, prior-output loads, volume resources, and broader interval/provider grammar remain open. |
