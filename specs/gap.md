...
| B8: `DrawParam` / `FlatDrawStateView` -> Metal encoder draw command | ⚠️ | `dxmt9-metal-encoder-recorder-spec` overrides `MTLRenderCommandEncoder_encodeCommands` and records final WMT command payloads for `drawIndexedPrimitives`, including the 3DMark05 fan-fix shape: triangle primitive, `u16`, index count 6, generated index-buffer offset 0, instance count 1, base vertex 0, base instance 0, plus the stream-0 Metal vertex-buffer bind offset 3712 at slot 1. Focused 3DMark05 diagnostics also show the fixed bad draw as `api=DrawIndexedPrimitiveUP`, `indexType=u16`, `userIndexBytes=12`, and `indexed=1`. | Extend the recorder seam through the full `encodeDraw` path so tests also assert final `DrawVolatile` bytes, live user-vs-bound vertex/index source selection, final command ordering, and `drawPrimitives` arguments. Logs and screenshots remain useful evidence but not deterministic unit coverage. |
