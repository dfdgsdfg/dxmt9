...
| Argument-buffer hybrid Stage 2 (`R-BACK-12.22`–`R-BACK-12.26`) | ⚠️ | Stage 1 ✅. Stage 2 uniform-buffer path and CPU descriptor population are implemented and covered by argbuf native specs. **2026-05-17 audit:** texture-bound draws were temporarily promoted to Stage 2, but live shader-corpus readbacks produced Metal GPU address faults even though descriptor-value tests passed. The default selector now keeps texture-bound draws on Stage 1 while uniform-only Stage 2 remains available. `dxmt9-shader-argbuf-binding-value-spec`, `dxmt9-argbuf-populator-spec`, and related MSL/source tests still pin 2D/cube/3D/sampler descriptor ids and resource writes. Remaining evidence gap is live GPU-visible equality for Stage 2 argument-buffer texture/sampler sampling on Apple Silicon. Do not re-promote texture-bound Stage 2 until representative 2D, cube, 3D, sRGB, mip/LOD, unbound/default sampler, filter, and address-mode shader-corpus readbacks are fault-free and pixel-equal. |
