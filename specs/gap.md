...
| Wine d3d9 test inventory (visual.c / device.c / d3d9ex.c / stateblock.c) | ✅ | [`specs/gap_wine_d3d9_test.md`](gap_wine_d3d9_test.md) inventories every Wine `START_TEST` entry across the four files (268 total), records the dxmt9 porting status, and links each to the PE conformance function(s) carrying the evidence. Regenerated from Wine source + the gitignored `specs/wine_test.plan.md` via `scripts/tools/gen_wine_d3d9_test_inventory.py`. |
