...
| Wine internal `NtGdiCreateDIBSection` user-memory aliasing (for `IDirect3DSurface9::GetDC` on `D3D9Ex` SYSTEMMEM user-memory surfaces) | ❌ | Wine-only Win32 extension: a DIB section whose `dsBm.bmBits` is the caller-supplied user pointer cannot be created via the public Win32 API alone. Wine's wined3d goes through `NtGdiCreateDIBSection` (`win32u.dll`) with an opt-in user-bits pointer; that entry point is not part of the documented Win32 SDK and its calling convention drifts across Wine versions. **Test affected:** `tests/conformance/d3d9/d3d9_conformance_resource.c::test_ex_user_memory_getdc_dib_identity` (`dib.dsBm.bmBits == data` assertion). Workaround attempts surveyed in `specs/wine_test_failures.plan.md` §8 (`MapViewOfFileEx(lpBaseAddress=user_ptr)` rejected on macOS Wine because the user heap is unreserved; copy-on-GetDC + sync-on-ReleaseDC breaks the pointer-identity check). Same category as the documented `NtQueryVirtualMemory(info=kMemoryWineLoadUnixLibByName=1002)` blocker — both are Wine internals reachable only from inside Wine itself. Closing this gap requires either calling Wine-private GDI entry points (not portable) or a Wine source patch upstream. |
