...
| Shader-runner fixtures: SR-blocked `visual.c` entries | ❌ | 4 entries truly deferred to runner DSL/fixture work: `fixed_function_bumpmap_test` (TEXBEM runner support), `pretransformed_varying_test` (XYZRHW pretransformed-varying fixture), `vface_register_test` (face-orientation input fixture), `fp_special_test` (NaN/Inf oracle + runner tolerance policy). All four remain `planned` in `specs/wine_test.plan.md` because no PE-route equivalent captures the observable contract. |
