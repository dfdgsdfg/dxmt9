...
| Texture/sampler argbuf resource-array lane (`R-BACK-12.22`–`R-BACK-12.26`) | ⚠️ | Not part of the default Stage 2 hot path. Current Stage 2 is constants-only: slot-30 argbuf holds `VsConsts` / `FfpVsConsts` / `PsConsts` / `FfpPsConsts`, while texture and sampler resources stay on direct `set*Texture` / `set*SamplerState` encoder calls. Earlier attempts to place texture/sampler arrays in the argbuf faulted on representative texture corpus readback. Current fault candidates are sampler argument-buffer support (`supportArgumentBuffers` / resource-id bridge), sampler lifetime retention through command-buffer completion, texture/sampler `MTLResourceID` ABI mismatch, incomplete `useResource` / `useHeap` residency for argbuf-pointed resources, and MSL layout mismatch for resource arrays or root argbuf reference shape. Missing evidence: feature-flagged probes for texture-only argbuf sampling, sampler-only argbuf sampling, combined 2D sampling, cube/3D/volume matrix, sampler filter/address/sRGB/mip matrix, paired Stage 1 vs resource-array Stage 2 pixel equality, and a microbenchmark proving argument-buffer patch + residency cost beats direct binding for target workloads. **2026-05-25 attempt + revert (`695a0d8` → `41ba0fe`):** an opt-in lane (`DXMT9_ARGBUF_RESOURCE_ARRAY`) wiring textures/samplers into the slot-30 argbuf with `useResource(Read\|Sample)` residency was implemented and reverted — readback-verified on Apple Silicon: the 2D path (texture_2x2 / dependent_texture_read / ffp_pixel_texture) **still fails** while cube/volume correctly fall back to the direct lane. So the fault is **not** `useResource` residency. **2026-05-25 root-cause diagnostic (lane re-applied uncommitted, run under `MTL_DEBUG_LAYER=1` + `MTL_SHADER_VALIDATION=1`, then discarded — branch clean at `d5932c1`):** the runtime error is `MTLCommandBufferErrorDomain Code=3 "Caused GPU Address Fault Error (kIOGPUCommandBufferCallbackErrorPageFault)"` — a **GPU page fault at dereference time**; the readback garbage (`probe rt0 expected_bgra=(0,0,255,255) actual=(255,0,255,255)`) is the downstream symptom, not the cause. The Metal **API validation layer reports no error**, so the argbuf encoding is structurally valid CPU-side — the GPU is dereferencing a `gpuResourceID` whose backing texture is not addressable on the executing encoder. Two descriptor shapes were A/B'd and **both page-fault identically**: (a) N consecutive single-texture descriptors `arrayLength=0` at ids 4..11 / 12..19, and (b) one descriptor per array with `arrayLength=8` at id 4 / id 12 — so the MSL `array<…,8>` reference shape is **ruled out** as the cause. Remaining suspects: the encoded `gpuResourceID` points at a texture recycled by the transient pool before GPU execution, or `useResource` lands on a different encoder than the one running the draw. Decisive next step is a `.gputrace` frame capture (`DXMT_METAL_CAPTURE_FRAME`) to read the faulting address + resource — not another static descriptor change. Lane stays default-OFF. |
