...
| Submission grain: 1 chunk → N MTLCommandBuffer chain (`R-BACK-2.29`–`R-BACK-2.32`) | ⚠️ | **Implementation + measurement landed 2026-05-10.** S2 (`encodeChunk` mid-chunk split via `splitMidChunk` lambda mirroring `splitBeforeBlockingPresent`, env-driven `DXMT9_MID_CHUNK_COMMIT_POLICY=off\|per-render-pass\|per-n-records`, default off) and S1 (`CommandQueue.tla` extended with `MAX_SUBCB`, `subCBProgress`, `subCBChainLength`, `finalCommitted`, `slotIsPresent`, `presentTokenSignaled`; new actions `EncodeMidChunkCommit`/`EncodeFinalCommit`; new invariants `SubCBProgressBounded`/`OnlyFinalAdvancesSeqId`/`PresentRoutedToTail`; TLC pass with 39,638 states / 18,902 distinct at MAX_SUBCB=3). New counters `sub_command_buffers` + `chunk_subcb_count_max` in `kCounterTable` and per-frame snapshot. S3 SFIV A/B: `command_buffers / frame` 1 → 2 (policy works as designed); steady-state p99 deltas: `encode_chunk_cpu_ms` −25%, `completion_wait_ms` −13%, `present_acquire_wait_ms` **−93%** (the structural win — drawable acquire overlaps with GPU work because mid-chunk commit lets GPU start before chain tail closes). Capture was menu workload (RP=2 / 11 draws per frame); heavy SFIV-scene re-measurement is the remaining evidence. **Still missing:** heavy-scene A/B comparable to the original P1 13.25fps measurement; `per-n-records` split policy is implemented but not exercised by perf probes (no `DXMT9_MID_CHUNK_COMMIT_POLICY` setting on `dxmt9-perf-*` probes); R-BACK-2.32 reclaim-on-last-sub-CB invariant relies on Metal's same-queue in-order guarantee and on the existing tracker not enqueueing seqId for mid-chunk commits — formal evidence (test or instrumentation) is desirable. |
