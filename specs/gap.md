...
| Argument-buffer hybrid Stage 2 (`R-BACK-12.22`–`R-BACK-12.27`) | ✅ | Stage 1 ✅. **2026-05-17 narrowing:** Stage 2 is now constants-only — only the four per-frequency constant-buffer pointers (`VsConsts`/`FfpVsConsts`/`PsConsts`/`FfpPsConsts`) live in the slot-30 argbuf at `[[id(0..3)]]`. Texture and sampler resources continue to use the validated direct `[[texture(N)]]` / `[[sampler(N)]]` binding lane that Stage 1 already proves. Reason: keeps Stage 2 from inheriting the argbuf-indirect resource-residency hazard class while still removing per-draw constant-slot rebinds. `dxmt9-argbuf-hybrid-msl-spec` pins the constants-only argbuf descriptor shape (no `textures2d/Cube/3d` / `array<sampler>` members), `dxmt9-shader-argbuf-binding-value-spec` pins that Stage 2 keeps the direct `[[texture(N)]]` / `[[sampler(N)]]` MSL parameters when textures are bound, and the encoder gates the direct fragment texture/sampler bind on `!fragmentTextureSamplers.empty()` regardless of `argbufHybridMode`. **A/B pixel-equality gap closed 2026-05-17 by `R-BACK-12.27`:** every currently-passing `.shader_test` is fanned out as a paired Meson test — `dxmt9-shader-corpus-<case>` (suite `shader-corpus`, Stage 2 default lane) and `dxmt9-shader-corpus-stage1-<case>` (suite `shader-corpus-stage1`, forces `DXMT9_DISABLE_ARGBUF_HYBRID=1`). Both lanes assert the same `[probe]` lines through `shader_runner_dxmt9`, so passing both *is* the pixel-equality proof; a regression in either lane is a Stage 2 release blocker. A future widening that puts textures or samplers back in the argbuf must re-prove resource residency and update §5.4 of `specs/backend/design.md` before landing. |
...
| B5: draw state -> shader texture/sampler bindings | ⚠️ | `dxmt9-shader-argbuf-binding-value-spec` asserts `FlatDrawStateRecord` texture/sampler slot values through shader context, Stage 1 and Stage 2 `texN`/`sampN` direct `[[texture(N)]]` / `[[sampler(N)]]` MSL bindings, the constants-only Stage 2 argbuf descriptor shape (only the four constant-buffer pointers at `[[id(0..3)]]`), null texture slots, and sampler/LOD defaults. `dxmt9-encode-draw-recorder-spec` covers representative texture/sampler encoder calls. `dxmt9-argbuf-populator-spec` covers the Stage 2 argbuf FfpVs repoint seam. | Remaining evidence gap is live GPU-visible equality for Stage 2 sampling on Apple Silicon and Stage 2 constant-buffer dirty rewrite interactions beyond the deterministic write seam. |
...
| Current shader failing evidence | ⚠️ | The shader-runner manifest currently has no failing entries; `shader_corpus_tool.py gaps` reports 175 / 175 passing and all runtime shader models covered. Recently promoted evidence includes SM1 depth/specular/vspec/dependent texture op families, SM1 color-output routing through `r0`, `VFACE`, `POSITIONT`, TFACTOR, `RESULTARG = TEMP`, `D3DTA_CONSTANT`, texture transforms, generated texcoords, generated mip and autogen update `TEXLDL`, 2D `SetLOD`/maxmip, float/G16R16/V16U16 and depth=1 volume sampling, 2D/cube `UpdateTexture` evidence, cube +X sampling, NPOT wrap, FFP fog/range/`oFog`, FFP lighting/specular/material-source/Gouraud probes, constant-clamp/FLOAT16/default-varying probes, and programmable skinning edge probes. SH-D EXP diagnostics still record vertex texture fetch, negative fixed-function fog, reflection/spheremap non-identity, point-size/flat-shading edges, YUV/color-key/vendor formats, and broader lighting/material gaps. | Keep failing diagnostic probes out of the passing manifest until direct runner/experiment evidence is green. |
...
| `shader_runner_dxmt9` extended probe layer | ⚠️ | dxmt9-local runtime probes now cover texture setup/readback, SM1 `ps_1_1` through `ps_1_4` texture-coordinate baselines, Wine-valid SM1 dependent/depth/specular/vspec opcodes, SM1 `COLOROUT`/`r0` export routing, `vs_1_1` constant color and `oFog`, dependent texture read, VS geometry, constant-clamp, FLOAT16 input, default varying components, `VFACE`, `POSITIONT`, multi-stream VS texturing, programmable skinning, viewport/half-pixel/rasterization, color-write/MRT/alpha/depth/blend/fill/clip/fog/sRGB states, TFACTOR, `RESULTARG = TEMP`, `D3DTA_CONSTANT`, texture transforms, generated camera-space position/normal texcoords, generated mip and autogen-update `TEXLDL`, 2D `SetLOD`/maxmip, float/G16R16/V16U16 and depth=1 volume sampling, cube +X sampling, 2D/cube update evidence, NPOT wrap, FFP lighting/specular/material-source/Gouraud probes, and FFP vertex-blending intent across one-weight, 2/3-weight, indexed, FVF `XYZB2`, and outside-probe variants. FFP last-beta indexed FVF variants, normal/lighting vertex-blend interactions, negative fixed-function fog, reflection/spheremap non-identity generated texcoords, true multi-slice volume, YUV/color-key/vendor formats, and multi-draw state-transition probes remain open. |
| Module-boundary harness (`tests/module_boundary/`) | ⚠️ | `tests/module_boundary/MANIFEST.toml` enumerates artifact staging, provider-side probe, app-local loader smoke, builtin loader smoke, result schema, and boundary-separation cases. `scripts/check/check_module_boundary_manifest.py` validates R-TEST-13 coverage; `run_module_boundary.py` validates manifests/results, stages x64/x86 PE artifacts plus MinGW/Wine runtime dependencies, runs dry-run app-local results, auto-discovers configured Wine, separates app-local artifacts from builtin Wine-tree artifacts, records PE probe phase events in timeout JSON, classifies app-local unix-provider load failure from bridge diagnostics, executes the provider-side `dxmt9-unix-chunk-injection-probe` with perf counters, and emits the common `dxmt9.debug.result.v1` path via `--debug-output` with staged artifact manifests plus provider-locator and ABI-handshake sidecars. `dxmt9-module-boundary-probe_x64/_x86` cross-build successfully; short Heroic Wine x64 runs now identify app-local failure after `load_d3d9`/exports at unix-provider load and builtin completion through factory before Wine wrapper timeout. Remaining work is to resolve app-local unix provider load under Heroic Wine, make the Wine wrapper exit cleanly after completed PE probe, and add passing bridge ABI live checks through Wine. |
| Expanded `.shader_test` corpus (arithmetic, comparison, flow control, transcendental, matrix, source modifiers, texture, FFP sanity/alpha test) | ✅ | Corpus manifest tracks 175 passing probes and no failing entries. All runtime shader models are represented. Sequential `meson test -C build-x86_64-builtin --suite shader-corpus --print-errorlogs --num-processes 1` passes 175/175. FFP vertex blending, lighting/specular/material-source/Gouraud, fog/range/oFog, cube/depth=1 volume/autogen update, and SM1 legacy texture opcode runtime coverage are represented. |
