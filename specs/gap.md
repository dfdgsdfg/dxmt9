...
- **tile-FFP ⚠️ TOGGLE LANDED + REAL DIVERGENCE FOUND — NOT closed** (`e165154`): the `DXMT9_TILE_FFP=force\|off\|auto` toggle works (force→tile, off→portable, proven by `tile_ffp_pass_count`/`portable_ffp_pass_count`). But the A/B readback on the same non-textured FFP fog+alpha-test draw **DIVERGES**: tile path → `(0,0,0)` (black), portable path → `(64,128,191)` (the fog colour). They are NOT pixel-equal, violating R-BACK-13 equality. The portable result (a fully-fogged fragment) is the plausibly-correct D3D9 behaviour; the tile path producing black looks buggy (discarded/unwritten), but the direction is not certain without the D3D9 oracle (depends on the quad's vertex alpha vs the `>= 0.5` alpha-test). **This is a latent bug in a default-on Apple-Silicon path.** Repro: `DXMT9_TILE_FFP=force` vs `off` on `tests/shader_runner/corpus/ffp/dxmt9_tile_ffp_fog_equality_readback.shader_test` (orphan repro file, intentionally unregistered in MANIFEST so it does not false-green the suite). Next: a sub-agent must determine the D3D9-correct result, fix the divergent path (likely `makeFfpTilePixelSource` fog/alpha-test emit vs the portable FFP emitter), then turn the repro into a registered equality assertion.
