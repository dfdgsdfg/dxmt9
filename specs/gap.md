...
| Clip planes | ⚠️ | `transformClipPlane()` + `FfpVsConsts.clipPlanes` + `[[clip_distance]]` emitter path exist; degenerate planes work (the `dxmt9_clip_plane_runtime_readback.shader_test` plane (0,0,0,-1) → dot=-w=-1 clips every fragment as expected). **2026-05-23 (`234cc2d`):** FFP and metal_ir vs_3_0 emitters now pre-init the six `out.clipDistance[i]` slots to `+1.0` before the mask-conditional dot assignment — necessary because Metal's `[[clip_distance]]` discards a fragment when **any** slot is < 0, so disabled-slot uninitialised trash could clip otherwise-passing pixels. That fix is necessary but not sufficient. **Owner:** `backend/ffp`. **Effort:** medium-large. **Symptom:** Wine `clip_planes_test` (visual.c:16182) reports half-space discrimination failure — every non-degenerate FFP clip plane still clips ALL fragments on Apple7 regardless of plane direction (verified X+, X−, Y+, Y−). The emitted MSL source is correct on inspection (`DXMT_DUMP_SHADER_DIR` confirms init + masked dot). Hypothesised but unverified: `buildFfpVsConsts` (`src/dxmt9/dxmt9_draw_state.cpp`) is inlined under `-O2`, so the FfpVsConsts payload reaching the GPU has not been traced; the `clipPlaneMask` and `clipPlanes[i]` values at the unix upload boundary remain opaque. **Regression seed:** `tests/shader_runner/corpus/render_state/dxmt9_clip_plane_halfspace_readback.shader_test` (status=failing in MANIFEST.toml). **Investigation path:** rebuild with `-Doptimization=0` (or `[[gnu::noinline]]` on `buildFfpVsConsts`), add `std::fprintf` of `out.clipPlaneMask` + `out.clipPlanes[0]` at the bottom of that function, observe whether the mask reaches 0 (upstream bug in `clipPlaneMaskFromState` / `core_draw.cpp:2210`) or the plane reaches zero (payload assembly), then trace the uniform-buffer encode path. **Inventory tracker:** `specs/gap_d3d9_wine_test.md` row `clip_planes_test` (visual.c:16182, currently 🟡). |
...
| Texture/sampler argbuf resource-array lane (`R-BACK-12.22`–`R-BACK-12.26`) | ⚠️ | Not part of the default Stage 2 hot path. Current Stage 2 is constants-only: slot-30 argbuf holds `VsConsts` / `FfpVsConsts` / `PsConsts` / `FfpPsConsts`, while texture and sampler resources stay on direct `set*Texture` / `set*SamplerState` encoder calls. Earlier attempts to place texture/sampler arrays in the argbuf faulted on representative texture corpus readback. Current fault candidates are sampler argument-buffer support (`supportArgumentBuffers` / resource-id bridge), sampler lifetime retention through command-buffer completion, texture/sampler `MTLResourceID` ABI mismatch, incomplete `useResource` / `useHeap` residency for argbuf-pointed resources, and MSL layout mismatch for resource arrays or root argbuf reference shape. Missing evidence: feature-flagged probes for texture-only argbuf sampling, sampler-only argbuf sampling, combined 2D sampling, cube/3D/volume matrix, sampler filter/address/sRGB/mip matrix, paired Stage 1 vs resource-array Stage 2 pixel equality, and a microbenchmark proving argument-buffer patch + residency cost beats direct binding for target workloads. |
