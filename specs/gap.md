...
| Per-frequency draw-uniform split (`R-BACK-12.1`–`R-BACK-12.21`) | ❌ | `specs/backend/draw-uniforms/` adopted as the target layout. Today the encoder writes a single 9 KB `DrawUniforms` slab per draw (after `46b234d` slab coalescing) which hits Metal `MTLStorageModeShared` write bandwidth at ~3 GB/s. Missing: backend-derived `DirtyMask` + range counters at chunk-import apply time, `VsConsts`/`PsConsts`/`FfpVsConsts`/`FfpPsConsts`/`DrawVolatile` host structs, MSL prelude split, shader-translator field routing, `WMTRenderCommandSetVertexBytes` bridge command, `tests/native/core/draw_uniforms_layout_spec` and `draw_uniforms_dirty_spec`, `uniform_*_bytes` perf counters. Empirical motivation in `docs/perfomance-bottleneck.md`. |
