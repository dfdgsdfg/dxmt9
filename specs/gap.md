...
| Buffer / Texture / Surface lifecycle | ⚠️ | Pool-based reset behavior exists; public COM refcount/lifetime, common private-data semantics, D3D9Ex user-memory texture/offscreen-surface success paths, and shared-handle/user-memory error codes need Wine-oracle `device.c` / `d3d9ex.c` conformance coverage |
...
| Per-frequency draw-uniform split (`R-BACK-12.1`–`R-BACK-12.21`) | ✅ | Landed: `VsConsts`/`PsConsts`/`FfpVsConsts`/`FfpPsConsts`/`DrawVolatile` host structs + MSL prelude (A2), `setVertexBytes` bridge (A1), shader translator routing (B1), backend `DirtyMask` + range counters at chunk import (C1), encoder per-stage bind + `setVertexBytes` push (C2), `uniform_*` perf counters (C3), layout + dirty unit specs (D1, D2). Measured (offscreen-heavy@256, 3 reps): `transient_upload_bytes` 87 MB → 1.17 MB (−98.7%), `uniform_build_cpu_ms` 380 → 15 (−96%), pixel-identical output, fps within noise of baseline. Cleanup follow-ups: replace encoder's `LocalDirtyState` stand-in with C1's `DirtyState` type, remove legacy `DrawUniforms` host struct + MSL declaration. |
| Render-pass load/store action policy (`R-BACK-15.1`–`R-BACK-15.16`) | ❌ | `specs/backend/render-pass-actions/` adopted as the contract. Today the encoder uses `MTLLoadActionLoad` for ~75% of color attachments (17,793 of 24,995 in SFIV `-benchmark` 1272-frame run) and `MTLStoreActionStore` unconditionally on RT change (per the original `R-BACK-2.6`, now amended). Empirical motivation: SFIV pays ~150 MB/frame of pure tile preservation traffic on Apple TBDR vs ~0 for vanilla wined3d-on-GL. Missing: per-CommandQueue touched-attachment-handle set, `R-BACK-15.4` first-use DontCare-load wiring in `beginRenderPass`, `R-BACK-15.7`/`15.8` live-out look-ahead at `flushRender`, `render_pass_load_action_*` / `_store_action_*` / `tile_preservation_bytes` perf counters, `tests/native/backend/render_pass_actions_spec.cpp` with the 8 cases enumerated in section 6 of design.md. Performance contract: ≥30% Load reduction, ≥50% depth Store reduction, `completion_present_wait_ms` ≥20% reduction on SFIV. |
