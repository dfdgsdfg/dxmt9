...
| Attachment / render-state intent | ⚠️ | Pipeline-key/render-pass tests cover descriptors and load/store policy; shader-runner covers color-write, MRT basics, `oDepth` depth-test intent, alpha-test discard, alpha-blend RGB/alpha, FFP linear fog, and sRGB write behavior. | Add additional depth/stencil, MRT interaction, and clear/load/store combined behavior probes. |
...
|---|---|---|---|
| `texbem_test` / `TEXBEM` / `TEXBEML` | ⚠️ | Native FFP key/source-contract coverage records bump-env texture-stage state and SM1 opcode identity/lowering seams. | Shader-runner runtime coverage remains deferred until the legacy `ps_1_x` dependent texture/bump-env path has parser, emitter, and readback support that can distinguish coordinate perturbation from ordinary sampling. |
...
| `ps_1_1` / `ps_1_2` / `ps_1_3` / `ps_1_4` runtime shader lanes | ⚠️ | Native shader source-contract coverage now decodes and lowers representative SM1 pixel opcodes, including legacy texture/dependent-texture families. | `shader_runner_dxmt9` still has no runtime readback lane for SM1 pixel bytecode. Add parser/emitter/runtime fixtures with deterministic texture/depth oracles before marking these models supported by runtime evidence. |
| `vs_1_1` runtime shader lane | ⚠️ | Native shader source-contract coverage now decodes and lowers representative `vs_1_1` fixed-output style bytecode. | `shader_runner_dxmt9` still has no `vs_1_1` runtime fixture with matching declaration, constant, and readback setup. |
| SM1 dependent texture and depth op families | ⚠️ | Source-contract coverage records `TEXBEM*`, `TEXCOORD`, `TEXDEPTH`, `TEXDP3*`, `TEXM3x*`, and `TEXREG2*` opcode identity/lowering. | Runtime readback is deferred until the runner can express legacy dependent-coordinate sampling, bump-env perturbation, `ps_1_4` depth write/readback, and matrix texture op expectations. |
| `VFACE`, `POSITIONT`, and pretransformed-varying shader fixtures | ⚠️ | Source-contract and EXP probes now cover fragment `POSITION`/`VPOS`, `XYZRHW` draw intent, and undeclared/default varying intent at adjacent seams. | Deterministic runtime fixtures are still missing for face-orientation input, `POSITIONT`/pretransformed interpolation, and Wine `pretransformed_varying_test` / `vface_register_test` parity. |
| Shader constant and FFP TSS constant runtime matrices | ⚠️ | PE conformance scaffolds now cover VS/PS constants and stateblock constant apply; EXP `per-stage-constant` records a valid failing probe. | Broader runtime evidence needs shader_runner or EXP hooks for float/int/bool constant edge cases, TSS `D3DTSS_CONSTANT`, `D3DTA_TEMP`, and `RESULTARG` dataflow. |
| Current shader failing evidence | ⚠️ | Manifested failing shader-runner entries now include FFP `DOTPRODUCT3` alpha routing, unbound sampler alpha, `DSY` over `VPOS`, `ps_3_0` nested loop readback, `VPOS` fragment coords, and `MOVA` relative color. SH-D EXP probes also record shader fog, vertex texture fetch, FFP W/RHW, texture transform projection, texcoord-index matrix, uninitialized varyings, per-stage constant, and fragment coords failures. | Fix runtime/compiler/state handling, then promote each failing entry to passing only after direct runner/experiment evidence and manifest status are updated together. |
...
| Programmable skinned draw result | ✅ | `dxmt9-shader-corpus-vs_specific-dxmt9_vs_skinned_triangle` passes and proves a deterministic framebuffer mask from multi-stream declaration, exact VS constants, indexed matrix access, and matrix deformation. | Broaden beyond the minimum focused fixture. |
| Fixed-function vertex blending | ⚠️ | FFP vertex-blend runtime probes now pass for non-indexed one-weight declaration, 2/3-weight declarations, indexed declaration matrix selection, and FVF `XYZB2` beta-weight decoding. The FFP VS reads `BLENDWEIGHT` / `BLENDINDICES`, uses the selected world matrices, and produces the expected rendered pose. | Add last-beta indexed FVF variants, normal/lighting interactions, and broader matrix/weight edge cases. |
...
| `shader_runner_dxmt9` extended probe layer | ⚠️ | dxmt9-local runtime probes now cover texture setup/readback, dependent texture read, VS geometry, multi-stream VS texturing, programmable skinning pose (`vs_specific/dxmt9_vs_skinned_triangle.shader_test`), viewport-bounded and nonzero-origin rasterization, half-pixel edge masks, color-write/MRT render-state interactions, `oDepth` depth testing (`render_state/dxmt9_odepth_depth_test.shader_test`), alpha-test, alpha-blend alpha-channel, fill-mode wireframe, clip-plane runtime, FFP linear fog, sRGB write, sampler sRGB decode, and FFP vertex-blending intent across one-weight, 2/3-weight, indexed, and FVF `XYZB2` probes. | Broader mip/4x4/LOD variants, FFP last-beta indexed FVF variants, normal/lighting vertex-blend interactions, and combined render-state/resource interactions remain open. |
...
| Expanded `.shader_test` corpus (arithmetic, comparison, flow control, transcendental, matrix, source modifiers, texture, FFP sanity/alpha test) | ✅ | Corpus manifest tracks passing probes plus known failing regression/intent probes. Alpha-test readback remains tracked as a failing regression probe; FFP vertex blending now has passing one-weight, 2/3-weight, indexed declaration, and FVF `XYZB2` runtime probes. Meson shards only `status = "passing"` entries for the normal shader-corpus suite. |
...
| Fixed-function `.shader_test` files | ⚠️ | `ffp/alpha_test.shader_test`, native fixed-function coverage, and the FFP vertex-blend runtime probe set pass for one-weight, 2/3-weight, indexed, and FVF `XYZB2` paths. |
