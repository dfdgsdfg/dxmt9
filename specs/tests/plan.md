...
| Declaration source | FVF/key tests, vertex-decl hash tests, programmable FVF<->decl recorder transitions, color/multistream readback, `POSITION+NORMAL` FFP-decodable readback, declaration A->B readback, and `testTangentUsageRejectsCleanly` for unsupported usage | Closed for supported runtime usages currently exposed by shader-runner; `TANGENT`/`BINORMAL` are safe-reject coverage until Metal lowering exists | Add more runtime usage probes only when new supported usages are promoted |
