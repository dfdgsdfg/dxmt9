...
| `DrawParam` / `FlatDrawStateView` -> encoder draw inputs | Metal primitive type, indexed vs non-indexed method, index count, index type, base vertex, vertex buffer offset, stream stride, `DrawVolatile` values, user vs bound buffer source | `dxmt9-metal-encoder-recorder-spec`, focused geometry diagnostics, selected backend tests | The WMT wrapper seam now records final `drawIndexedPrimitives` command payloads, including the 3DMark05 fan-fix shape. A fuller encodeDraw integration seam is still needed for `DrawVolatile`, live vertex/index buffer source selection, and final command ordering across the whole draw issue path. |
