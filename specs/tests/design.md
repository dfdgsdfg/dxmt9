...
| Render state -> backend descriptors / raster plans | Blend factors/ops, color write masks, DSS compare/write values, cull/front-face, viewport/scissor, alpha/fog/sRGB policy | `backend_pipeline_key_spec`, `render_pass_actions_spec`, raster/core coverage, shader-runner cull/depth/scissor/color-write/blend probes | Combined state behavior still needs runtime readback for interactions static descriptors cannot prove; alpha-blend alpha-channel output is currently a failing probe. |
