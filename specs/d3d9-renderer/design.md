# D3D9 Renderer (Modern Path) Design
...
consumes (`specs/backend/design.md` §4-§5), and shares the PSO cache, ring
...
per R-BACK-32.5 and design.md §5.1, so it must run after memoryless;
...
existing `ArgumentEncodingContext` defined in `specs/backend/design.md` §4
...
This split is a **backend-spec change** (`specs/backend/design.md`) that
this spec depends on. The L0 layer in §14 lands the AEC split together
with the renderer factory; it is not optional. Tracked in `specs/gap.md`
...
tracked in `specs/gap.md` under the modern renderer row.
...
- `specs/backend/design.md` — shared Metal infrastructure this design
