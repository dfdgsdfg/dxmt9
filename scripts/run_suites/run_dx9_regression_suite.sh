...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_suites/run_dx9_regression_suite.sh [options]

Options:
  --wine-root <path>      Wine runtime root passed to run_experiment.py.
  --pe-build-dir <path>   PE build dir passed to run_experiment.py.
...
This script runs the current DX9 regression catalogue and writes:
  experiments/output/dx9-regression-suite/summary.json
  experiments/output/dx9-regression-suite/summary.md
EOF
}
...
suite_dir="$repo_root/experiments/output/dx9-regression-suite"
mkdir -p "$suite_dir"
...
for app in "${apps[@]}"; do
  ...
  if [[ -n "$pe_build_dir" ]]; then
    cmd+=(--pe-build-dir "$pe_build_dir")
  fi
...
for app in apps:
    result_path = output_root / app / "result.json"
