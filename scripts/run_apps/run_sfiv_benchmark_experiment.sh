...
if [[ "$has_binary" == false ]]; then
  if resolved_binary=$(discover_binary); then
  ...
  else
    {
      ...
      echo "  bash scripts/run_apps/run_sfiv_benchmark_experiment.sh --binary \"/path/to/Benchmark.exe\""
    } >&2
    exit 2
  fi
fi
...
cmd=(python3 "$repo_root/scripts/run_apps/run_experiment.py" run "$app_name")
