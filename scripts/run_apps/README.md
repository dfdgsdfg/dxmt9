...
- `run_app-d3d9-3dmark05-verify_direct.sh` — direct foreground runner for the
  ...
  heuristic run needs different switches. By default it stages the current
  dxmt9 x64/x86 PE DLLs plus the unix provider into the verify prefix before
  launch; set `DXMT_3DMARK05_STAGE=0` only when intentionally testing the
  already-installed prefix binaries. Auto-Enter is controlled with
