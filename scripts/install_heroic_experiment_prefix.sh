...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/install_heroic_experiment_prefix.sh --prefix <wine-prefix> [options]
...
if [[ "$skip_build" != true ]]; then
  ...
  bash "$repo_root/scripts/build_irrlicht_managed_lights.sh"
fi

cmd=(
  bash "$repo_root/scripts/install_heroic_wine.sh"
  --prefix "$prefix"
...
Experiment commands:
  ...
  bash scripts/run_irrlicht_managed_lights_experiment.sh --prefix "$prefix"${wine_root:+ --wine-root "$wine_root"}
EOF
